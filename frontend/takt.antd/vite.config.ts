...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        // 统一使用 api 前缀的路由（无前导斜杠）
        // 前端请求 api/xxx，Vite 代理匹配 api/xxx，转发到后端的 /api/xxx
        // 使用正则表达式匹配，支持有/无前导斜杠
        '^/?(api/.*)$': {
          ...
          configure: (proxy, options) => {
            ...
            proxy.on('proxyReq', (proxyReq, req, res) => {
              console.log('[Vite Proxy] 转发请求:', req.method, req.url, '->', options.target)
            })
        ...
        '/hubs': {
          ...
          configure: (proxy, options) => {
            proxy.on('error', (err, req, res) => {
              console.error('[Vite Proxy WS Error]', err.message, req.url)
            })
          }
        }
      }
