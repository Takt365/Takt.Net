...
<template>
  <div class="accounting-financial-company">
    <TaktQueryBar
      ...
      :placeholder="t('common.form.placeholder.search', { keyword: [t('entity.company.name'), t('entity.company.code')].join(t('common.action.or')) })"
      :loading="loading"
      @search="handleSearch"
      @reset="handleReset"
    />
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'companyStatus'">
          <TaktDictTag
            :value="getCompanyField(record, 'companyStatus')"
    ...
    >
      ...
      <a-form-item :label="t('entity.company.code')">
      ...
      <a-form-item :label="t('entity.company.status')">
        <TaktSelect
          ...
          dict-type="sys_normal_disable"
          :placeholder="t('common.form.placeholder.select', { field: t('entity.company.status') })"
          allow-clear
        />
      </a-form-item>
    </TaktQueryDrawer>
    ...
    <TaktModal
      ...
      :title="t('common.button.import') + t('entity.company._self')"
      :width="600"
      :footer="null"
    ...
    >
      <TaktImportFile
        file-type="xlsx"
        :sheet-name="t('common.action.import.sheetNameTemplate', { entity: t('entity.company._self') })"
        :template-file-name="t('common.action.import.sheetNameTemplate', { entity: t('entity.company._self') })"
        template-permission="accounting:financial:company:template"
        import-permission="accounting:financial:company:import"
        :download-template="handleDownloadTemplate"
        :import-file="handleImportFile"
        :template-text="t('common.action.import.templateText', { entity: t('entity.company._self') })"
...
import type { Company } from '@/types/accounting/financial/company'
...
const getCompanyId = (record: any): string =>
  record?.companyId != null ? String(record.companyId) : record?.id != null ? String(record.id) : ''
...
const columns = computed<TableColumnsType>(() => [
  {
    ...
    customRender: ({ record }: { record: any }) =>
      getCompanyField(record, 'companyId') ?? getCompanyField(record, 'id') ?? ''
  },
  {
...
const onClickRow = (record: Company) => ({
  onClick: () => {
    ...
    selectedRows.value = dataSource.value.filter(item =>
      selectedRowKeys.value.includes(getCompanyId(item))
    )
    selectedRow.value =
...
const loadData = async () => {
  try {
    ...
    if (advancedQueryForm.value.companyName)
      params.companyName = advancedQueryForm.value.companyName
    if (advancedQueryForm.value.companyCode)
      params.companyCode = advancedQueryForm.value.companyCode
    if (advancedQueryForm.value.companyStatus !== undefined)
      params.companyStatus = advancedQueryForm.value.companyStatus
...
const handleTableChange = (_pagination: any, _filters: any, sorter: any) => {
  if (sorter?.order) logger.debug('[Company] 排序:', sorter.field, sorter.order)
}
...
const handleCreate = () => {
  formTitle.value = t('common.button.create') + t('entity.company._self')
  formData.value = {}
  formVisible.value = true
}

const handleEdit = (record: Company) => {
  formTitle.value = t('common.button.edit') + t('entity.company._self')
  ...
  if (selectedRow.value) handleEdit(selectedRow.value)
    ...
    message.warning(
      t('common.action.warnSelectToAction', {
        ...
        entity: t('entity.company._self')
      })
    )
}

const handleDeleteOne = (record: Company) => {
  const name = getCompanyField(record, 'companyName') || getCompanyId(record)
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        loading.value = true
        await deleteCompany(getCompanyId(record))
        message.success(t('common.msg.deleteSuccess', { target: t('entity.company._self') }))
        loadData()
      } catch (error: any) {
        message.error(error?.message || t('common.msg.deleteFail', { target: t('entity.company._self') }))
      } finally {
        loading.value = false
      }
    }
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        loading.value = true
        await Promise.all(
          selectedRows.value.map(record => deleteCompany(getCompanyId(record)))
        )
        message.success(t('common.msg.deleteSuccess', { target: t('entity.company._self') }))
        selectedRows.value = []
...
const handleFormSubmit = async () => {
  try {
    ...
    if (formData.value?.companyId) {
      await update(formData.value.companyId, {
        ...formValues,
        companyId: formData.value.companyId
      })
      message.success(t('common.msg.updateSuccess', { target: t('entity.company._self') }))
    } else {
      await create(formValues)
      message.success(t('common.msg.createSuccess', { target: t('entity.company._self') }))
    }
...
const handleImportFile = async (
  ...
  return await importCompanies(file, sheetName)
}
...
const handleExport = async () => {
  try {
    ...
    if (advancedQueryForm.value.companyName)
      queryParams.companyName = advancedQueryForm.value.companyName
    if (advancedQueryForm.value.companyCode)
      queryParams.companyCode = advancedQueryForm.value.companyCode
    if (advancedQueryForm.value.companyStatus !== undefined)
      queryParams.companyStatus = advancedQueryForm.value.companyStatus
    const blob = await exportCompanies(
      queryParams,
      undefined,
      t('entity.company._self') + t('common.action.exportDataSuffix')
    )
    const ts = new Date()
    const pad = (n: number, w = 2) => String(n).padStart(w, '0')
    const fileName = `${t('entity.company._self')}${t('common.action.exportDataSuffix')}_${ts.getFullYear()}${pad(ts.getMonth() + 1)}${pad(ts.getDate())}${pad(ts.getHours())}${pad(ts.getMinutes())}${pad(ts.getSeconds())}.xlsx`
    ...
    message.success(t('common.msg.exportSuccess', { target: t('entity.company._self') }))
  } catch (error: any) {
    message.error(error?.message || t('common.msg.exportFail', { target: t('entity.company._self') }))
  } finally {
    loading.value = false
  }
}
