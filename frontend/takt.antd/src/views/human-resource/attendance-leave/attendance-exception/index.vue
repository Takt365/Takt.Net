  ...
  <div class="humanresource-attendance-leave-exception">
    ...
    >
      <TaktImportFile
        ...
        :template-text="t('common.page.action.import.templatetext', { entity: t('entity.attendanceexception._self') })"
...
const handleUpdate = () => {
  if (selectedRow.value) handleEdit(selectedRow.value)
    ...
    message.warning(
      t('common.page.action.warnselecttoaction', { action: t('common.page.button.edit'), entity: t('entity.attendanceexception._self') })
    )
}
...
const handleDeleteOne = (record: AttendanceException) => {
  ...
  Modal.confirm({
    ...
    content: t('common.page.confirm.deleteentity', { entity: t('entity.attendanceexception._self'), name }),
    ...
    onOk: async () => {
      try {
        ...
        message.success(t('common.page.msg.deletesuccess', { target: t('entity.attendanceexception._self') }))
        loadData()
      } catch (error: unknown) {
        message.error(getErrorMessage(error) || t('common.page.msg.deletefail', { target: t('entity.attendanceexception._self') }))
      } finally {
        loading.value = false
      }
    }
...
const handleDelete = () => {
  if (selectedRows.value.length === 0) {
    message.warning(
      t('common.page.action.warnselecttoaction', { action: t('common.page.button.delete'), entity: t('entity.attendanceexception._self') })
    )
    return
  }
  Modal.confirm({
...
const handleFormSubmit = async () => {
  try {
    ...
    if (id != null && String(id).length > 0) {
      ...
      message.success(t('common.page.msg.updatesuccess', { target: t('entity.attendanceexception._self') }))
    } else {
      await createAttendanceException(formValues as AttendanceExceptionCreate)
      message.success(t('common.page.msg.createsuccess', { target: t('entity.attendanceexception._self') }))
    }
...
const handleExport = async () => {
  try {
    ...
    message.success(t('common.page.msg.exportsuccess', { target: t('entity.attendanceexception._self') }))
  } catch (error: unknown) {
    logger.error('[AttendanceException] 导出失败:', error)
    message.error(getErrorMessage(error) || t('common.page.msg.exportfail', { target: t('entity.attendanceexception._self') }))
  } finally {
    loading.value = false
  }
}
