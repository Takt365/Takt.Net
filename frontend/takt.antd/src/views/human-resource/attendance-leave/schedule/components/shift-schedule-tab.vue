  ...
  <div class="humanresource-attendance-leave-schedule-shiftschedule">
    ...
    >
      <TaktImportFile
        ...
        :template-text="t('common.page.action.import.templatetext', { entity: t('entity.shiftschedule._self') })"
...
const handleUpdate = () => {
  ...
  else message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.edit'), entity: entitySelf.value }))
}
const handleDeleteOne = (record: Record<string, unknown>) => {
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deleteentity', { entity: entitySelf.value, name: getRowId(record) }),
    ...
    onOk: async () => {
      try {
        ...
        message.success(t('common.page.msg.deletesuccess', { target: entitySelf.value }))
        void loadData()
      } catch (error: unknown) {
        message.error(getErrorMessage(error) || t('common.page.msg.deletefail', { target: entitySelf.value }))
      } finally {
        loading.value = false
      }
    }
...
const handleDelete = () => {
  if (selectedRows.value.length === 0) {
    message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.delete'), entity: entitySelf.value }))
    return
  }
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deletecountentity', { entity: entitySelf.value, count: selectedRows.value.length }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      try {
...
const handleFormSubmit = async () => {
  try {
    ...
    if (id != null && String(id).length > 0) {
      ...
      message.success(t('common.page.msg.updatesuccess', { target: entitySelf.value }))
    } else {
      await createShiftSchedule(formValues)
      message.success(t('common.page.msg.createsuccess', { target: entitySelf.value }))
    }
...
const handleExport = async () => {
  try {
    ...
    const blob = await exportShiftScheduleData(
      queryParams,
      undefined,
      entitySelf.value + t('common.page.action.exportdatasuffix')
    )
    const ts = new Date()
    const pad = (n: number, w = 2) => String(n).padStart(w, '0')
    const fileName = `${entitySelf.value + t('common.page.action.exportdatasuffix')}_${ts.getFullYear()}${pad(ts.getMonth() + 1)}${pad(ts.getDate())}${pad(ts.getHours())}${pad(ts.getMinutes())}${pad(ts.getSeconds())}.xlsx`
    ...
    message.success(t('common.page.msg.exportsuccess', { target: entitySelf.value }))
  } catch (error: unknown) {
    logger.error('[ShiftSchedule] 导出失败:', error)
    message.error(getErrorMessage(error) || t('common.page.msg.exportfail', { target: entitySelf.value }))
  } finally {
    loading.value = false
  }
}
