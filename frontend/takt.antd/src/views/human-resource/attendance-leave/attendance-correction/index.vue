  ...
  <div class="humanresource-attendance-leave-correction">
    ...
    >
      <TaktImportFile
        ...
        :template-text="t('common.page.action.import.templatetext', { entity: t('entity.attendancecorrection._self') })"
...
const handleDeleteOne = (record: AttendanceCorrection) => {
  ...
  Modal.confirm({
    ...
    content: t('common.page.confirm.deleteentity', { entity: t('entity.attendancecorrection._self'), name }),
    ...
    onOk: async () => {
      try {
        ...
        message.success(t('common.page.msg.deletesuccess', { target: t('entity.attendancecorrection._self') }))
        loadData()
      } catch (error: unknown) {
        message.error(getErrorMessage(error) || t('common.page.msg.deletefail', { target: t('entity.attendancecorrection._self') }))
      } finally {
        loading.value = false
      }
    }
...
const handleFormSubmit = async () => {
  try {
    ...
    if (cid != null && String(cid).length > 0) {
      ...
      message.success(t('common.page.msg.updatesuccess', { target: t('entity.attendancecorrection._self') }))
    } else {
      await createAttendanceCorrection(formValues as AttendanceCorrectionCreate)
      message.success(t('common.page.msg.createsuccess', { target: t('entity.attendancecorrection._self') }))
    }
...
const handleExport = async () => {
  try {
    ...
    message.success(t('common.page.msg.exportsuccess', { target: t('entity.attendancecorrection._self') }))
  } catch (error: unknown) {
    logger.error('[AttendanceCorrection] 导出失败:', error)
    message.error(getErrorMessage(error) || t('common.page.msg.exportfail', { target: t('entity.attendancecorrection._self') }))
  } finally {
    loading.value = false
  }
}
