  ...
  <div class="humanresource-attendance-leave-device">
    ...
    >
      <TaktImportFile
        ...
        :template-text="t('common.page.action.import.templatetext', { entity: t('entity.attendancedevice._self') })"
...
const handleUpdate = () => {
  if (selectedRow.value) handleEdit(selectedRow.value)
    ...
    message.warning(
      t('common.page.action.warnselecttoaction', { action: t('common.page.button.edit'), entity: t('entity.attendancedevice._self') })
    )
}
...
const handleDeleteOne = (record: AttendanceDevice) => {
  ...
  Modal.confirm({
    ...
    content: t('common.page.confirm.deleteentity', { entity: t('entity.attendancedevice._self'), name }),
    ...
    onOk: async () => {
      try {
        ...
        message.success(t('common.page.msg.deletesuccess', { target: t('entity.attendancedevice._self') }))
        loadData()
      } catch (error: unknown) {
        message.error(getErrorMessage(error) || t('common.page.msg.deletefail', { target: t('entity.attendancedevice._self') }))
      } finally {
        loading.value = false
      }
    }
...
const handleDelete = () => {
  if (selectedRows.value.length === 0) {
    message.warning(
      t('common.page.action.warnselecttoaction', { action: t('common.page.button.delete'), entity: t('entity.attendancedevice._self') })
    )
    return
  }
  Modal.confirm({
...
const handleFormSubmit = async () => {
  try {
    ...
    if (id != null && String(id).length > 0) {
      ...
      message.success(t('common.page.msg.updatesuccess', { target: t('entity.attendancedevice._self') }))
    } else {
      await createAttendanceDevice(formValues as AttendanceDeviceCreate)
      message.success(t('common.page.msg.createsuccess', { target: t('entity.attendancedevice._self') }))
    }
...
const handleExport = async () => {
  try {
    ...
    message.success(t('common.page.msg.exportsuccess', { target: t('entity.attendancedevice._self') }))
  } catch (error: unknown) {
    logger.error('[AttendanceDevice] 导出失败:', error)
    message.error(getErrorMessage(error) || t('common.page.msg.exportfail', { target: t('entity.attendancedevice._self') }))
  } finally {
    loading.value = false
  }
}
