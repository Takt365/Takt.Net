...
<template>
  <div class="humanresource-attendance-leave-holiday">
    ...
    <TaktQueryBar
      ...
      :placeholder="t('common.page.form.placeholder.search', { keyword: t('entity.holiday.keyword') })"
      :loading="loading"
      @search="handleSearch"
      @reset="handleReset"
    />
    ...
    >
      <TaktImportFile
        file-type="xlsx"
        :sheet-name="t('common.page.action.import.sheetnametemplate', { entity: t('entity.holiday._self') })"
        :template-file-name="t('common.page.action.import.sheetnametemplate', { entity: t('entity.holiday._self') })"
        template-permission="humanresource:attendanceleave:holiday:template"
        import-permission="humanresource:attendanceleave:holiday:import"
        :download-template="handleDownloadTemplate"
        :import-file="handleImportFile"
        :template-text="t('common.page.action.import.templatetext', { entity: t('entity.holiday._self') })"
...
const handleUpdate = () => {
  ...
  else message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.edit'), entity: t('entity.holiday._self') }))
}
...
const handleDeleteOne = (record: Holiday) => {
  ...
  Modal.confirm({
    ...
    content: t('common.page.confirm.deleteentity', { entity: t('entity.holiday._self'), name }),
    ...
    onOk: async () => {
      try {
        ...
        message.success(t('common.page.msg.deletesuccess', { target: t('entity.holiday._self') }))
        loadData()
      } catch (error: unknown) {
        message.error(getErrorMessage(error) || t('common.page.msg.deletefail', { target: t('entity.holiday._self') }))
      } finally {
        loading.value = false
      }
    }
...
const handleDelete = () => {
  if (selectedRows.value.length === 0) {
    message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.delete'), entity: t('entity.holiday._self') }))
    return
  }
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deletecountentity', { entity: t('entity.holiday._self'), count: selectedRows.value.length }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      try {
...
const handleFormSubmit = async () => {
  try {
    ...
    if (formData.value?.holidayId) {
      ...
      message.success(t('common.page.msg.updatesuccess', { target: t('entity.holiday._self') }))
    } else {
      await createHoliday(formValues as HolidayCreate)
      message.success(t('common.page.msg.createsuccess', { target: t('entity.holiday._self') }))
    }
...
const handleExport = async () => {
  try {
    ...
    const blob = await exportHolidayData(
      queryParams,
      undefined,
      t('entity.holiday._self') + t('common.page.action.exportdatasuffix')
    )
    const ts = new Date()
    const pad = (n: number, w = 2) => String(n).padStart(w, '0')
    const fileName = `${t('entity.holiday._self') + t('common.page.action.exportdatasuffix')}_${ts.getFullYear()}${pad(ts.getMonth() + 1)}${pad(ts.getDate())}${pad(ts.getHours())}${pad(ts.getMinutes())}${pad(ts.getSeconds())}.xlsx`
    ...
    message.success(t('common.page.msg.exportsuccess', { target: t('entity.holiday._self') }))
  } catch (error: unknown) {
    logger.error('[Holiday] 导出失败:', error)
    message.error(getErrorMessage(error) || t('common.page.msg.exportfail', { target: t('entity.holiday._self') }))
  } finally {
    loading.value = false
  }
}
