...
<template>
  <div class="humanresource-attendance-leave-leave">
    ...
    <TaktQueryBar
      ...
      :placeholder="t('common.page.form.placeholder.search', { keyword: t('entity.leave.keyword') })"
      :loading="loading"
      @search="handleSearch"
      @reset="handleReset"
    />
    ...
    >
      ...
      <a-form-item :label="t('entity.leave.leavestatus')">
        <TaktSelect
          ...
          dict-type="hr_leave_status"
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.leave.leavestatus') })"
          allow-clear
          style="width: 100%"
        />
      </a-form-item>
    ...
    >
      <TaktImportFile
        ...
        :template-text="t('common.page.action.import.templatetext', { entity: t('entity.leave._self') })"
...
const handleClone = (record: Leave) => {
  formTitle.value = t('common.page.button.clone') + t('entity.leave._self')
...
const handleUpdate = () => {
  ...
  else message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.edit'), entity: t('entity.leave._self') }))
}
...
const handleDeleteOne = (record: Leave) => {
  ...
  Modal.confirm({
    ...
    content: t('common.page.confirm.deleteentity', { entity: t('entity.leave._self'), name }),
    ...
    onOk: async () => {
      try {
        ...
        message.success(t('common.page.msg.deletesuccess', { target: t('entity.leave._self') }))
        loadData()
      } catch (error: unknown) {
        message.error(getErrorMessage(error) || t('common.page.msg.deletefail', { target: t('entity.leave._self') }))
      } finally {
        loading.value = false
      }
    }
...
const handleDelete = () => {
  if (selectedRows.value.length === 0) {
    message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.delete'), entity: t('entity.leave._self') }))
    return
  }
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deletecountentity', { entity: t('entity.leave._self'), count: selectedRows.value.length }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      try {
...
const handleFormSubmit = async () => {
  try {
    ...
    if (formData.value?.leaveId) {
      ...
      message.success(t('common.page.msg.updatesuccess', { target: t('entity.leave._self') }))
    } else {
      await createLeave(formValues as LeaveCreate)
      message.success(t('common.page.msg.createsuccess', { target: t('entity.leave._self') }))
    }
...
const handleExport = async () => {
  try {
    ...
    message.success(t('common.page.msg.exportsuccess', { target: t('entity.leave._self') }))
  } catch (error: unknown) {
    logger.error('[Leave] 导出失败:', error)
    message.error(getErrorMessage(error) || t('common.page.msg.exportfail', { target: t('entity.leave._self') }))
  } finally {
    loading.value = false
  }
}
