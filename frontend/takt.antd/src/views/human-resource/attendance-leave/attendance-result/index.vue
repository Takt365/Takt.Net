  ...
  <div class="humanresource-attendance-leave-result">
    ...
    >
      <TaktImportFile
        ...
        :template-text="t('common.page.action.import.templatetext', { entity: t('entity.attendanceresult._self') })"
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        ...
        message.success(t('common.page.msg.deletesuccess', { target: t('entity.attendanceresult._self') }))
        loadData()
      } catch (error: unknown) {
        message.error(getErrorMessage(error) || t('common.page.msg.deletefail', { target: t('entity.attendanceresult._self') }))
      } finally {
        loading.value = false
      }
    }
...
const handleFormSubmit = async () => {
  try {
    ...
    if (id != null && String(id).length > 0) {
      ...
      message.success(t('common.page.msg.updatesuccess', { target: t('entity.attendanceresult._self') }))
    } else {
      await createAttendanceResult(formValues as AttendanceResultCreate)
      message.success(t('common.page.msg.createsuccess', { target: t('entity.attendanceresult._self') }))
    }
...
const handleExport = async () => {
  try {
    ...
    message.success(t('common.page.msg.exportsuccess', { target: t('entity.attendanceresult._self') }))
  } catch (error: unknown) {
    logger.error('[AttendanceResult] 导出失败:', error)
    message.error(getErrorMessage(error) || t('common.page.msg.exportfail', { target: t('entity.attendanceresult._self') }))
  } finally {
    loading.value = false
  }
}
