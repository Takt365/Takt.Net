...
<template>
  <div class="humanresource-personnel-employee-attachment">
    ...
    <TaktQueryBar
      ...
      :placeholder="t('common.page.form.placeholder.search', { keyword: t('entity.employeeattachment.keyword') })"
      :loading="loading"
      @search="handleSearch"
      @reset="handleReset"
    />
    ...
    >
      <TaktImportFile
        file-type="xlsx"
        :sheet-name="t('common.page.action.import.sheetnametemplate', { entity: t('entity.employeeattachment._self') })"
        :template-file-name="t('common.page.action.import.sheetnametemplate', { entity: t('entity.employeeattachment._self') })"
        template-permission="humanresource:personnel:employeeattachment:template"
        import-permission="humanresource:personnel:employeeattachment:import"
        :download-template="handleDownloadTemplate"
        :import-file="handleImportFile"
        :template-text="t('common.page.action.import.templatetext', { entity: t('entity.employeeattachment._self') })"
...
const columns = computed<TableColumnsType>(() => [
  ...
  { title: t(`${entityKey}.sortOrder`), dataIndex: 'sortOrder', key: 'sortOrder', width: 80 },
  ...
  CreateActionColumn<EmployeeAttachment>({
    actions: [
      { key: 'update', label: t('common.page.button.edit'), shape: 'plain', icon: RiEditLine, permission: 'humanresource:personnel:employeeattachment:update', onClick: (r: EmployeeAttachment) => handleEdit(r) },
      { key: 'delete', label: t('common.page.button.delete'), shape: 'plain', icon: RiDeleteBinLine, permission: 'humanresource:personnel:employeeattachment:delete', onClick: (r: EmployeeAttachment) => handleDeleteOne(r) }
    ]
  })
])
...
async function loadData() {
  try {
    ...
    message.error(e?.message || t('common.page.msg.loadfail'))
    dataSource.value = []
    total.value = 0
  } finally {
    loading.value = false
...
async function handleEdit(record: EmployeeAttachment) {
  ...
  } catch (e: any) { message.error(e?.message || t('common.page.msg.loadfail')) } finally { formLoading.value = false }
}
...
function handleDeleteOne(record: EmployeeAttachment) {
  Modal.confirm({
    ...
    content: t('common.page.confirm.deleteentity', { entity: t(`${entityKey}._self`), name: getField(record, 'fileName') || getRowId(record) }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => { try { loading.value = true; await deleteEmployeeAttachmentById(getRowId(record)); message.success(t('common.page.msg.deletesuccess', { target: t(`${entityKey}._self`) })); loadData() } catch (e: any) { message.error(e?.message || t('common.page.msg.deletefail', { target: t(`${entityKey}._self`) })) } finally { loading.value = false } }
  })
}
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        ...
        loadData()
      } catch (e: any) { message.error(e?.message || t('common.page.msg.deletefail', { target: t(`${entityKey}._self`) })) } finally { loading.value = false }
    }
  })
}
...
async function handleFormSubmit() {
  try {
    ...
    message.error(e?.message || t('common.page.msg.operatefail', { action: t('common.page.action.operation') }))
  } finally {
...
async function handleExport() {
  try {
    ...
    const blob = await exportEmployeeAttachmentData(queryParams, undefined, t(`${entityKey}._self`) + t('common.page.action.exportdatasuffix'))
  ...
  } catch (e: any) { logger.error('[EmployeeAttachment] export:', e); message.error(e?.message || t('common.page.msg.exportfail', { target: t(`${entityKey}._self`) })) } finally { loading.value = false }
}
