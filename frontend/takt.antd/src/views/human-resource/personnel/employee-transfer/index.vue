...
<template>
  <div class="humanresource-personnel-employee-transfer">
    ...
    <TaktQueryBar
      ...
      :placeholder="t('common.page.form.placeholder.search', { keyword: t('entity.employeetransfer.keyword') })"
      :loading="loading"
      @search="handleSearch"
      @reset="handleReset"
    />
...
const columns = computed<TableColumnsType>(() => [
  ...
  CreateActionColumn<EmployeeTransfer>({
    actions: [
      { key: 'update', label: t('common.page.button.edit'), shape: 'plain', icon: RiEditLine, permission: 'humanresource:personnel:employeetransfer:update', onClick: (r: EmployeeTransfer) => handleEdit(r) },
      { key: 'delete', label: t('common.page.button.delete'), shape: 'plain', icon: RiDeleteBinLine, permission: 'humanresource:personnel:employeetransfer:delete', onClick: (r: EmployeeTransfer) => handleDeleteOne(r) }
    ]
  })
])
...
async function loadData() {
  ...
  } catch (e: any) { logger.error('[EmployeeTransfer] loadData:', e); message.error(e?.message || t('common.page.msg.loadfail')); dataSource.value = []; total.value = 0 } finally { loading.value = false }
}
...
function handleCreate() { formTitle.value = t('common.page.button.create') + t(`${entityKey}._self`); formData.value = {}; formVisible.value = true }
async function handleEdit(record: EmployeeTransfer) {
  formTitle.value = t('common.page.button.edit') + t(`${entityKey}._self`)
  try { formLoading.value = true; formData.value = { ...(await getEmployeeTransferById(getRowId(record))) }; formVisible.value = true } catch (e: any) { message.error(e?.message || t('common.page.msg.loadfail')) } finally { formLoading.value = false }
}
function handleUpdate() { if (selectedRow.value) handleEdit(selectedRow.value); else message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.edit'), entity: t(`${entityKey}._self`) })) }
function handleDeleteOne(record: EmployeeTransfer) {
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deleteentity', { entity: t(`${entityKey}._self`), name: getField(record, 'reason') || getRowId(record) }),
    okText: t('common.page.button.delete'), cancelText: t('common.page.button.cancel'),
    onOk: async () => { try { loading.value = true; await deleteEmployeeTransferById(getRowId(record)); message.success(t('common.page.msg.deletesuccess', { target: t(`${entityKey}._self`) })); loadData() } catch (e: any) { message.error(e?.message || t('common.page.msg.deletefail', { target: t(`${entityKey}._self`) })) } finally { loading.value = false } }
  })
}
function handleDelete() {
  if (selectedRows.value.length === 0) { message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.delete'), entity: t(`${entityKey}._self`) })); return }
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deletecountentity', { entity: t(`${entityKey}._self`), count: selectedRows.value.length }),
    okText: t('common.page.button.delete'), cancelText: t('common.page.button.cancel'),
    onOk: async () => { try { loading.value = true; selectedRows.value.length === 1 ? await deleteEmployeeTransferById(getRowId(selectedRows.value[0])) : await deleteEmployeeTransferBatch(selectedRows.value.map((r) => getRowId(r))); message.success(t('common.page.msg.deletesuccess', { target: t(`${entityKey}._self`) })); selectedRows.value = []; selectedRowKeys.value = []; selectedRow.value = null; loadData() } catch (e: any) { message.error(e?.message || t('common.page.msg.deletefail', { target: t(`${entityKey}._self`) })) } finally { loading.value = false } }
  })
}
async function handleFormSubmit() {
  try {
    ...
    if (formData.value?.transferId) { await updateEmployeeTransfer(formData.value.transferId, { ...values, transferId: formData.value.transferId }); message.success(t('common.page.msg.updatesuccess', { target: t(`${entityKey}._self`) })) }
    else { await createEmployeeTransfer(values); message.success(t('common.page.msg.createsuccess', { target: t(`${entityKey}._self`) })) }
    formRef.value?.resetFields(); formData.value = {}; formVisible.value = false; loadData()
  } catch (e: any) { if (e?.errorFields) return; message.error(e?.message || t('common.page.msg.operatefail', { action: t('common.page.action.operation') })) } finally { formLoading.value = false }
}
...
async function handleExport() {
  try {
    ...
    const blob = await exportEmployeeTransferData(queryParams, undefined, t(`${entityKey}._self`) + t('common.page.action.exportdatasuffix'))
    const data = (blob as any)?.data ?? blob
    const ts = new Date(); const pad = (n: number, w = 2) => String(n).padStart(w, '0')
    const name = `${t(`${entityKey}._self`) + t('common.page.action.exportdatasuffix')}_${ts.getFullYear()}${pad(ts.getMonth() + 1)}${pad(ts.getDate())}${pad(ts.getHours())}${pad(ts.getMinutes())}${pad(ts.getSeconds())}.xlsx`
  ...
  } catch (e: any) { logger.error('[EmployeeTransfer] export:', e); message.error(e?.message || t('common.page.msg.exportfail', { target: t(`${entityKey}._self`) })) } finally { loading.value = false }
}
