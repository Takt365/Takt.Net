  ...
  <div class="identity-menu">
    ...
    <div class="menu-query-row">
      ...
      <TaktTreeRightQueryBar
        ...
        :placeholder="t('common.page.form.placeholder.search', { keyword: t('entity.menu.name') + t('common.page.action.or') + t('entity.menu.code') })"
        :loading="loading"
        @search="handleSearch"
        @reset="handleReset"
      />
    ...
    >
      <TaktImportFile
        ...
        :template-text="t('common.page.action.import.templatetext', { entity: t('entity.menu._self') })"
...
import { getMenuList, createMenu, updateMenu, updateMenuStatus, deleteMenuById, getMenuTemplate, importMenuData, exportMenuData } from '@/api/identity/menu'
...
const handleMenuTreeDrop = async (payload: TreeDropPayload) => {
  ...
  try {
    loading.value = true
    await updateMenu(String(dragKey), { parentId: pos.parentId, sortOrder: pos.sortOrder } as Partial<MenuUpdate>)
...
async function handleMenuSwitch(record: any, field: 'isVisible' | 'isCache' | 'isExternal', value: number) {
...
// 列顺序与后端 TaktMenu 实体、TaktMenuDto、前端 Menu 类型完全一致
...
const handleUpdate = () => {
  ...
  else message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.edit'), entity: t('entity.menu._self') }))
}
...
const handleDeleteOne = (record: Menu) => {
  ...
  Modal.confirm({
    ...
    content: t('common.page.confirm.deleteentity', { entity: t('entity.menu._self'), name }),
...
const handleDelete = () => {
  if (selectedRows.value.length === 0) {
    message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.delete'), entity: t('entity.menu._self') }))
    return
  }
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deletecountentity', { count: selectedRows.value.length, entity: t('entity.menu._self') }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      try {
...
const handleImportFile = async (file: File, sheetName?: string): Promise<{ success: number; fail: number; errors: string[] }> => {
  return await importMenuData(file, sheetName)
}
...
const handleExport = async () => {
  try {
    ...
    const blob = await exportMenuData(queryParams, menuExcelNames.sheet, menuExcelNames.fileBase)
    const ts = new Date()
