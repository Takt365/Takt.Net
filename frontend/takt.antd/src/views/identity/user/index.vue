...
<template>
  <div class="identity-user">
    ...
    <TaktQueryBar
      ...
      :placeholder="t('common.form.placeholder.required', { field: [t('entity.user.username'), t('entity.user.realname')].join('、') }) + t('common.button.query')"
      :loading="loading"
      @search="handleSearch"
      @reset="handleReset"
    />
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'gender'">
          <TaktDictTag
            :value="getUserField(record, 'gender')"
    ...
    >
      ...
      <a-form-item :label="t('entity.user.realname')">
        <a-input v-model:value="advancedQueryForm.RealName" />
      </a-form-item>
      <a-form-item :label="t('entity.user.useremail')">
        <a-input v-model:value="advancedQueryForm.UserEmail" />
      </a-form-item>
      <a-form-item :label="t('entity.user.userphone')">
        <a-input v-model:value="advancedQueryForm.UserPhone" />
      </a-form-item>
      <a-form-item :label="t('entity.user.userstatus')">
        <TaktSelect
...
import { getList, create, update, deleteUser, unlock, exportUsers, getTemplate, importUsers, resetPassword, changePassword } from '@/api/identity/user'
...
const columns = computed<TableColumnsType>(() => [
  ...
  },
  {
    ...
    ellipsis: true,
    sorter: (a: any, b: any) => {
      const aName = getUserField(a, 'realName') || ''
      const bName = getUserField(b, 'realName') || ''
      return aName.localeCompare(bName)
    }
  },
  {
...
// 下载导入模板（仅传名称；下载文件名在 takt-import-file 里用 名称_时间戳.xlsx）
...
const handleImportFile = async (file: File, sheetName?: string): Promise<{ success: number; fail: number; errors: string[] }> => {
  return await importUsers(file, sheetName)
}
...
const handleExport = async () => {
  try {
    ...
    // 调用导出 API（仅传名称）；下载文件名在视图中用 名称_时间戳.xlsx
    const blob = await exportUsers(queryParams, undefined, t('entity.user._self') + t('common.action.exportDataSuffix'))
    const ts = new Date(); const pad = (n: number, w = 2) => String(n).padStart(w, '0')
    const fileName = `${t('entity.user._self') + t('common.action.exportDataSuffix')}_${ts.getFullYear()}${pad(ts.getMonth()+1)}${pad(ts.getDate())}${pad(ts.getHours())}${pad(ts.getMinutes())}${pad(ts.getSeconds())}.xlsx`
  ...
  try {
    ...
    if (formData.value.userId) {
      ...
      const updateData: any = {
        ...
        userStatus: formValues.userStatus ?? 0,
        remark: formValues.remark || '',
        // ID 数组转换为数字类型（后端期望 List<long>），空数组也保留
        roleIds: Array.isArray(formValues.roleIds) 
          ? formValues.roleIds.map((id: string | number) => Number(id))
          : [],
        deptIds: Array.isArray(formValues.deptIds)
          ? formValues.deptIds.map((id: string | number) => Number(id))
          : [],
        postIds: Array.isArray(formValues.postIds)
          ? formValues.postIds.map((id: string | number) => Number(id))
          : [],
        tenantIds: Array.isArray(formValues.tenantIds)
          ? formValues.tenantIds.map((id: string | number) => Number(id))
          : []
      }
      ...
      // 后端需要 passwordHash，这里先传 password，后端会处理
