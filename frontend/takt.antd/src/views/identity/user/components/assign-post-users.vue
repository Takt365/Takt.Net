...
const loadUserPosts = async () => {
  ...
  try {
    ...
    message.error(error.message || t('common.page.msg.loadtargetfail', { target: t('entity.user._self') + t('entity.post._self') }))
  } finally {
    loading.value = false
...
const handleSubmit = async () => {
  ...
  try {
    ...
    message.success(t('common.page.msg.assignsuccess', { target: t('entity.post._self') }))
    emit('success')
    handleCancel()
  } catch (error: any) {
    logger.error('[AssignPostUsers] 分配岗位失败:', error)
    message.error(error.message || t('common.page.msg.assignfail', { target: t('entity.post._self') }))
  } finally {
    loading.value = false
  }
}
