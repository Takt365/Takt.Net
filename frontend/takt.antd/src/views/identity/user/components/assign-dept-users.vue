...
const loadUserDepts = async () => {
  ...
  try {
    ...
    message.error(error.message || t('common.page.msg.loadtargetfail', { target: t('entity.user._self') + t('entity.dept._self') }))
  } finally {
    loading.value = false
...
const handleSubmit = async () => {
  ...
  try {
    ...
    message.success(t('common.page.msg.assignsuccess', { target: t('entity.dept._self') }))
    emit('success')
    handleCancel()
  } catch (error: any) {
    logger.error('[AssignDeptUsers] 分配部门失败:', error)
    message.error(error.message || t('common.page.msg.assignfail', { target: t('entity.dept._self') }))
  } finally {
    loading.value = false
  }
}
