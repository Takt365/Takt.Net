...
const loadUserRoles = async () => {
  ...
  try {
    ...
    message.error(getErrorMessage(error) || t('common.page.msg.loadtargetfail', { target: t('entity.user._self') + t('entity.role._self') }))
  } finally {
    loading.value = false
...
const handleSubmit = async () => {
  ...
  try {
    ...
    message.success(t('common.page.msg.assignsuccess', { target: t('entity.role._self') }))
    emit('success')
    handleCancel()
  } catch (error: unknown) {
    logger.error('[AssignRoleUsers] 分配角色失败:', error)
    message.error(getErrorMessage(error) || t('common.page.msg.assignfail', { target: t('entity.role._self') }))
  } finally {
    loading.value = false
  }
}
