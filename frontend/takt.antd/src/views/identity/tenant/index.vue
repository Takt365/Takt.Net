...
<template>
  <div class="identity-tenant">
    ...
    <TaktQueryBar
      ...
      :placeholder="t('common.page.form.placeholder.required', { field: [t('entity.tenant.tenantname'), t('entity.tenant.tenantcode')].join('、') }) + t('common.page.button.query')"
      :loading="loading"
      @search="handleSearch"
      @reset="handleReset"
    />
    ...
    >
      ...
      <a-form-item :label="t('entity.tenant.tenantstatus')">
        <TaktSelect
          ...
          :placeholder="t('common.page.form.placeholder.select', { field: t('entity.tenant.tenantstatus') })"
          allow-clear
    ...
    >
      <TaktImportFile
        ...
        :template-text="t('common.page.action.import.templatetext', { entity: t('entity.tenant._self') })"
...
const columns = computed<TableColumnsType>(() => [
  ...
  CreateActionColumn<Tenant>({
    actions: [
      ...
      {
        key: 'enable',
        label: t('common.page.button.enable'),
        shape: 'plain',
        icon: RiCheckLine,
        permission: 'identity:tenant:update',
...
const handleUpdate = () => {
  if (selectedRow.value) {
    ...
    message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.edit'), entity: t('entity.tenant._self') }))
  }
}

// 删除单条
const handleDeleteOne = (record: Tenant) => {
  const name = getTenantField(record, 'tenantName') || t('common.page.action.thistarget', { target: t('entity.tenant._self') })
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deleteentity', { entity: t('entity.tenant._self'), name }),
...
const handleDelete = () => {
  if (selectedRows.value.length === 0) {
    message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.delete'), entity: t('entity.tenant._self') }))
    return
  }
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deletecountentity', { entity: t('entity.tenant._self'), count: selectedRows.value.length }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      try {
...
const handleToggleStatus = (record: Tenant) => {
  ...
  const action = newStatus === 1 ? t('common.page.button.enable') : t('common.page.button.disable')
  Modal.confirm({
    title: t('common.page.action.confirmaction', { action }),
    content: t('common.page.action.confirmaction', { action }) + '？',
    ...
    onOk: async () => {
      try {
        ...
        message.success(t('common.page.msg.actionsuccess', { action }))
        loadData()
      } catch (error: any) {
        message.error(error.message || t('common.page.msg.actionfail', { action }))
      } finally {
        loading.value = false
      }
    }
