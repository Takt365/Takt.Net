...
import {
  getDatabaseConfigs,
  getDatabaseTablesWithGenerator,
  type DatabaseConfig,
  type DatabaseTableInfo
} from '@/api/code/generator/code-generator-specific'
import {
...
function handleDeleteOne(record: GenTable) {
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        loading.value = true
        await deleteGenTable(String(record.genTableId))
...
function handleDelete() {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        loading.value = true
        await Promise.all(selectedRows.value.map((r: GenTable) => deleteGenTable(String(r.genTableId))))
        message.success(t('common.page.msg.deletesuccess'))
...
async function doGenerateCode(record: GenTable, genPathOverride?: string) {
  ...
  if (
    ...
    typeof (result as { fileCount?: unknown }).fileCount === 'number'
  ) {
    const res = result as GenerateCodeResult
    message.success(res.message + (res.fileCount ? t('code.generator.page.gensuccesscount', { count: res.fileCount }) : ''))
    return
  }
...
async function handleImportConfigChange(configId: string) {
  ...
  try {
    ...
    // 使用 getDatabaseTablesWithGenerator 获取尚未生成控制器的表列表，避免覆盖已有功能
    const list = await getDatabaseTablesWithGenerator(configId)
...
async function handleImportSubmit(payload: { configId: string; tableName: string }) {
  try {
    ...
    if (!payload.configId || !payload.tableName) {
