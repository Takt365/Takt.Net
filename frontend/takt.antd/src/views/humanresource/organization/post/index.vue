...
import {
  ...
  importPosts,
  exportPosts
} from '@/api/humanresource/organization/post'
import type { Post } from '@/types/humanresource/organization/post'
...
const handleDelete = () => {
  ...
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        loading.value = true
        await Promise.all(selectedRows.value.map(record => remove(getPostId(record))))
        message.success('删除成功')
        selectedRows.value = []
...
const handleFormSubmit = async () => {
  try {
    ...
    if (formData.value?.postId) {
      await update(formData.value.postId, { ...formValues, postId: formData.value.postId })
...
const handleExport = async () => {
  try {
    ...
    const blob = await exportPosts(queryParams, undefined, '岗位数据')
    const ts = new Date()
