...
import { generateCaptcha as generateCaptchaApi, type CaptchaGenerateResult } from '@/api/identity/captcha-engine/captcha'
...
const productCodeWithVersion = computed(() => {
  return `${t('common.page.app.productcode')} V${appVersion}`
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (result.data.type !== 'Slider' && result.data.type !== 'Behavior') {
      message.error(t('login.page.fields.captcha.validation.typerequired'))
      loading.value = false
      return
    }
    captchaModalType.value = result.data.type as 'Slider' | 'Behavior'
    captchaModalData.value = result.data
...
/** 挂载时请求健康检查以携带 Cookie，利于 CSRF 场景，并同步假日信息 */
onMounted(async () => {
  try {
    const { resetRedirectingToLoginFlag } = await import('@/api/request')
    resetRedirectingToLoginFlag()
  ...
  try {
    ...
    const userStore = useUserStore()
    await userStore.syncHolidayThemeFromPublicApi()
  } catch (error: any) {
    logger.warn('[Login] 同步假日信息失败:', error)
  }
})
</script>
