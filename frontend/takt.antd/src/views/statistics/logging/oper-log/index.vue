      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'operStatus'">
          <a-tag :color="record.operStatus === 0 ? 'success' : 'error'">
            {{ record.operStatus === 0 ? t('common.page.state.success') : t('common.page.state.failed') }}
          </a-tag>
        </template>
      </template>
    </TaktSingleTable>
...
const searchPlaceholder = computed(
  () => t('common.page.form.placeholder.required', { field: [t('entity.operlog.username'), t('entity.operlog.opermodule'), t('entity.operlog.opermethod')].join('、') }) + t('common.page.button.query')
)
...
const handleDeleteOne = (record: OperLog) => {
  ...
  Modal.confirm({
    ...
    content: t('common.page.confirm.deleteentity', { entity: t('entity.operlog._self'), name }),
...
const handleDelete = () => {
  if (selectedRows.value.length === 0) {
    message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.delete'), entity: t('entity.operlog._self') }))
    return
  }
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deletecountentity', { entity: t('entity.operlog._self'), count: selectedRows.value.length }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      try {
