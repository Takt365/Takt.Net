      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'loginStatus'">
          <a-tag :color="record.loginStatus === 0 ? 'success' : 'error'">
            {{ record.loginStatus === 0 ? t('common.page.state.success') : t('common.page.state.failed') }}
          </a-tag>
        </template>
      </template>
    </TaktSingleTable>
...
const searchPlaceholder = computed(
  () => t('common.page.form.placeholder.required', { field: [t('entity.loginlog.username'), t('entity.loginlog.loginip')].join('、') }) + t('common.page.button.query')
)
...
const handleDeleteOne = (record: LoginLog) => {
  ...
  Modal.confirm({
    ...
    content: t('common.page.confirm.deleteentity', { entity: t('entity.loginlog._self'), name }),
...
const handleDelete = () => {
  if (selectedRows.value.length === 0) {
    message.warning(t('common.page.action.warnselecttoaction', { action: t('common.page.button.delete'), entity: t('entity.loginlog._self') }))
    return
  }
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deletecountentity', { entity: t('entity.loginlog._self'), count: selectedRows.value.length }),
    okText: t('common.page.button.delete'),
    cancelText: t('common.page.button.cancel'),
    onOk: async () => {
      try {
