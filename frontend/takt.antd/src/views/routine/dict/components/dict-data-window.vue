...
import type { DictType } from '@/types/routine/dict/dicttype'
import type { DictData, DictDataQuery, DictDataCreate, DictDataUpdate } from '@/types/routine/dict/dictdata'
...
const loadData = async () => {
  ...
  try {
    ...
    const result = await dictDataApi.getList(query)
...
const handleDelete = async () => {
  ...
  try {
    loading.value = true
    if (ids.length === 1) {
      await dictDataApi.remove(ids[0])
    } else {
      // 批量删除
      for (const id of ids) {
...
const handleDeleteOne = async (record: DictData) => {
  ...
  try {
    loading.value = true
    await dictDataApi.remove(record.dictDataId)
    message.success('删除成功')
    await loadData()
...
const handleExport = async () => {
  ...
  try {
    ...
    const blob = await dictDataApi.exportDictDatas(query, undefined, '字典数据')
...
const handleFormSubmit = async () => {
  ...
  try {
    ...
    if ('dictDataId' in formDataValue && formDataValue.dictDataId) {
      // 更新
      await dictDataApi.update(formDataValue.dictDataId, formDataValue as DictDataUpdate)
      message.success('更新成功')
    } else {
      // 新增
      await dictDataApi.create(formDataValue as DictDataCreate)
...
const handleSaveCell = async (record: DictData, field: keyof DictData) => {
  ...
  try {
    ...
    await dictDataApi.update(record.dictDataId, updateData)
