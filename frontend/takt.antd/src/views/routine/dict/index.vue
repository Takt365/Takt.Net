...
<template>
  <div class="routine-dict-type">
    ...
    <!-- 工具栏 -->
    <TaktToolsBar
      create-permission="routine:dict:create"
      update-permission="routine:dict:update"
...
import * as dictTypeApi from '@/api/routine/dict/dicttype'
import * as dictDataApi from '@/api/routine/dict/dictdata'
import type { DictType, DictTypeQuery, DictTypeCreate, DictTypeUpdate } from '@/types/routine/dict/dicttype'
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'routine:dict:update',
      ...
      },
      {
        ...
        permission: 'routine:dict:delete',
...
const loadData = async () => {
  try {
    ...
    const result = await dictTypeApi.getList(query)
...
const handleDelete = async () => {
  ...
  try {
    loading.value = true
    if (ids.length === 1) {
      await dictTypeApi.remove(ids[0])
    } else {
      await dictTypeApi.removeBatch(ids)
    }
...
const handleDeleteOne = async (record: DictType) => {
  ...
  try {
    loading.value = true
    await dictTypeApi.remove(record.dictTypeId)
    message.success('删除成功')
    await loadData()
    // 如果删除的是当前选中的行，清除选中状态
...
const handleExport = async () => {
  try {
    ...
    const blob = await dictTypeApi.exportDictTypes(query, undefined, '字典类型')
...
const handleStatusChange = async (record: DictType, checked: boolean) => {
  try {
    await dictTypeApi.updateStatus({
...
const handleFormSubmit = async () => {
  ...
  try {
    ...
    if ('dictTypeId' in formData && formData.dictTypeId) {
      // 更新
      await dictTypeApi.update(formData.dictTypeId, formData as DictTypeUpdate)
      message.success('更新成功')
    } else {
      // 新增
      await dictTypeApi.create(formData as DictTypeCreate)
...
const loadDictData = async (record: DictType) => {
  ...
  try {
    ...
    const result = await dictDataApi.getList({
      pageIndex: 1,
