  ...
  <div class="routine-i18n">
    <a-tabs v-model:activeKey="activeTab" type="card">
      ...
      <a-tab-pane key="translation" tab="翻译（主）">
        ...
        />
        <TaktToolsBar
          create-permission="routine:i18n:create"
          update-permission="routine:i18n:update"
...
import type { Language, LanguageQuery, LanguageCreate, LanguageUpdate } from '@/types/routine/i18n/language'
import type { Translation, TranslationQuery, TranslationTransposed } from '@/types/routine/i18n/translation'
    ...
    actions: [
      {
        ...
        shape: 'plain',
        icon: RiEditLine,
        permission: 'routine:i18n:update',
      ...
      },
      {
        ...
        permission: 'routine:i18n:delete',
...
const loadData = async () => {
  try {
    ...
    const result = await languageApi.getList(query)
...
const loadTranslationTransposed = async () => {
  try {
    ...
    const result = await getListTransposed(query)
...
const handleTranslationDeleteOne = async (record: Translation) => {
  ...
  try {
    translationLoading.value = true
    await removeTranslation(record.translationId)
...
const handleTranslationDelete = async () => {
  ...
  try {
    translationLoading.value = true
    for (const id of ids) await removeTranslation(id)
    message.success('删除成功')
    if (translationViewMode.value === 'list') loadTranslationList()
    else loadTranslationTransposed()
...
const handleTranslationExport = async () => {
  try {
    ...
    const blob = await exportTranslation(query, undefined, '翻译')
...
const handleDelete = async () => {
  ...
  try {
    loading.value = true
    if (ids.length === 1) {
      await languageApi.remove(ids[0])
    } else {
      await languageApi.removeBatch(ids)
    }
    message.success('删除成功')
    await loadData()
...
const handleDeleteOne = async (record: Language) => {
  ...
  try {
    loading.value = true
    await languageApi.remove(record.languageId)
    message.success('删除成功')
    await loadData()
    // 如果删除的是当前选中的行，清除选中状态
...
const handleExport = async () => {
  try {
    ...
    const blob = await languageApi.exportLanguage(query, undefined, '语言')
    const ts = new Date(); const t = (n: number, w = 2) => String(n).padStart(w, '0')
...
const handleStatusChange = async (record: Language, checked: boolean) => {
  try {
    await languageApi.updateStatus({
      languageId: record.languageId,
...
const handleFormSubmit = async () => {
  ...
  try {
    ...
    if ('languageId' in formData && formData.languageId) {
      // 更新
      await languageApi.update(formData.languageId, formData as LanguageUpdate)
      message.success('更新成功')
    } else {
      // 新增
      await languageApi.create(formData as LanguageCreate)
      message.success('新增成功')
    }
