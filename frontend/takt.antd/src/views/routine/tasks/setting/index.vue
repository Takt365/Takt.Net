...
<template>
  <div class="routine-setting">
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'isBuiltIn'">
          {{ record.isBuiltIn === 0 ? t('common.page.button.yes') : t('common.page.button.no') }}
        </template>
        <template v-else-if="column.key === 'isEncrypted'">
          {{ record.isEncrypted === 0 ? t('common.page.button.yes') : t('common.page.button.no') }}
        </template>
      </template>
    </TaktSingleTable>
    ...
    >
      <a-form-item :label="t('routine.setting.page.settingkey')">
        <a-input
          ...
          :placeholder="t('common.page.form.placeholder.required', { field: t('routine.setting.page.settingkey') })"
          allow-clear
        />
      </a-form-item>
      <a-form-item :label="t('routine.setting.page.settinggroup')">
        <TaktSelect
          ...
          :placeholder="t('common.page.form.placeholder.select', { field: t('routine.setting.page.settinggroup') })"
          allow-clear
      ...
      <a-form-item :label="t('routine.setting.page.settingstatus')">
        <TaktSelect
          ...
          :placeholder="t('common.page.form.placeholder.select', { field: t('routine.setting.page.settingstatus') })"
          allow-clear
          :field-names="{ label: 'dictLabel', value: 'extLabel' }"
        />
      </a-form-item>
    ...
    <TaktModal
      ...
      :title="t('common.page.button.import') + t('routine.setting.page._self')"
      :width="600"
      :footer="null"
    ...
    >
      <TaktImportFile
        ...
        :template-text="t('common.page.action.import.templatetext', { entity: t('routine.setting.page._self') })"
...
const columns = computed<TableColumnsType>(() => [
  ...
  {
    title: t('routine.setting.columns.sortOrder'),
    dataIndex: 'sortOrder',
    key: 'sortOrder',
    width: 80
  },
...
async function loadData() {
  try {
    ...
    message.error(pickErrorMessage(error, t('common.page.msg.loadfail')))
    dataSource.value = []
    total.value = 0
  } finally {
    loading.value = false
...
function handleCreate() {
  formTitle.value = t('common.page.button.create') + t('routine.setting.page._self')
  formData.value = {}
  formVisible.value = true
}

function handleEdit(record: Settings) {
  formTitle.value = t('common.page.button.edit') + t('routine.setting.page._self')
...
function handleUpdate() {
  if (selectedRow.value) handleEdit(selectedRow.value)
    ...
    message.warning(
      t('common.page.action.warnselecttoaction', { action: t('common.page.button.edit'), entity: t('routine.setting.page._self') })
    )
  }
}
...
function handleDeleteOne(record: Settings) {
  const name = record.settingKey || t('common.page.action.thistarget', { target: t('routine.setting.page._self') })
  Modal.confirm({
    title: t('common.page.action.confirmdelete'),
    content: t('common.page.confirm.deleteentity', { entity: t('routine.setting.page._self'), name }),
...
function handleDelete() {
  if (selectedRows.value.length === 0) {
    message.warning(
      t('common.page.action.warnselecttoaction', { action: t('common.page.button.delete'), entity: t('routine.setting.page._self') })
    )
    return
  }
  Modal.confirm({
