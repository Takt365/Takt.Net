...
import { refreshToken as refreshTokenApi } from './identity/security-engine/auth'
...
function shouldOmitBearerToken(url?: string): boolean {
  if (!url) return false
  return (
    url.includes('/api/TaktHealths') ||
    url.includes('/TaktHealths') ||
    url.includes('/api/TaktLanguages/options') ||
    url.includes('/api/TaktTranslations/options') ||
    url.includes('/api/TaktCaptchas')
  )
}
...
const UNAUTHORIZED_USER_MESSAGE = () => t('common.page.api.loginexpired')
...
async function handleUnauthorized(errorMessage: string, config?: InternalAxiosRequestConfig): Promise<unknown> {
  ...
  if (isRedirectingToLogin) {
    return Promise.reject(new Error(t('common.page.api.redirectingtologin')))
  }
...
function handle401AndRetry(config?: InternalAxiosRequestConfig): Promise<unknown> {
  return handleUnauthorized(t('common.page.api.loginexpired'), config)
}
...
export function startTokenRefreshTimer() {
  ...
  if (expiresAt == null) {
    const defaultExpiresIn = 7200 // 2小时（与后端 AccessTokenLifetimeHours: 2 一致）
...
service.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    // 调试日志：字典数据和翻译 API 的请求状态
    if (import.meta.env.DEV && config.url && (config.url.includes('TaktDictDatas') || config.url.includes('TaktTranslations'))) {
      console.debug('[Request Interceptor]', config.method?.toUpperCase(), config.url, {
        hasToken: !!token,
        tokenLength: token?.length ?? 0,
        omitToken: shouldOmitBearerToken(config.url)
      })
    }
...
service.interceptors.response.use(
  (response: AxiosResponse) => {
    ...
    if (isApiResultFormat && typeof code === 'number') {
      const errorMessage = messageText || t('common.page.api.requestfail')
  ...
  async (error) => {
    ...
    if (error.isBusinessError && error.response) {
      ...
      const errorMessage = error.message || t('common.page.api.requestfail')
      ...
      if (shouldShowError) {
        if (businessCode === TaktResultCode.Unauthorized) {
          ...
          showErrorOnce(t('common.page.api.forbidden'))
        } else if (businessCode === TaktResultCode.NotFound) {
          showErrorOnce(t('common.page.api.notfound'))
        } else if (businessCode === TaktResultCode.ServerError) {
          showErrorOnce(t('common.page.api.servererror'))
        } else if (businessCode === TaktResultCode.SystemError) {
          const msg = errorMessage || t('common.page.api.systemerror')
    ...
    if (error.response) {
        ...
        switch (status) {
          ...
          case 403: {
            ...
            if (isCsrfError) {
              ...
              showErrorOnce(t('common.page.api.csrffail'))
            } else {
      ...
      if (shouldShowError) showErrorOnce(t('common.page.api.requestconfigerror'))
    }
