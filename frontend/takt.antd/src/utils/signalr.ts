...
import { eventBus, NotificationEvents, SignalREvents } from '@/utils/eventBus'
...
export class TaktSignalRManager {
  ...
  private registerConnectHubEvents(
    onUserConnected?: (event: UserConnectedEventDto) => void,
    onUserDisconnected?: (event: UserDisconnectedEventDto) => void,
    onOnlineMessage?: (event: OnlineMessageEventDto) => void
  ): void {
    if (!this.connectHub) return

    // 上线消息事件 - 通过事件总线发布
    this.connectHub.on('OnlineMessage', (event: OnlineMessageEventDto) => {
      logger.info('[SignalR] 收到上线消息:', event)
      
      // 发布到事件总线
      eventBus.$emit(NotificationEvents.Received, {
        title: '用户上线',
        content: `用户 ${event.userName || event.userId} 已上线`,
        type: 'success',
        data: event,
        timestamp: Date.now()
      })
    ...
    // 用户连接事件 - 通过事件总线发布
    this.connectHub.on('UserConnected', (event: UserConnectedEventDto) => {
    ...
    // 用户断开连接事件 - 通过事件总线发布  
    this.connectHub.on('UserDisconnected', (event: UserDisconnectedEventDto) => {
      logger.debug('[SignalR] 用户已断开连接:', event)
      
      // 发布到事件总线
      eventBus.$emit(SignalREvents.Disconnected, {
        reason: event.reason || '用户断开连接'
      })
  ...
  private registerNotificationHubEvents(
    ...
    onReceiveBroadcast?: (message: BroadcastMessageDto) => void,
    onMessageSent?: (event: MessageSentEventDto) => void,
    onMessageRead?: (event: MessageReadEventDto) => void,
    onError?: (error: SignalRErrorEventDto) => void,
    onOnlineMessage?: (event: OnlineMessageEventDto) => void
  ): void {
    if (!this.notificationHub) return

    // 上线消息事件
    this.notificationHub.on('OnlineMessage', (event: OnlineMessageEventDto) => {
      logger.info('[SignalR] 收到上线消息:', event)
      onOnlineMessage?.(event)
    })

    // 接收消息 - 通过事件总线发布
    this.notificationHub.on('ReceiveMessage', (message: SignalRMessageDto) => {
      logger.debug('[SignalR] 收到消息:', message)
      
      // 发布到事件总线
      eventBus.$emit(NotificationEvents.Received, {
        title: message.messageTitle || '新消息',
    ...
    // 接收广播消息 - 通过事件总线发布
    this.notificationHub.on('ReceiveBroadcast', (message: BroadcastMessageDto) => {
      logger.debug('[SignalR] 收到广播消息:', message)
      
      // 发布到事件总线
      eventBus.$emit(NotificationEvents.Broadcast, {
        title: message.messageTitle || '系统广播',
        content: message.messageContent || '',
        type: 'warning',
        data: message,
        timestamp: Date.now()
    ...
    // 消息已发送
    this.notificationHub.on('MessageSent', (event: MessageSentEventDto) => {
      logger.debug('[SignalR] 消息已发送:', event)
      onMessageSent?.(event)
    })

    // 消息已读
    this.notificationHub.on('MessageRead', (event: MessageReadEventDto) => {
      logger.debug('[SignalR] 消息已读:', event)
      onMessageRead?.(event)
    })

    // 错误事件
    this.notificationHub.on('Error', (error: SignalRErrorEventDto) => {
  ...
  async getOnlineUsers(): Promise<OnlineUserDto[]> {
    ...
    try {
      const users = await this.connectHub.invoke<OnlineUserDto[]>('GetOnlineUsers')
      return users || []
    } catch (error) {
