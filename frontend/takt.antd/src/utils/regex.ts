...
export const RegexPatterns = {
  ...
  /** 昵称（中文、英文、数字、下划线、横线、点，1-50位） */
  NICK_NAME: /^[\u4e00-\u9fa5a-zA-Z0-9_.-]{1,50}$/,
  ...
  /** 菜单编码（字母开头，允许字母、数字、下划线、横线，3-50位） */
  MENU_CODE: /^[a-zA-Z][a-zA-Z0-9_-]{2,49}$/,
...
export class RegexHelper {
   ...
   * 验证密码强度
   * @param password 密码
   * @param strong 是否要求强密码（默认false）
   * @returns 是否有效
   */
  static isValidPassword(password: string, strong: boolean = false): boolean {
    return this.test(strong ? 'STRONG_PASSWORD' : 'PASSWORD', password)
  }
