...
命名空间:@/components/common/takt-captcha-behavior
文件名称:index.vue
创建时间:2025-01-20
创建人:Takt365(Cursor AI)
功能描述:行为验证码组件,支持滑块拖拽验证、目标位置指示、自动刷新等功能
...
<template>
  <div
    ref="containerRef"
    class="takt-captcha-behavior-wrapper"
  >
    <!-- 按登录页视口/表单宽度缩放，与同表单输入框一致 -->
    <div
      class="takt-captcha-behavior-scaled"
      :style="scaledStyle"
    >
      <div
        class="takt-captcha-behavior-inner"
...
import { generateCaptcha, verifyCaptcha, type CaptchaGenerateResult, type CaptchaVerifyRequest } from '@/api/identity/captcha-engine/captcha'
...
interface Props {
  ...
  initialData?: CaptchaGenerateResult | null | undefined
}
...
async function verify(position: number, timeSpent: number) {
  ...
  try {
    ...
    if (result.success) {
      ...
      logger.warn('[Captcha Behavior] 验证码验证失败', { captchaId: request.captchaId, position, targetPosition: targetPosition.value, message: result.data.message })
      errorMessage.value = result.data.message || t('components.common.page.captcha.failed')
...
async function generate() {
  ...
  try {
    ...
    if (result.data.enabled === false || result.data.enabled === undefined) {
      // 验证码未启用，直接通过
      logger.info('[Captcha Behavior] 验证码未启用（后端配置），直接通过验证')
      verified.value = true
      // 设置一个标记数据，让组件显示"已通过"状态
      captchaData.value = { ...result.data, enabled: false } as CaptchaGenerateResult
      emit('success')
      emit('verified', true)
      return
    }
    ...
    targetPosition.value = result.data.targetPosition || 0
