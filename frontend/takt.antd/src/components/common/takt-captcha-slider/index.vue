...
命名空间:@/components/common/takt-captcha-slider
文件名称:index.vue
创建时间:2025-01-20
创建人:Takt365(Cursor AI)
功能描述:滑块验证码组件,支持图片滑块、拖拽验证、轨迹记录等功能
...
const generate = async () => {
  try {
    ...
    logger.debug('[Captcha Slider] 验证码生成成功', { 
      ...
      hasBackgroundImage: !!result.data.backgroundImage, 
      hasSliderImage: !!result.data.sliderImage 
    })
...
const verify = async (position: number, timeSpent: number) => {
  ...
  try {
    ...
    if (result.data.success) {
      ...
      logger.warn('[Captcha Slider] 验证码验证失败', { captchaId: request.captchaId, position, message: result.data.message })
