...
/** connect/token 密码模式请求体中的用户名与密码（字段名与 OAuth 表单一致；与 TaktLoginDto 命名不同） */
export interface LoginParams {
  username: string
  password: string
}

/** `login()` 对 token 端点响应的归一化结果（与 OpenAPI LoginResponse / accessToken 等命名不同） */
...
import type { UserInfo } from '@/types/identity/specific-engine/auth'

/** userinfo / 假日接口在用户态裁剪后的「今日假日」展示结构（见 useUserStore.holidayFromToken；前端通用，非后端 DTO 名） */
...
/**
 * userinfo 扩展：在自动生成 `UserInfo` 基础上补 roles/permissions/employeeId，并允许可选假日展示字段（JSON 可能来自扩展约定）。
 */
export interface UserInfoWithHoliday extends UserInfo {
  roles: string[]
  permissions: string[]
  employeeId: string
