...
import { eventBus, AuthEvents, CrudEvents, DataEvents, SystemEvents } from '@/utils/eventBus'
import { eventBusMonitor } from '@/utils/eventBusMonitor'
import { startTokenRefreshTimer } from '@/api/request'
...
watchEffect(() => {
  ...
  const htmlTitle = String(translate('common.page.app.htmltitle'))
  if (htmlTitle && htmlTitle !== 'common.page.app.htmltitle') {
...
router.isReady().then(() => {
  ...
  /**
   * 监听登录成功事件 - 启动 Token 自动刷新定时器
   * 这是 OpenIddict Token 生命周期的关键：
   * - Access Token 有效期：2小时（后端配置）
   * - Refresh Token 有效期：7天（后端配置）
   * - 前端提前5分钟自动刷新 Access Token
   * - 401错误时被动触发刷新
   */
  eventBus.$on(AuthEvents.LoginSuccess, () => {
    logger.info('[EventBus] 登录成功，启动 Token 自动刷新定时器')
    startTokenRefreshTimer()
  })
  
  /**
   * 监听 Token 刷新事件 - 重新设置定时器
   * 每次刷新成功后，需要重新计算下一次刷新时间
   */
  eventBus.$on(AuthEvents.TokenRefreshed, ({ expiresIn }) => {
    logger.info('[EventBus] Token 已刷新，过期时间:', expiresIn, '秒')
    startTokenRefreshTimer()
  })
   ...
   * 登出后不应继续尝试刷新 Token
   */
  eventBus.$on(AuthEvents.DidLogout, () => {
    logger.info('[EventBus] 用户登出，Token 刷新定时器将自动失效')
  })
  
  /**
   * 监听数据刷新事件
   */
  eventBus.$on(DataEvents.Refresh, ({ module }) => {
    logger.debug('[EventBus] 数据刷新请求:', module)
  })
  ...
  if (import.meta.env.DEV) {
    ...
    logger.info('[EventBus Monitor] 性能监控已启用(开发环境)')
    
    eventBus.$on(CrudEvents.Created, ({ module, id }) => {
      logger.info(`[CRUD] 创建: ${module} - ${id}`)
    })
    
    eventBus.$on(CrudEvents.Updated, ({ module, id }) => {
      logger.info(`[CRUD] 更新: ${module} - ${id}`)
    })
    
    eventBus.$on(CrudEvents.Deleted, ({ module, id }) => {
      logger.info(`[CRUD] 删除: ${module} - ${id}`)
    })
    
    eventBus.$on(CrudEvents.CreateFailed, ({ module, error }) => {
      logger.error(`[CRUD] 创建失败: ${module}`, error)
    })
    
    eventBus.$on(CrudEvents.UpdateFailed, ({ module, error }) => {
      logger.error(`[CRUD] 更新失败: ${module}`, error)
    })
    
    eventBus.$on(CrudEvents.DeleteFailed, ({ module, error }) => {
      logger.error(`[CRUD] 删除失败: ${module}`, error)
    })
    
    // 定期输出性能报告(每 60 秒)
    setInterval(() => {
      const summary = eventBusMonitor.getSummary()
      if (summary.totalTriggers > 0) {
        logger.info('[EventBus Monitor] 性能统计:', summary)
      }
    }, 60000)
  }
   ...
   * 监听系统就绪事件
   */
  eventBus.$on(SystemEvents.Ready, () => {
    logger.info('[EventBus] 系统已就绪')
  })
}).catch((error) => {
