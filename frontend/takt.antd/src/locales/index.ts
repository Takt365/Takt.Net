 ...
 *    语言代码格式为 xx-YY（如 zh-CN、en-US、zh-HK）。新增后无需改 index.ts，列表会自动包含该语言。
...
function buildNestedMessages(path: string, messages: MessageRecord): MessageRecord {
  // 移除 ./ 前缀和文件扩展名（包括语言代码）
  // 使用动态正则表达式匹配所有语言代码格式（如：zh-CN, en-US, zh-HK, ja-JP, ko-KR）
...
export async function loadTranslationsFromBackend(cultureCode: string, resourceType: string = 'Frontend'): Promise<void> {
  try {
    ...
    if (Object.keys(translations).length > 0) {
      ...
      // 优化：只对包含点号的路径进行嵌套转换，减少不必要的处理
      const entriesWithDots = Object.entries(translations).filter(([key]) => key.includes('.'))
      const entriesWithoutDots = Object.entries(translations).filter(([key]) => !key.includes('.'))
      
      // 先处理不带点号的路径（直接赋值）
      entriesWithoutDots.forEach(([key, value]) => {
        backendMessages[key] = value
      })
      
      // 再处理带点号的路径（需要嵌套转换）
      // 按路径长度排序：先处理长路径（更具体的），再处理短路径（更通用的）
      const sortedEntries = entriesWithDots.sort(([keyA], [keyB]) => {
      ...
      logger.info(`[Perf] [I18n] 翻译数据合并完成，耗时: ${mergeDuration.toFixed(2)}ms`)
