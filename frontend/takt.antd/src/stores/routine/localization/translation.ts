...
import { getI18nOptions } from '@/api/routine/tasks/i18n/transposed-engine/i18n-engine'
...
export const useTranslationStore = defineStore('translation', () => {
  ...
  const loadTranslations = async (cultureCode: string, resourceType: string = 'Frontend', forceRefresh: boolean = false): Promise<Record<string, string>> => {
    ...
    try {
      ...
      // 使用 getI18nOptions 获取所有翻译选项（不分页）
      const options = await getI18nOptions()
      const apiDuration = performance.now() - apiStart
      logger.info(`[Perf] [Translation] API 请求完成 (${cultureCode})，耗时: ${apiDuration.toFixed(2)}ms`)
      ...
      logger.debug(`[Perf] [Translation] 数据处理完成，过滤后: ${Object.keys(allTranslations).length} 条，耗时: ${processDuration.toFixed(2)}ms`)
      ...
      if (import.meta.env.DEV) {
        logger.debug(`[Translation] 已加载翻译数据 (${cultureCode}, ${resourceType}):`, {
          ...
          sampleKeys: Object.keys(allTranslations).slice(0, 10)
        })
      }
