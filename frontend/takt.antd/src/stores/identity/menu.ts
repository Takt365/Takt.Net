...
export const useMenuStore = defineStore('menu', () => {
  ...
  const generateRoutes = async () => {
    try {
      // logger.debug('[Menu Store] 开始生成路由，调用 getMenuTree()...')
      // 从后端获取菜单树（响应拦截器已返回 res.data，所以直接使用）
      const menus = (await getMenuTree()) as unknown as MenuTree[]
  ...
  const extractMenuFields = (menu: any) => ({
    ...
    menuStatus: menu.menuStatus ?? 0,
    isVisible: menu.isVisible ?? 0,
  ...
  const generateRoutesFromMenuTree = (menus: MenuTree[], parentPath?: string): RouteRecordRaw[] => {
    ...
    sortedMenus.forEach((menu: any, index) => {
      ...
      // 只处理启用的菜单（MenuStatus = 0 表示启用）
      if (menuStatus !== undefined && menuStatus !== 0) {
  ...
  const preloadIcon = (iconName: string) => {
    ...
    import('@ant-design/icons-vue').then((module: any) => {
