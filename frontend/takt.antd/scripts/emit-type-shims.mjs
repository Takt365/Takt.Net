 ...
 * 使用方法：
 *   # 空格格式（npm 推荐）
 *   npm run gen:contracts -- --module employee
 *   
 *   # 等号格式
 *   npm run gen:contracts -- --module=employee
 *   
 *   # 全量生成
 *   npm run gen:contracts -- --all
 *   
 *   # 开发时生成类型
 *   npm run dev:types -- --module employee
 *   
 *   # 直接启动（不生成类型）
 *   npm run dev
 * 
...
const DTO_SKIP_NAMESPACES = new Set([
  'Routine.Tasks.Cache',        // TaktCacheDtos - 缓存管理特殊处理
  'Statistics.Logging'          // TaktServerMonitorDtos - 服务器监控特殊处理
])

/**
 * 检查命名空间是否在 Engine 目录中
 * Engine 目录包含特殊的代码生成引擎、流程引擎等，不应生成前端类型
 */
function isInEngineNamespace(namespace) {
  // 匹配所有包含 Engine 的命名空间（不区分大小写）
  return /\b\w*engine\b/i.test(namespace)
}
...
function parseDtoExportMeta(schemaKey) {
  ...
  // 跳过特殊处理的命名空间（如缓存管理、服务器监控等）
  if (DTO_SKIP_NAMESPACES.has(dtoNamespace)) return undefined
  
  // 跳过 Engine 目录下的所有 DTO（代码生成引擎、流程引擎等）
  if (isInEngineNamespace(dtoNamespace)) return undefined
...
function mapCSharpTypeToTs(typeName, forceString = false, currentDtoName = '') {
  ...
  if (listMatch) {
    ...
    // 如果是 Tree DTO 的 Children 属性，递归引用自身类型
    if (currentDtoName && innerType === currentDtoName) {
      const exportName = dtoClassNameToTsExportName(currentDtoName)
      return exportName ? `${exportName}[]` : 'unknown[]'
    }
    const itemTs = mapCSharpTypeToTs(innerType, forceString, currentDtoName)
    return `${itemTs}[]`
  }
  if (t.endsWith('[]')) {
    const itemTs = mapCSharpTypeToTs(t.slice(0, -2), forceString, currentDtoName)
    return `${itemTs}[]`
  }
  ...
  for (const file of dtoFiles) {
    ...
    while ((classMatch = classRe.exec(content)) != null) {
      ...
      for (let idx = 0; idx < lines.length; idx += 1) {
        ...
        // 传入当前 DTO 类名，支持树形结构递归引用
        const rawType = mapCSharpTypeToTs(csharpType, hasStringConverter, className)
        props.push({
...
function compareDtoExportsForEmit(a, b, moduleEntityName) {
  const order = name => {
    ...
    if (name === `${moduleEntityName}Tree`) return 5
...
function main() {
  ...
  // 支持两种格式：--module=employee 或 --module employee
  let targetModule = args.find(a => a.startsWith('--module='))?.split('=')[1]
  if (!targetModule) {
    const moduleIndex = args.indexOf('--module')
    if (moduleIndex !== -1 && args.length > moduleIndex + 1) {
  ...
  // 安全检查：必须明确指定模块或使用 --all
  if (!targetModule && !generateAll) {
    console.error('❌ 错误：必须指定模块名称或使用 --all 参数')
    console.error('')
    console.error('使用方法：')
    console.error('  1. 生成单个模块：node scripts/emit-type-shims.mjs --module employee')
    console.error('  2. 全量生成（谨慎）：node scripts/emit-type-shims.mjs --all')
    console.error('  3. npm脚本：npm run gen:contracts -- --module employee')
    console.error('')
    process.exit(1)
  }
  
  if (targetModule) {
    console.log(`🎯 目标模块: ${targetModule}`)
  } else {
    console.log('🔍 全量生成所有模块...')
  }
  console.log('')
  ...
  for (const [rel, exports] of Object.entries(MODULE_EXPORTS)) {
    // 如果指定了模块，只处理该模块
    if (targetModule && !rel.includes(targetModule)) {
      continue
    }
  ...
  console.log(`\n[emit-type-shims] 完成！已生成 ${generatedCount} 个模块`)
}
