 ...
 * 
 * 使用方法：
 *   1. 生成单个实体（默认）：node scripts/generate_validators.cjs --entity TaktEmployee
 *   2. 全量生成（谨慎）：node scripts/generate_validators.cjs --all
 *   3. 预览模式：node scripts/generate_validators.cjs --entity TaktEmployee --dry-run
 */
...
function main() {
  ...
  if (!targetEntity && !generateAll) {
    ...
    console.error('');
    console.error('使用方法：');
    console.error('  1. 生成单个实体：node scripts/generate_validators.cjs --entity TaktEmployee');
    console.error('  2. 全量生成（谨慎）：node scripts/generate_validators.cjs --all');
    console.error('  3. 预览模式：node scripts/generate_validators.cjs --entity TaktEmployee --dry-run');
    console.error('');
    process.exit(1);
  }
  ...
  if (targetEntity) {
    ...
    // 查找实体文件
    function findEntity(dir, entityName) {
      const files = fs.readdirSync(dir);
      for (const file of files) {
        const fullPath = path.join(dir, file);
        const stat = fs.statSync(fullPath);
        if (stat.isDirectory()) {
          const found = findEntity(fullPath, entityName);
    ...
    const entityFile = findEntity(ENTITIES_DIR, targetEntity);
    if (!entityFile) {
      console.error(`❌ 实体文件未找到: ${targetEntity}`);
      process.exit(1);
    }
    ...
    console.log(`找到 ${entityFiles.length} 个实体\n`);
  }
  ...
  for (const entityFile of entityFiles) {
    ...
    if (dryRun) {
      console.log(`   📄 [预览模式] 将生成: ${path.relative(VALIDATORS_DIR, validatorPath)}\n`);
    } else {
