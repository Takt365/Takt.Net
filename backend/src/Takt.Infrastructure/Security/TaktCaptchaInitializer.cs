...
public class TaktCaptchaInitializer : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        TaktLogger.Information("[Captcha Initializer] 开始初始化验证码服务（类型: {Type}）...", captchaType);
        
        try
        {
            // 只有 Slider 类型才需要初始化模板和背景图片资源
            if (captchaType.Equals("Slider", StringComparison.OrdinalIgnoreCase))
            {
                TaktLogger.Information("[Captcha Initializer] 开始初始化 Slider 验证码资源...");
                await InitializeResourcesAsync(cancellationToken);
                TaktLogger.Information("[Captcha Initializer] Slider 验证码资源初始化完成");
            }
            else
            {
                TaktLogger.Information("[Captcha Initializer] 验证码类型为 {Type}，跳过 Slider 资源初始化", captchaType);
            }
            ...
            var result = await captchaService.GenerateAsync();
            TaktLogger.Information("[Captcha Initializer] 首个验证码生成成功: CaptchaId={CaptchaId}, Type={Type}", 
                result.CaptchaId, result.Type);
        ...
        catch (Exception ex)
        {
            ...
            // 不抛出异常，避免阻止应用启动
            TaktLogger.Warning("[Captcha Initializer] 验证码初始化失败，但应用将继续启动。验证码功能可能不可用。");
        }
    }
    ...
    private async Task InitializeResourcesAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            // 确保 wwwroot 目录存在（使用统一的wwwroot路径解析）
            var wwwrootPath = TaktFileHelper.GetWwwRootPath(_webHostEnvironment.ContentRootPath);
