...
/// </summary>
public class TaktNumberingSeedData : ITaktSeedData
{
    /// <summary>
    ...
    public async Task<(int InsertCount, int UpdateCount)> SeedAsync(IServiceProvider serviceProvider, string configId)
    {
        ...
        var repository = serviceProvider.GetRequiredService<ITaktRepository<TaktNumbering>>();
        int insertCount = 0;
        int updateCount = 0;
        ...
        {
            var (i, u) = await CreateOrUpdateNumberingAsync(
                repository, def.RuleCode, def.RuleName, def.Prefix, def.NumberLength, def.SortOrder, def.CurrentNumber, now);
            insertCount += i;
            updateCount += u;
        }
    ...
    /// <summary>
    /// 创建或更新编码规则（统一在方法内设置 RuleStatus = 1）
    /// </summary>
    private static async Task<(int InsertCount, int UpdateCount)> CreateOrUpdateNumberingAsync(
        ...
        DateTime now)
    {
        var existing = await repository.GetAsync(r => r.RuleCode == ruleCode);
        if (existing == null)
        {
            var entity = new TaktNumbering
            {
                ConfigId = "2",
                ExtFieldJson = null,
                Remark = $"种子：{ruleName}，前缀{prefix}",
        ...
        else
        {
            ...
            if (existing.CurrentNumber < currentNumber)
                existing.CurrentNumber = currentNumber;
            existing.RuleStatus = 1; // 1=启用
