...
public class TaktMenuLevel3SeedData
{
    ...
    public static async Task<(int InsertCount, int UpdateCount)> SeedAsync(IServiceProvider serviceProvider, string configId)
    {
        ...
        {
            var (i1, u1) = await CreateOrUpdateMenuAsync(menuRepository, "ROUTINE_TASKS_Numbering", menu =>
            {
                menu.MenuName = "编码规则";
                menu.MenuCode = "ROUTINE_TASKS_Numbering";
                menu.MenuL10nKey = "menu.routine.tasks.Numbering";
                menu.MenuIcon = "RiHashtag";
                menu.ParentId = routineTasksMenu.Id;
                menu.MenuType = 1;
                menu.Permission = "routine:tasks:Numbering:list";
            ...
            var (i8, u8) = await CreateOrUpdateMenuAsync(menuRepository, "ROUTINE_TASKS_WORD_FILTER", menu =>
            {
                ...
                menu.MenuL10nKey = "menu.routine.tasks.Vocabulary";
                menu.MenuIcon = "RiShieldKeyholeLine";
                menu.ParentId = routineTasksMenu.Id;
                menu.MenuType = 1;
                menu.Permission = "routine:tasks:Vocabulary:list";
        ...
        if (statisticsLoggingMenu != null)
        {
            ...
            // 服务监控
            var (insertServerMonitor, updateServerMonitor) = await CreateOrUpdateMenuAsync(menuRepository, "LOGGING_SERVER_MONITOR", menu =>
            {
                menu.MenuName = "服务监控";
                menu.MenuCode = "LOGGING_SERVER_MONITOR";
                menu.MenuL10nKey = "menu.statistics.logging.servermonitor";
                menu.MenuIcon = "RiServerLine";
                menu.ParentId = statisticsLoggingMenu.Id;
                menu.MenuType = 1;
                menu.Permission = "statistics:logging:monitor:list";
                menu.Path = "/statistics/logging/server-monitor";
                menu.Component = "statistics/logging/server-monitor/index";
                menu.SortOrder = 5;
                menu.MenuStatus = 1;
                menu.IsVisible = 1;
                menu.IsCache = 0;
