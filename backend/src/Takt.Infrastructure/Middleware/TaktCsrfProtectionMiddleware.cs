...
public class TaktCsrfProtectionMiddleware
{
    ...
    private static bool ShouldSkipCsrfProtection(HttpContext context)
    {
        ...
        // 认证端点（登录、刷新令牌）跳过CSRF保护
        // 这些端点是公开的，用户还未登录，没有会话可被利用
        if (path.Contains("/TaktAuths/connect/token", StringComparison.OrdinalIgnoreCase) ||
            path.Contains("/connect/token", StringComparison.OrdinalIgnoreCase))
        {
            return true;
        }
