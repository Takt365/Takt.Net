...
public static class TaktServiceCollectionExtensions
{
    ...
    private static void RegisterApplicationServices(ContainerBuilder builder)
    {
        ...
        builder.RegisterType<TaktGenEngine>()
            .As<ITaktGenEngine>()
            .InstancePerLifetimeScope();

        // 注册编码规则生成引擎（类名不以 Service 结尾，需显式注册）
        builder.RegisterType<TaktRuleEngineService>()
            .As<ITaktRuleEngineService>()
            .InstancePerLifetimeScope();

        // 注册所有应用服务（自动扫描Takt.Application程序集中以Service结尾的类)
        // 排除验证码服务，因为它们已经手动注册
        builder.RegisterAssemblyTypes(typeof(ITaktUserService).Assembly)
    ...
    private static void RegisterCaptchaService(ContainerBuilder builder)
    {
        ...
        builder.Register(c =>
        {
            ...
            return new TaktCaptchaService(configuration, captchaOptions, webHostEnvironment);
