...
5. **`Serilog`** - 日志配置（遵循 [Serilog 官方配置](https://github.com/serilog/serilog-settings-configuration)）：
   - `Serilog.MinimumLevel.Default` - 全局最低级别，如 `Debug`、`Information`、`Warning`。
   - `Serilog.MinimumLevel.Override` - 按命名空间覆盖级别，键如 `Microsoft`、`System`、`Takt`，值为级别名。
   - **日志文件分类**：主输出与主文件使用 Default；`logs/takt-warning-*.log` 使用 `restrictedToMinimumLevel: "Warning"`；`logs/takt-error-*.log` 使用 `restrictedToMinimumLevel: "Error"`。
...
### SingleDeviceLogin（单点单设备登录）

同一账号允许的同时在线设备数，新登录会踢出最旧会话：
- `false` - **单设备**：同一账号同一时间只能在**一个设备**上保持登录，新登录会踢出旧会话（默认）
- `true` - **三设备**：同一账号同一时间只能在**三个设备**上保持登录，新登录会踢出最旧会话

**说明：** 登录与 SignalR 连接时均按此配置限制该账号的在线记录条数，超出时按 ConnectTime 将最旧的会话置为离线。
