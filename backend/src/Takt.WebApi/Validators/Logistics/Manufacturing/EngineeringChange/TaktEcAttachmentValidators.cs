...
public class TaktEcAttachmentCreateDtoValidator : AbstractValidator<TaktEcAttachmentCreateDto>
{
    public TaktEcAttachmentCreateDtoValidator(ITaktLocalizer? localizer = null)
    {
        RuleFor(x => x.AttachmentType)
            .NotEmpty().WithMessage(TaktValidationMessages.Required(localizer, "entity.ecattachment.attachmenttype"))
            .Length(1, 30).WithMessage(TaktValidationMessages.LengthBetween(localizer, "entity.ecattachment.attachmenttype", 1, 30));

        RuleFor(x => x.DocNo)
            .NotEmpty().WithMessage(TaktValidationMessages.Required(localizer, "entity.ecattachment.docno"))
            .Length(1, 50).WithMessage(TaktValidationMessages.LengthBetween(localizer, "entity.ecattachment.docno", 1, 50));

        RuleFor(x => x.FileName)
            .NotEmpty().WithMessage(TaktValidationMessages.Required(localizer, "entity.ecattachment.filename"))
            .Length(1, 200).WithMessage(TaktValidationMessages.LengthBetween(localizer, "entity.ecattachment.filename", 1, 200));
