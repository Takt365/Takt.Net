...
public class TaktProductionTeamCreateDtoValidator : AbstractValidator<TaktProductionTeamCreateDto>
{
    public TaktProductionTeamCreateDtoValidator(ITaktLocalizer? localizer = null)
    {
        RuleFor(x => x.PlantCode)
            ...
            .Must(TaktRegexHelper.IsValidPlantCode).WithMessage(TaktValidationMessages.FormatInvalid(localizer, "entity.productionteam.plantcode"));
        ...
        RuleFor(x => x.TeamCategory)
            .MaximumLength(10).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.productionteam.teamcategory", 10))
            .When(x => !string.IsNullOrWhiteSpace(x.TeamCategory));

        RuleFor(x => x.TeamCategoryName)
            .MaximumLength(50).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.productionteam.teamcategoryname", 50))
