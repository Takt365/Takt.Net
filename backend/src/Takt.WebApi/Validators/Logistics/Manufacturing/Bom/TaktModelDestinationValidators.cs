...
public class TaktModelDestinationCreateDtoValidator : AbstractValidator<TaktModelDestinationCreateDto>
{
    public TaktModelDestinationCreateDtoValidator(ITaktLocalizer? localizer = null)
    {
        RuleFor(x => x.PlantCode)
            .NotEmpty().WithMessage(TaktValidationMessages.Required(localizer, "entity.modeldestination.plantcode"))
            .Must(TaktRegexHelper.IsValidPlantCode).WithMessage(TaktValidationMessages.FormatInvalid(localizer, "entity.modeldestination.plantcode"));
