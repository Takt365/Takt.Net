...
{
    public TaktQualityIssueMeetingCreateDtoValidator(ITaktLocalizer? localizer = null)
    {
        RuleFor(x => x.MeetingRecorder)
            .MaximumLength(30).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.qualityissuemeeting.meetingrecorder", 30))
...
{
    public TaktQualityIssueMeetingUpdateDtoValidator(ITaktLocalizer? localizer = null)
    {
        Include(new TaktQualityIssueMeetingCreateDtoValidator(localizer));

        RuleFor(x => x.QualityIssueMeetingId)
            .GreaterThan(0)
            .WithMessage(TaktValidationMessages.Required(localizer, "entity.qualityissuemeeting.qualityissuemeetingid"));
