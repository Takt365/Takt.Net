...
public class TaktPurchasePriceChangeLogCreateDtoValidator : AbstractValidator<TaktPurchasePriceChangeLogCreateDto>
{
    public TaktPurchasePriceChangeLogCreateDtoValidator(ITaktLocalizer? localizer = null)
    {
        RuleFor(x => x.ChangeFields)
            .MaximumLength(4000).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.purchasepricechangelog.changefields", 4000))
            .When(x => !string.IsNullOrWhiteSpace(x.ChangeFields));

        RuleFor(x => x.ChangeBy)
            .MaximumLength(50).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.purchasepricechangelog.changeby", 50))
