...
public class TaktPurchaseRequestItemCreateDtoValidator : AbstractValidator<TaktPurchaseRequestItemCreateDto>
{
    public TaktPurchaseRequestItemCreateDtoValidator(ITaktLocalizer? localizer = null)
    {
        ...
        RuleFor(x => x.ReferenceSupplierCode)
            .MaximumLength(50).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.purchaserequestitem.referencesuppliercode", 50))
            .When(x => !string.IsNullOrWhiteSpace(x.ReferenceSupplierCode));

        RuleFor(x => x.ReferenceSupplierName)
            .MaximumLength(200).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.purchaserequestitem.referencesuppliername", 200))
