...
public class TaktSalesPriceChangeLogCreateDtoValidator : AbstractValidator<TaktSalesPriceChangeLogCreateDto>
{
    public TaktSalesPriceChangeLogCreateDtoValidator(ITaktLocalizer? localizer = null)
    {
        RuleFor(x => x.ChangeFields)
            .MaximumLength(4000).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.salespricechangelog.changefields", 4000))
            .When(x => !string.IsNullOrWhiteSpace(x.ChangeFields));

        RuleFor(x => x.ChangeBy)
            .MaximumLength(50).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.salespricechangelog.changeby", 50))
