...
{
    public TaktUserTenantCreateDtoValidator(ITaktLocalizer? localizer = null)
    {
    }
}
...
{
    public TaktUserTenantUpdateDtoValidator(ITaktLocalizer? localizer = null)
    {
        Include(new TaktUserTenantCreateDtoValidator(localizer));

        RuleFor(x => x.UserTenantId)
            .GreaterThan(0)
            .WithMessage(TaktValidationMessages.Required(localizer, "entity.usertenant.usertenantid"));
