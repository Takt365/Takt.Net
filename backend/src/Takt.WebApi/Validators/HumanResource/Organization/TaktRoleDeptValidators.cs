...
{
    public TaktRoleDeptCreateDtoValidator(ITaktLocalizer? localizer = null)
    {
    }
}
...
{
    public TaktRoleDeptUpdateDtoValidator(ITaktLocalizer? localizer = null)
    {
        Include(new TaktRoleDeptCreateDtoValidator(localizer));

        RuleFor(x => x.RoleDeptId)
            .GreaterThan(0)
            .WithMessage(TaktValidationMessages.Required(localizer, "entity.roledept.roledeptid"));
