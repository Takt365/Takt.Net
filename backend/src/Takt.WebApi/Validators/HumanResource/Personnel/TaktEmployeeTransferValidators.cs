...
public class TaktEmployeeTransferCreateDtoValidator : AbstractValidator<TaktEmployeeTransferCreateDto>
{
    public TaktEmployeeTransferCreateDtoValidator(ITaktLocalizer? localizer = null)
    {
        RuleFor(x => x.ApplicantBy)
            .MaximumLength(50).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.employeetransfer.applicantby", 50))
        ...
        RuleFor(x => x.ApproverBy)
            .MaximumLength(50).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.employeetransfer.approverby", 50))
            .When(x => !string.IsNullOrWhiteSpace(x.ApproverBy));

        RuleFor(x => x.ApproveComment)
            .MaximumLength(1000).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.employeetransfer.approvecomment", 1000))
            .When(x => !string.IsNullOrWhiteSpace(x.ApproveComment));

        RuleFor(x => x.HandlingBy)
            .MaximumLength(50).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.employeetransfer.handlingby", 50))
            .When(x => !string.IsNullOrWhiteSpace(x.HandlingBy));

        RuleFor(x => x.HandlingComment)
            .MaximumLength(500).WithMessage(TaktValidationMessages.LengthMax(localizer, "entity.employeetransfer.handlingcomment", 500))
