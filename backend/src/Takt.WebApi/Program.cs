...
try
{
    ...
    {
        var wwwrootPath = TaktFileHelper.GetWwwRootPath(builder.Environment.ContentRootPath);
    ...
    // 注册全局统一响应格式过滤器（自动包装 TaktApiResult）
    builder.Services.AddScoped<TaktApiResultResponseFilter>();

    // 注册敏感词过滤引擎（单例，全局共享）
    builder.Services.AddSingleton<ITaktFilteringEngineService, TaktFilteringEngineService>();

    // 添加服务
    builder.Services.AddControllers(options =>
        {
            options.Filters.AddService<TaktFluentValidationActionFilter>();
            options.Filters.AddService<TaktApiResultResponseFilter>();
        })
    ...
    {
        var wwwrootPath = TaktFileHelper.GetWwwRootPath(builder.Environment.ContentRootPath);
        var ipv4DbPath = Path.Combine(wwwrootPath, "Region", "ip2region_v4.xdb");
        var ipv6DbPath = Path.Combine(wwwrootPath, "Region", "ip2region_v6.xdb");
    ...
    Log.Information("即将调用 app.Run()，应用将开始监听请求...");
