...
try
{
    ...
    try
    {
        ...
        if (!Directory.Exists(wordsDirectory))
        {
        ...
        else
        {
            ...
            var wordFiles = Directory.GetFiles(wordsDirectory, "*.txt", SearchOption.TopDirectoryOnly)
                .ToList();
            ...
            else
            {
                ...
                foreach (var wordFile in wordFiles)
                {
                    try
                    {
                        var words = File.ReadAllLines(wordFile, System.Text.Encoding.UTF8)
