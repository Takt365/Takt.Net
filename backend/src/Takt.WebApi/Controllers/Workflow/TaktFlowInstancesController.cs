...
public class TaktFlowInstancesController : TaktControllerBase
{
    ...
    {
        var dto = await _instanceService.GetByIdAsync(id);
    ...
    {
        var list = await _instanceService.GetInstanceOperationHistoriesAsync(id);
        return Ok(list);
    }
    ...
    {
        var result = await _instanceService.GetTodoListAsync(query);
        return Ok(result);
    }
    ...
    {
        var result = await _instanceService.GetMyListAsync(query);
        return Ok(result);
    }
    ...
    {
        var result = await _instanceService.GetProcessedListAsync(query);
        return Ok(result);
    }
    ...
    {
        var (resultFileName, content) = await _instanceService.ExportAsync(query, sheetName, fileName);
    ...
    {
        var (resultFileName, content) = await _instanceService.ExportTodoAsync(query, sheetName, fileName);
        return File(content, TaktExcelExportFileHelper.GetExportContentType(resultFileName), resultFileName);
    }
    ...
    {
        var (resultFileName, content) = await _instanceService.ExportMyAsync(query, sheetName, fileName);
        return File(content, TaktExcelExportFileHelper.GetExportContentType(resultFileName), resultFileName);
    }
    ...
    {
        var (resultFileName, content) = await _instanceService.ExportProcessedAsync(query, sheetName, fileName);
        return File(content, TaktExcelExportFileHelper.GetExportContentType(resultFileName), resultFileName);
    }
    ...
    {
        var result = await _instanceService.StartAsync(dto);
        return Ok(result);
    }
    ...
    /// </summary>
    /// <param name="dto">同启动DTO（ProcessKey、流程标题、表单数据等）</param>
    ...
    {
        var result = await _instanceService.CreateDraftAsync(dto);
        return Ok(result);
    }
    ...
    {
        var result = await _instanceService.StartFromDraftAsync(id);
        return Ok(result);
    }
    ...
    {
        await _instanceService.CompleteTaskAsync(dto);
        return NoContent();
    }
    ...
    {
        var instanceCode = await _instanceService.ResolveInstanceCodeAsync(body.InstanceCode, body.FlowInstanceId);
        ...
        await _instanceService.RevokeAsync(instanceCode);
        return NoContent();
    }
    ...
    {
        await _instanceService.SuspendAsync(dto);
        return NoContent();
    }
    ...
    {
        await _instanceService.ResumeAsync(dto);
        return NoContent();
    }
    ...
    {
        await _instanceService.TerminateAsync(dto);
        return NoContent();
    }
    ...
    {
        await _instanceService.TransferAsync(dto);
        return NoContent();
    }
    ...
    {
        await _instanceService.AddApproversAsync(dto);
        return NoContent();
    }
    ...
    {
        await _instanceService.ReduceApprovalAsync(dto);
        return NoContent();
    }
    ...
    {
        await _instanceService.UpdateAsync(dto);
        return NoContent();
    }
    ...
    {
        await _instanceService.UndoVerificationAsync(dto);
        return NoContent();
    }
    ...
    {
        await _instanceService.DeleteAsync(id);
        return NoContent();
    }
    ...
    public async Task<IActionResult> DeleteBatch([FromBody] List<long> ids)
    {
        ...
        await _instanceService.DeleteAsync(ids);
        return NoContent();
    }
    ...
    /// </summary>
    /// <param name="processKey">流程 Key（如 Leave、Reimburse），必填</param>
    ...
    public async Task<ActionResult<TaktFlowVerifyCcflowReportDto>> VerifyCcflow([FromQuery] string processKey, [FromQuery] string? processTitle = null)
    {
        if (string.IsNullOrWhiteSpace(processKey))
            return BadRequest(GetLocalizedString("validation.processKeyQueryRequired", "Frontend"));

        var report = new TaktFlowVerifyCcflowReportDto { ProcessKey = processKey.Trim(), VerifyTime = DateTime.Now };
        ...
        try
        {
            var startRes = await _instanceService.StartAsync(new TaktFlowStartDto { ProcessKey = processKey.Trim(), ProcessTitle = $"{titlePrefix}-办结至结束" });
            r1.Steps.Add($"启动实例 {startRes.InstanceCode}");
            const int maxRounds = 25;
            for (int i = 0; i < maxRounds; i++)
            {
                var todoRes = await _instanceService.GetTodoListAsync(new TaktFlowTodoQueryDto { PageIndex = 1, PageSize = 50 });
                var todo = todoRes.Data?.FirstOrDefault(t => t.InstanceCode == startRes.InstanceCode);
                if (todo == null)
                {
                    var detail = await _instanceService.GetByIdAsync(startRes.InstanceId);
                ...
                await _instanceService.CompleteTaskAsync(new TaktFlowCompleteDto { FlowInstanceId = todo.InstanceId, Comment = "验证通过", Approved = true });
                r1.Steps.Add($"办结节点 {todo.NodeName}");
                var d = await _instanceService.GetByIdAsync(startRes.InstanceId);
        ...
        try
        {
            var startRes = await _instanceService.StartAsync(new TaktFlowStartDto { ProcessKey = processKey.Trim(), ProcessTitle = $"{titlePrefix}-撤回" });
            r2.Steps.Add($"启动实例 {startRes.InstanceCode}");
            await _instanceService.RevokeAsync(startRes.InstanceCode);
            r2.Steps.Add("执行撤回");
            var detail = await _instanceService.GetByIdAsync(startRes.InstanceId);
        ...
        try
        {
            var startRes = await _instanceService.StartAsync(new TaktFlowStartDto { ProcessKey = processKey.Trim(), ProcessTitle = $"{titlePrefix}-挂起恢复" });
            r3.Steps.Add($"启动实例 {startRes.InstanceCode}");
            await _instanceService.SuspendAsync(new TaktFlowSuspendDto { InstanceCode = startRes.InstanceCode, Reason = "验证挂起" });
            r3.Steps.Add("执行挂起");
            var d1 = await _instanceService.GetByIdAsync(startRes.InstanceId);
            if (d1?.IsSuspended != 1) { r3.Message = "挂起后 IsSuspended 应为 1"; }
            else
            {
            r3.Steps.Add("已挂起");
            await _instanceService.ResumeAsync(new TaktFlowResumeDto { InstanceCode = startRes.InstanceCode });
            r3.Steps.Add("执行恢复");
            var d2 = await _instanceService.GetByIdAsync(startRes.InstanceId);
    ...
    /// </summary>
    /// <param name="processKey">流程 Key（如 Leave、公文审批等），必填</param>
    /// <param name="processTitle">流程标题（可选），不传则用「工作流验证-{processKey}」</param>
    /// <returns>验证结果</returns>
    [HttpGet("verify")]
    [TaktPermission("workflow:instance:validate", "工作流验证")]
    public async Task<ActionResult<WorkflowVerifyResult>> Verify([FromQuery] string processKey, [FromQuery] string? processTitle = null)
    {
        if (string.IsNullOrWhiteSpace(processKey))
        {
        ...
        try
        {
            var title = string.IsNullOrWhiteSpace(processTitle) ? $"工作流验证-{processKey}" : processTitle;
            result.Steps.Add($"1. 启动流程 {processKey}");
            var startRes = await _instanceService.StartAsync(new TaktFlowStartDto
            {
            ...
            var todoRes = await _instanceService.GetTodoListAsync(new TaktFlowTodoQueryDto { PageIndex = 1, PageSize = 10 });
            var todo = todoRes.Data?.FirstOrDefault(t => t.InstanceCode == startRes.InstanceCode);
            if (todo == null)
            {
                result.Ok = false;
            ...
            await _instanceService.CompleteTaskAsync(new TaktFlowCompleteDto
            {
                FlowInstanceId = todo.InstanceId,
                Comment = "验证通过",
                Approved = true
            });
