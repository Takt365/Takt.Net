...
public class TaktPurchaseOrdersController : TaktControllerBase
{
    ...
    [HttpGet("template")]
    [TaktPermission("logistics:purchaseorder:template", "获取导入模板")]
    public async Task<IActionResult> GetTemplateAsync([FromQuery] string? sheetName = null, [FromQuery] string? fileName = null)
    {
        try
        {
            ...
            return File(content, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", actualFileName);
        }
        catch (Exception ex)
        {
            return HandleException(ex);
