...
public class TaktAuthsController : TaktControllerBase
{
    ...
    // 注意：/api/TaktAuths/login 和 /api/TaktAuths/refresh-token 端点已废弃
    ...
    [HttpPost("logout")]
    [AllowAnonymous]
    public async Task<ActionResult> LogoutAsync([FromBody] string refreshToken)
    {
        try
        {
            ...
            return Ok(GetLocalizedString("common.auth.logoutSuccess", "Frontend"));
        }
        catch (Exception ex)
        {
    ...
    public async Task<ActionResult<TaktUserInfoDto>> GetUserInfoAsync()
    {
        try
        {
