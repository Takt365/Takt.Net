...
public class TaktGenTablesController : TaktControllerBase
{
    ...
    public async Task<ActionResult<List<TaktDatabaseTableInfoDto>>> GetDatabaseTablesAsync([FromQuery] string configId)
    {
        if (string.IsNullOrWhiteSpace(configId))
            return BadRequest("configId 不能为空。");
    ...
    public async Task<ActionResult<TaktGenTableDto>> ImportTableAsync([FromBody] TaktImportTableRequestDto body)
    {
        if (body == null || string.IsNullOrWhiteSpace(body.ConfigId) || string.IsNullOrWhiteSpace(body.TableName))
            return BadRequest("configId 与 tableName 不能为空。");
    ...
    /// </summary>
    [HttpGet("template")]
    [TaktPermission("code:generator:template", "获取代码生成表配置导入模板")]
    public async Task<IActionResult> GetTemplateAsync([FromQuery] string? sheetName = null, [FromQuery] string? fileName = null)
    {
        try
        {
            var (resultFileName, content) = await _genTableService.GetTemplateAsync(sheetName, fileName);
    ...
    [TaktPermission("code:generator:import", "导入代码生成表配置")]
    public async Task<ActionResult<object>> ImportExcelAsync(IFormFile file, [FromForm] string? sheetName = null)
    {
        try
        {
            ...
            var (success, fail, errors) = await _genTableService.ImportAsync(stream, sheetName);
    ...
    /// </summary>
    [HttpPost("export")]
    [TaktPermission("code:generator:export", "导出代码生成表配置")]
    public async Task<IActionResult> ExportAsync([FromBody] TaktGenTableQueryDto query, [FromQuery] string? sheetName = null, [FromQuery] string? fileName = null)
    {
        try
        {
            var (resultFileName, content) = await _genTableService.ExportAsync(query, sheetName, fileName);
            return File(content, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", resultFileName);
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> InitializeAsync(long id)
    {
        ...
        if (string.IsNullOrWhiteSpace(configId))
            return BadRequest("表配置的 ConfigId 为空，无法确定目标数据库。");
        if (string.IsNullOrWhiteSpace(className))
            return BadRequest("表配置的实体类名为空，无法初始化数据表。");
    ...
    /// <summary>
    /// 预览生成：根据当前表配置与 wwwroot/Generator 下模板实时渲染，返回将生成的文件相对路径、已存在的文件列表及预览内容（name+content）。
    /// 预览内容仅来自 <see cref="BuildPreviewFromTemplatesOnlyAsync"/>，绝不读取生成目标路径下的文件。
    /// </summary>
    [HttpGet("{id}/generate-preview")]
    [TaktPermission("code:generator:preview", "代码预览")]
    public async Task<IActionResult> GeneratePreviewAsync(long id, [FromQuery] string? genPath = null)
    {
        TaktLogger.Information("[CodePreview] 请求预览: Id={Id}, GenPath={GenPath}", id, genPath ?? "(null)");
        var tableEntity = await _genTableRepository.GetByIdAsync(id).ConfigureAwait(false);
        if (tableEntity == null)
        {
            TaktLogger.Warning("[CodePreview] 表配置不存在: Id={Id}", id);
            return NotFound();
        }
        ...
        if (table == null)
        {
            TaktLogger.Warning("[CodePreview] 表 DTO 不存在: Id={Id}", id);
            return NotFound();
        }
        ...
        {
            TaktLogger.Warning("[CodePreview] Generator 目录不存在: {Path}", generatorRoot);
            return Ok(new { files = Array.Empty<string>(), existingFiles = Array.Empty<string>(), previewFiles = Array.Empty<object>() });
        }

        var templates = await LoadTemplateContents(generatorRoot).ConfigureAwait(false);
        TaktLogger.Information("[CodePreview] 已加载模板数: {Count}", templates.Count);
        var previewItems = await BuildPreviewFromTemplatesOnlyAsync(id, table, templates).ConfigureAwait(false);
        if (previewItems.Count == 0)
        {
            TaktLogger.Warning("[CodePreview] 模板渲染后无预览项: Id={Id}", id);
            return Ok(new { files = Array.Empty<string>(), existingFiles = Array.Empty<string>(), previewFiles = Array.Empty<object>() });
        }

        foreach (var item in previewItems)
            TaktLogger.Information("[CodePreview] 预览项: Name={Name}, ContentLength={Len}", item.name, (item.content?.Length ?? 0));

        var files = previewItems.ConvertAll(x => x.name);
        ...
        var previewFiles = previewItems.ConvertAll(x => new { name = x.name, content = x.content });
        TaktLogger.Information("[CodePreview] 模板已渲染成代码，通过 previewFiles 返回前端: 文件数={Count}, 已存在数={Existing}", previewFiles.Count, existingFiles.Count);
        return Ok(new { files, existingFiles, previewFiles });
    }

    /// <summary>
    /// 仅用模板渲染结果构建预览列表，绝不读取生成目标路径下的文件内容。预览接口只使用此方法得到的内容。
    /// </summary>
    private async Task<List<(string name, string content)>> BuildPreviewFromTemplatesOnlyAsync(
        long tableId,
        ...
        IReadOnlyDictionary<string, string> templates)
    {
        ...
        {
            TaktLogger.Information("[CodePreview] BuildPreview: 模板为空");
            return new List<(string, string)>();
        }

        var results = await _workflow.GenerateCodeAsync(tableId, templates, GetCurrentUserName()).ConfigureAwait(false);
        if (results == null)
        {
            TaktLogger.Warning("[CodePreview] BuildPreview: 工作流返回 null");
            return new List<(string, string)>();
        }

        TaktLogger.Information("[CodePreview] BuildPreview: 工作流返回 {Count} 项（模板已渲染为代码）", results.Count);
        var list = new List<(string name, string content)>(results.Count);
        foreach (var item in results)
        {
            ...
            list.Add((relativePath, item.Content ?? string.Empty));
        }
        return list;
    }

    /// <summary>从 Generator 目录加载 .sbn 模板内容（仅读模板文件，不读生成目标）。</summary>
    private static async Task<Dictionary<string, string>> LoadTemplateContents(string generatorRoot)
    {
    ...
    public async Task<IActionResult> GenerateCodeAsync(long id, [FromBody] GenerateCodeRequest? request = null)
    {
        ...
        var generatorRoot = Path.Combine(_env.WebRootPath ?? string.Empty, "Generator");
        if (!Directory.Exists(generatorRoot))
            return BadRequest("未找到 Generator 模板目录。");
        ...
        if (key.StartsWith("Backend/", StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (key.StartsWith("Backend/Crud/Csharp/", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (file.Equals("Controller.cs", StringComparison.OrdinalIgnoreCase))
                    return Path.Combine(backendSrc, NamespaceToBackendPath(table.ControllerNamespace), TaktGenTableTemplateModel.ToPluralControllerName(entityName) + ".cs");
            }
            if (key.StartsWith("Backend/Crud/Sql/", StringComparison.OrdinalIgnoreCase))
            {
                var file = key.Substring("Backend/Crud/Sql/".Length);
                if (file.Equals("menu_and_translation.sql", StringComparison.OrdinalIgnoreCase))
                {
                    var entitySnake = ToEntityNameKebab(table.EntityClassName ?? "entity").Replace("-", "_");
                    file = $"{entitySnake}_menu_and_translation.sql";
                }
