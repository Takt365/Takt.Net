...
// 命名空间：Takt.WebApi.Controllers.Accounting
...
using Takt.Application.Services.Accounting;
...
namespace Takt.WebApi.Controllers.Accounting;
...
[TaktPermission("accounting:costcenter", "成本中心管理")]
public class TaktCostCentersController : TaktControllerBase
{
    ...
    [HttpGet("list")]
    [TaktPermission("accounting:costcenter:list", "查询成本中心列表")]
    ...
    [HttpGet("{id}")]
    [TaktPermission("accounting:costcenter:query", "查询成本中心详情")]
    ...
    [HttpGet("options")]
    [TaktPermission("accounting:costcenter:list", "查询成本中心选项")]
    public async Task<ActionResult<List<TaktSelectOption>>> GetCostCenterOptionsAsync()
    {
        var options = await _costCenterService.GetCostCenterOptionsAsync();
        return Ok(options);
    }
    ...
    /// <returns>成本中心DTO</returns>
    [HttpPost]
    [TaktPermission("accounting:costcenter:create", "创建成本中心")]
    public async Task<ActionResult<TaktCostCenterDto>> CreateCostCenterAsync([FromBody] TaktCostCenterCreateDto dto)
    {
        try
        {
            var costCenter = await _costCenterService.CreateCostCenterAsync(dto);
            return CreatedAtAction(nameof(GetCostCenterByIdAsync), new { id = costCenter.CostCenterId }, costCenter);
        }
        catch (Exception ex)
        {
    ...
    /// <returns>成本中心DTO</returns>
    [HttpPut("{id}")]
    [TaktPermission("accounting:costcenter:update", "更新成本中心")]
    public async Task<ActionResult<TaktCostCenterDto>> UpdateCostCenterAsync(long id, [FromBody] TaktCostCenterUpdateDto dto)
    {
        try
        {
            var costCenter = await _costCenterService.UpdateCostCenterAsync(id, dto);
            return Ok(costCenter);
        }
        catch (Exception ex)
        {
    ...
    [HttpDelete("{id}")]
    [TaktPermission("accounting:costcenter:delete", "删除成本中心")]
    ...
    /// <returns>成本中心DTO</returns>
    [HttpPut("status")]
    [TaktPermission("accounting:costcenter:status", "更新成本中心状态")]
    public async Task<ActionResult<TaktCostCenterDto>> UpdateCostCenterStatusAsync([FromBody] TaktCostCenterStatusDto dto)
    {
        try
        {
            var costCenter = await _costCenterService.UpdateCostCenterStatusAsync(dto);
            return Ok(costCenter);
        }
        catch (Exception ex)
        {
    ...
    [HttpGet("template")]
    [TaktPermission("accounting:costcenter:import", "获取导入模板")]
    ...
    [HttpPost("import")]
    [TaktPermission("accounting:costcenter:import", "导入成本中心")]
    ...
    [HttpPost("export")]
    [TaktPermission("accounting:costcenter:export", "导出成本中心")]
