// ========================================
// 项目名称：节拍数字工厂 ·Takt Digital Factory (TDF)
// 命名空间：Takt.Application.Services.Identity
// 文件名称：TaktTenantService.cs
// 创建时间：2026-05-11
// 创建人：Takt365(Cursor AI)
// 功能描述：租户信息表应用服务，提供Tenant管理的业务逻辑
//
// 版权信息：Copyright (c) 2026 Takt  All rights reserved.
// 免责声明：此软件使用 MIT License，作者不承担任何使用风险。
// ========================================

using Takt.Application.Dtos.Identity;
using Takt.Domain.Entities.Identity;

namespace Takt.Application.Services.Identity;

/// <summary>
/// 租户信息表应用服务
/// </summary>
public class TaktTenantService : TaktServiceBase, ITaktTenantService
{
    private readonly ITaktRepository<TaktTenant> _repository;
    private readonly ITaktUniqueValidator _uniqueValidator;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="repository">Tenant仓储</param>
    /// <param name="uniqueValidator">唯一性验证器</param>
    /// <param name="userContext">用户上下文（可选）</param>
    /// <param name="tenantContext">租户上下文（可选）</param>
    /// <param name="localizer">本地化器（可选）</param>
    public TaktTenantService(
        ITaktRepository<TaktTenant> repository,
        ITaktUniqueValidator uniqueValidator,
        ITaktUserContext? userContext = null,
        ITaktTenantContext? tenantContext = null,
        ITaktLocalizer? localizer = null)
        : base(userContext, tenantContext, localizer)
    {
        _repository = repository;
        _uniqueValidator = uniqueValidator;
    }


    /// <summary>
    /// 获取租户信息表(Tenant)列表（分页）
    /// </summary>
    /// <param name="queryDto">查询DTO</param>
    /// <returns>分页结果</returns>
    public async Task<TaktPagedResult<TaktTenantDto>> GetTenantListAsync(TaktTenantQueryDto queryDto)
    {
        var predicate = QueryExpression(queryDto);
        var (data, total) = await _repository.GetPagedAsync(queryDto.PageIndex, queryDto.PageSize, predicate);
        return TaktPagedResult<TaktTenantDto>.Create(
            data.Adapt<List<TaktTenantDto>>(),
            total,
            queryDto.PageIndex,
            queryDto.PageSize
        );
    }


    /// <summary>
    /// 根据ID获取租户信息表(Tenant)
    /// </summary>
    /// <param name="id">租户信息表(Tenant)ID</param>
    /// <returns>租户信息表(Tenant)DTO</returns>
    public async Task<TaktTenantDto?> GetTenantByIdAsync(long id)
    {
        var entity = await _repository.GetByIdAsync(id);
        return entity == null ? null : entity.Adapt<TaktTenantDto>();
    }


    /// <summary>
    /// 获取租户信息表(Tenant)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>租户信息表(Tenant)选项列表</returns>
    public async Task<List<TaktSelectOption>> GetTenantOptionsAsync()
    {
        var all = await _repository.FindAsync(x => x.IsDeleted == 0 && x.TenantStatus == 1);
        return all.Select(x => new TaktSelectOption
        {
            DictLabel = x.TenantName ?? string.Empty,
            DictValue = x.TenantCode

        }).ToList();
    }


    /// <summary>
    /// 创建租户信息表(Tenant)
    /// </summary>
    /// <param name="dto">创建租户信息表(Tenant)DTO</param>
    /// <returns>租户信息表(Tenant)DTO</returns>
    public async Task<TaktTenantDto> CreateTenantAsync(TaktTenantCreateDto dto)
    {
        var entity = dto.Adapt<TaktTenant>();
        // 验证租户编码的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.TenantCode, dto.TenantCode);
        if (!isUnique)
            throw new TaktBusinessException($"租户信息表租户编码 {dto.TenantCode} 已存在");

        entity = await _repository.CreateAsync(entity);
        return (await GetTenantByIdAsync(entity.Id)) ?? entity.Adapt<TaktTenantDto>();
    }


    /// <summary>
    /// 更新租户信息表(Tenant)
    /// </summary>
    /// <param name="id">租户信息表(Tenant)ID</param>
    /// <param name="dto">更新租户信息表(Tenant)DTO</param>
    /// <returns>租户信息表(Tenant)DTO</returns>
    public async Task<TaktTenantDto> UpdateTenantAsync(long id, TaktTenantUpdateDto dto)
    {
        var entity = await _repository.GetByIdAsync(id);
        if (entity == null)
            throw new TaktBusinessException("validation.tenantNotFound");
        // 验证租户编码的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.TenantCode, dto.TenantCode, id);
        if (!isUnique)
            throw new TaktBusinessException($"租户信息表租户编码 {dto.TenantCode} 已存在");

        dto.Adapt(entity, typeof(TaktTenantUpdateDto), typeof(TaktTenant));
        entity.UpdatedAt = DateTime.Now;
        await _repository.UpdateAsync(entity);

        return (await GetTenantByIdAsync(id)) ?? entity.Adapt<TaktTenantDto>();
    }


    /// <summary>
    /// 删除租户信息表(Tenant)
    /// </summary>
    /// <param name="id">租户信息表(Tenant)ID</param>
    /// <returns>任务</returns>
    public async Task DeleteTenantByIdAsync(long id)
    {
        var entity = await _repository.GetByIdAsync(id);
        if (entity == null)
            throw new TaktBusinessException("validation.tenantNotFound");
        
        // 软删除：设置 IsDeleted = 1
        entity.IsDeleted = 1;

        // 同步更新状态字段为禁用状态（1）
        entity.TenantStatus = 1;

        await _repository.UpdateAsync(entity);
    }


    /// <summary>
    /// 批量删除租户信息表(Tenant)
    /// </summary>
    /// <param name="ids">租户信息表(Tenant)ID列表</param>
    /// <returns>任务</returns>
    public async Task DeleteTenantBatchAsync(IEnumerable<long> ids)
    {
        var idList = ids.ToList();
        if (idList.Count == 0) return;
        // 获取所有要删除的实体
        var entities = new List<TaktTenant>();
        foreach (var id in idList)
        {
            var entity = await _repository.GetByIdAsync(id);
            if (entity != null) entities.Add(entity);
        }
        
        if (entities.Count == 0) return;
        
        // 批量更新：设置 IsDeleted = 1，并同步更新 TenantStatus = 1（禁用）
        foreach (var entity in entities)
        {
            entity.IsDeleted = 1;
            entity.TenantStatus = 1;
        }
        
        await _repository.UpdateRangeBulkAsync(entities);
    }


    /// <summary>
    /// 更新租户信息表(Tenant)状态
    /// </summary>
    /// <param name="dto">租户信息表(Tenant)状态DTO</param>
    /// <returns>租户信息表(Tenant)DTO</returns>
    public async Task<TaktTenantDto> UpdateTenantStatusAsync(TaktTenantStatusDto dto)
    {
        var entity = await _repository.GetByIdAsync(dto.TenantId);
        if (entity == null)
            throw new TaktBusinessException("validation.tenantNotFound");
        entity.TenantStatus = dto.TenantStatus;
        entity.UpdatedAt = DateTime.Now;
        await _repository.UpdateAsync(entity);
        return await GetTenantByIdAsync(entity.Id) ?? entity.Adapt<TaktTenantDto>();
    }


    /// <summary>
    /// 获取租户信息表(Tenant)导入模板
    /// </summary>
    /// <param name="sheetName">工作表名称（可选）</param>
    /// <param name="fileName">文件名（可选）</param>
    /// <returns>Excel模板文件信息（文件名和内容）</returns>
    public async Task<(string fileName, byte[] content)> GetTenantTemplateAsync(string? sheetName, string? fileName)
    {
        var (excelSheet, excelFile) = await ResolveExcelImportTemplateNamesAsync(sheetName, fileName, nameof(TaktTenant));
        return await TaktExcelHelper.GenerateTemplateAsync<TaktTenantTemplateDto>(
            sheetName: excelSheet,
            fileName: excelFile
        );
    }


    /// <summary>
    /// 导入租户信息表(Tenant)
    /// </summary>
    /// <param name="fileStream">Excel文件流</param>
    /// <param name="sheetName">工作表名称</param>
    /// <returns>导入结果（成功数量、失败数量、错误信息列表）</returns>
    public async Task<(int success, int fail, List<string> errors)> ImportTenantAsync(Stream fileStream, string? sheetName)
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktTenant));
        var importData = await TaktExcelHelper.ImportAsync<TaktTenantImportDto>(fileStream, excelSheet);
        
        var successCount = 0;
        var failCount = 0;
        var errors = new List<string>();
        var rowIndex = 0;

        foreach (var item in importData)
        {
            rowIndex++;
            try
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktTenant>();
                await _repository.CreateAsync(entity);
                successCount++;
            }
            catch (Exception ex)
            {
                errors.Add($"行{rowIndex}: {ex.Message}");
                failCount++;
            }
        }

        return (successCount, failCount, errors);
    }


    /// <summary>
    /// 导出租户信息表(Tenant)
    /// </summary>
    /// <param name="query">查询DTO（可为 null）</param>
    /// <param name="sheetName">工作表名称（可选）</param>
    /// <param name="fileName">文件名（可选）</param>
    /// <returns>Excel 文件信息（文件名与内容）</returns>
    public async Task<(string fileName, byte[] content)> ExportTenantAsync(TaktTenantQueryDto query, string? sheetName, string? fileName)
    {
        var predicate = QueryExpression(query ?? new TaktTenantQueryDto());
        List<TaktTenant> list;
        if (predicate != null)
            list = await _repository.FindAsync(predicate);
        else
            list = await _repository.GetAllAsync();

        var (excelSheet, excelFile) = await ResolveExcelExportNamesAsync(sheetName, fileName, nameof(TaktTenant));
        if (list == null || list.Count == 0)
        {
            return await TaktExcelHelper.ExportAsync(
                new List<TaktTenantExportDto>(),
                excelSheet,
                excelFile
            );
        }

        var exportData = list.Select(x => x.Adapt<TaktTenantExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
        );
    }



    /// <summary>
    /// 构建租户信息表查询表达式
    /// </summary>
    /// <param name="queryDto">租户信息表查询DTO</param>
    /// <returns>查询表达式</returns>
    private static Expression<Func<TaktTenant, bool>> QueryExpression(TaktTenantQueryDto? queryDto)
    {
        var exp = Expressionable.Create<TaktTenant>();

        if (!string.IsNullOrEmpty(queryDto?.KeyWords))
        {
            exp = exp.And(x =>
                x.TenantName!.Contains(queryDto.KeyWords) ||
                x.TenantCode!.Contains(queryDto.KeyWords) ||
                x.AllowedConfigIds!.Contains(queryDto.KeyWords) ||
                x.Remark!.Contains(queryDto.KeyWords) ||
                x.ExtFieldJson!.Contains(queryDto.KeyWords) ||
                x.CreatedBy!.Contains(queryDto.KeyWords)
            );
        }

        if (!string.IsNullOrEmpty(queryDto?.TenantName))
        {
            exp = exp.And(x => x.TenantName!.Contains(queryDto.TenantName));
        }

        if (!string.IsNullOrEmpty(queryDto?.TenantCode))
        {
            exp = exp.And(x => x.TenantCode!.Contains(queryDto.TenantCode));
        }

        if (!string.IsNullOrEmpty(queryDto?.AllowedConfigIds))
        {
            exp = exp.And(x => x.AllowedConfigIds!.Contains(queryDto.AllowedConfigIds));
        }

        if (queryDto?.SubscriptionStartTime.HasValue == true)
        {
            exp = exp.And(x => x.SubscriptionStartTime == queryDto.SubscriptionStartTime);
        }

        if (queryDto?.SubscriptionEndTime.HasValue == true)
        {
            exp = exp.And(x => x.SubscriptionEndTime == queryDto.SubscriptionEndTime);
        }

        if (queryDto?.TenantStatus.HasValue == true)
        {
            exp = exp.And(x => x.TenantStatus == queryDto.TenantStatus);
        }

        if (!string.IsNullOrEmpty(queryDto?.Remark))
        {
            exp = exp.And(x => x.Remark!.Contains(queryDto.Remark));
        }

        if (!string.IsNullOrEmpty(queryDto?.ExtFieldJson))
        {
            exp = exp.And(x => x.ExtFieldJson!.Contains(queryDto.ExtFieldJson));
        }

        if (queryDto?.CreatedById.HasValue == true)
        {
            exp = exp.And(x => x.CreatedById == queryDto.CreatedById);
        }

        if (!string.IsNullOrEmpty(queryDto?.CreatedBy))
        {
            exp = exp.And(x => x.CreatedBy!.Contains(queryDto.CreatedBy));
        }

        if (queryDto?.CreatedAt.HasValue == true)
        {
            exp = exp.And(x => x.CreatedAt == queryDto.CreatedAt);
        }

        // SubscriptionStartTime 日期范围查询
        if (queryDto?.SubscriptionStartTimeStart.HasValue == true)
        {
            exp = exp.And(x => x.SubscriptionStartTime >= queryDto.SubscriptionStartTimeStart);
        }
        if (queryDto?.SubscriptionStartTimeEnd.HasValue == true)
        {
            exp = exp.And(x => x.SubscriptionStartTime <= queryDto.SubscriptionStartTimeEnd);
        }

        // SubscriptionEndTime 日期范围查询
        if (queryDto?.SubscriptionEndTimeStart.HasValue == true)
        {
            exp = exp.And(x => x.SubscriptionEndTime >= queryDto.SubscriptionEndTimeStart);
        }
        if (queryDto?.SubscriptionEndTimeEnd.HasValue == true)
        {
            exp = exp.And(x => x.SubscriptionEndTime <= queryDto.SubscriptionEndTimeEnd);
        }

        // CreatedAt 日期范围查询
        if (queryDto?.CreatedAtStart.HasValue == true)
        {
            exp = exp.And(x => x.CreatedAt >= queryDto.CreatedAtStart);
        }
        if (queryDto?.CreatedAtEnd.HasValue == true)
        {
            exp = exp.And(x => x.CreatedAt <= queryDto.CreatedAtEnd);
        }

        return exp.ToExpression();
    }
}
