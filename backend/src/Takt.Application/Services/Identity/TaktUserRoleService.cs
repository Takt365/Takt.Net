...
public class TaktUserRoleService : TaktServiceBase, ITaktUserRoleService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.UserId, dto.UserId, null, $"用户角色关联表编码 {dto.UserId} 已存在");
    ...
    public async Task<TaktUserRoleDto> UpdateUserRoleAsync(long id, TaktUserRoleUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.UserId, dto.UserId, id, $"用户角色关联表编码 {dto.UserId} 已存在");
