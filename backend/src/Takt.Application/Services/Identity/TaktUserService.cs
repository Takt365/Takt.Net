...
public class TaktUserService : TaktServiceBase, ITaktUserService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.UserName, dto.UserName, null, $"用户信息表编码 {dto.UserName} 已存在");
    ...
    public async Task<TaktUserDto> UpdateUserAsync(long id, TaktUserUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.UserName, dto.UserName, id, $"用户信息表编码 {dto.UserName} 已存在");
