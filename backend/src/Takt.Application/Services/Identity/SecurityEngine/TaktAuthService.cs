...
public class TaktAuthService : TaktServiceBase, ITaktAuthService
{
    ...
    private readonly ITaktAttendanceLeaveService _attendanceLeaveService;
    ...
    /// <param name="attendanceLeaveService">考勤请假服务</param>
    ...
    public TaktAuthService(
        ...
        ITaktAttendanceLeaveService attendanceLeaveService,
        ...
        : base(userContext, tenantContext, localizer)
    {
        ...
        _attendanceLeaveService = attendanceLeaveService;
    }

    // 注意：LoginAsync 和 RefreshTokenAsync 方法已废弃
    // 前端已完全迁移到 OpenIddict 标准端点 POST /api/TaktAuths/connect/token
    // 登录和刷新令牌逻辑由 TaktAuthsController.ExchangeAsync() 处理
    // 如需查看旧实现，请参考 Git 历史提交记录
    ...
    private async Task<TaktUserInfoDto> BuildUserInfoAsync(TaktUser user)
    {
        ...
        // 获取假日信息（使用当前日期和默认区域CN）
        var holidayTheme = await _attendanceLeaveService.GetHolidayThemeAsync(null, "CN");
        ...
        var userInfo = new TaktUserInfoDto
        {
            ...
            // 假日信息
            HolidayToday = holidayTheme != null,
            HolidayName = holidayTheme?.HolidayName,
            HolidayGreeting = holidayTheme?.HolidayGreeting,
            HolidayQuote = holidayTheme?.HolidayQuote,
            HolidayTheme = holidayTheme?.HolidayTheme
        };
