...
// 创建时间：2026-05-10
// 创建人：Takt365
// 功能描述：租户信息表应用服务接口，定义Tenant管理的业务操作
// 
...
public interface ITaktTenantService
{
    ...
    /// 根据ID获取租户信息表
    /// </summary>
    /// <param name="id">租户信息表ID</param>
    /// <returns>租户信息表DTO</returns>
    Task<TaktTenantDto?> GetTenantByIdAsync(long id);

    /// <summary>
    /// 获取租户信息表选项列表（用于下拉框等）
    /// </summary>
    /// <returns>租户信息表选项列表</returns>
    Task<List<TaktSelectOption>> GetTenantOptionsAsync();

    /// <summary>
    /// 创建租户信息表
    /// </summary>
    /// <param name="dto">创建租户信息表DTO</param>
    /// <returns>租户信息表DTO</returns>
    ...
    /// </summary>
    /// <param name="id">租户信息表ID</param>
    /// <param name="dto">更新租户信息表DTO</param>
    /// <returns>租户信息表DTO</returns>
    ...
    /// <summary>
    /// 更新租户信息表(Tenant)Status
    /// </summary>
    /// <param name="dto">租户信息表(Tenant)StatusDTO</param>
    ...
    /// 导出租户信息表(Tenant)
    /// </summary>
    /// <param name="query">租户信息表(Tenant)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
