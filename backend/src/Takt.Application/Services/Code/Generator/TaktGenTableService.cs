...
public class TaktGenTableService : TaktServiceBase, ITaktGenTableService
{
    ...
    public async Task<TaktGenTableDto> CreateGenTableAsync(TaktGenTableCreateDto dto)
    {
        ...
        // 验证DataSource、TableName组合的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.DataSource == dto.DataSource && x.TableName == dto.TableName);
        if (!isUnique)
            throw new TaktBusinessException($"代码生成表配置表DataSource、TableName组合已存在");
    ...
    public async Task<TaktGenTableDto> UpdateGenTableAsync(long id, TaktGenTableUpdateDto dto)
    {
        ...
        // 验证DataSource、TableName组合的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.DataSource == dto.DataSource && x.TableName == dto.TableName, id);
        if (!isUnique)
            throw new TaktBusinessException($"代码生成表配置表DataSource、TableName组合已存在");
