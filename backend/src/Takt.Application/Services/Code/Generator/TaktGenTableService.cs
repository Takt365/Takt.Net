...
public class TaktGenTableService : TaktServiceBase, ITaktGenTableService
{
    ...
    public async Task<TaktGenTableDto> CreateAsync(TaktGenTableCreateDto dto)
    {
        ...
        return genTable.Adapt<TaktGenTableDto>();
    }
    ...
    public async Task<TaktGenTableDto> UpdateAsync(long id, TaktGenTableUpdateDto dto)
    {
        var genTable = await _genTableRepository.GetByIdAsync(id);
        if (genTable == null)
            throw new TaktBusinessException("代码生成表配置不存在");
        ...
        genTable.UpdateTime = DateTime.Now;
    ...
    public async Task<(string fileName, byte[] content)> GetTemplateAsync(string? sheetName, string? fileName)
    {
        return await TaktExcelHelper.GenerateTemplateAsync<TaktGenTableTemplateDto>(
            sheetName: string.IsNullOrWhiteSpace(sheetName) ? "代码生成表配置导入模板" : sheetName,
            fileName: string.IsNullOrWhiteSpace(fileName) ? "代码生成表配置导入模板" : fileName
        );
    }
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportAsync(Stream fileStream, string? sheetName)
    {
        ...
        try
        {
            var importData = await TaktExcelHelper.ImportAsync<TaktGenTableImportDto>(
                fileStream,
                string.IsNullOrWhiteSpace(sheetName) ? "代码生成表配置导入模板" : sheetName
            );
            ...
            foreach (var (item, index) in importData.Select((item, index) => (item, index + 3)))
            {
                try
                {
                    ...
                    {
                        errors.Add($"第{index}行：数据表名称不能为空");
                        fail++;
                        continue;
                    }

                    await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_genTableRepository, t => t.TableName, item.TableName, null, $"第{index}行：数据表名称 {item.TableName} 已存在");

                    var entity = item.Adapt<TaktGenTable>();
                    entity.EntityClassName = string.IsNullOrWhiteSpace(item.EntityClassName) ? item.TableName : item.EntityClassName;
                    entity.GenBusinessName = string.IsNullOrWhiteSpace(item.GenBusinessName) ? item.EntityClassName : item.GenBusinessName;
                ...
                {
                    LogWarning(ex, $"导入代码生成表配置失败（第{index}行）: {ex.Message}");
                    errors.Add($"第{index}行：{ex.Message}");
                    fail++;
                }
                catch (Exception ex)
                {
                    LogError(ex, $"导入代码生成表配置异常（第{index}行）: {ex.Message}");
        ...
        {
            LogError(ex, $"导入代码生成表配置过程发生错误: {ex.Message}");
    ...
    /// 导出代码生成表配置（Excel）
    /// </summary>
    public async Task<(string fileName, byte[] content)> ExportAsync(TaktGenTableQueryDto query, string? sheetName, string? fileName)
    {
        var predicate = QueryExpression(query);
        var data = await _genTableRepository.FindAsync(predicate);
        var list = data?.OrderBy(x => x.Id).ToList() ?? new List<TaktGenTable>();
        if (list.Count == 0)
        {
            return await TaktExcelHelper.ExportAsync(
                new List<TaktGenTableExportDto>(),
                string.IsNullOrWhiteSpace(sheetName) ? "代码生成表配置" : sheetName,
                string.IsNullOrWhiteSpace(fileName) ? "代码生成表配置导出" : fileName
            );
        }

        var dtos = list.Select(x => x.Adapt<TaktGenTableExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            dtos,
            string.IsNullOrWhiteSpace(sheetName) ? "代码生成表配置" : sheetName,
            string.IsNullOrWhiteSpace(fileName) ? "代码生成表配置导出" : fileName
