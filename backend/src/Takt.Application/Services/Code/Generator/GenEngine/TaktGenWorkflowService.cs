...
public class TaktGenWorkflowService : ITaktGenWorkflowService
{
        ...
        string? sqlCreateBy = null)
    {
        ...
        var columns = await _genTableColumnRepository.FindAsync(c => c.GenTableId == tableId).ConfigureAwait(false);
        if (columns == null || columns.Count == 0)
        {
    ...
    /// <summary>
    /// 按 menu_and_translation.sql 模板顺序构建翻译行（每行 Id 用 SnowFlakeSingle.Instance.NextId() 生成）。翻译键与种子一致：菜单标题用 menu.xxx（同 menu_l10n_key），字段用 xxx.entities.fieldname（全小写）。
    /// </summary>
    ...
    /// <summary>
    /// 判断数据库列名是否属于 TaktEntityBase 基类字段，若是则导入时排除。
    /// </summary>
