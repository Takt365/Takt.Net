...
public interface ITaktCodeEngine
{
    ...
    /// <param name="templateKey">模板标识（用于调试日志，如文件名）</param>
    /// <returns>渲染后的字符串</returns>
    /// <exception cref="TaktCodeEngineException">模板解析或渲染失败时抛出</exception>
    Task<string> RenderAsync(string templateContent, object model, string? templateKey = null);
    ...
    /// <returns>渲染后的字符串</returns>
    Task<string> RenderAsync(string templateContent, TaktGenTemplateContext context, string? templateKey = null);
}
