...
public class TaktNewsFavoriteService : TaktServiceBase, ITaktNewsFavoriteService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.NewsId, dto.NewsId, null, $"新闻收藏记录表编码 {dto.NewsId} 已存在");
    ...
    public async Task<TaktNewsFavoriteDto> UpdateNewsFavoriteAsync(long id, TaktNewsFavoriteUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.NewsId, dto.NewsId, id, $"新闻收藏记录表编码 {dto.NewsId} 已存在");
