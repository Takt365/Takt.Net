...
// 功能描述：新闻收藏记录表应用服务，提供NewsFavorite管理的业务逻辑
//
...
{
    private readonly ITaktRepository<TaktNewsFavorite> _repository;
    private readonly ITaktUniqueValidator _uniqueValidator;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="repository">NewsFavorite仓储</param>
    /// <param name="uniqueValidator">唯一性验证器</param>
    /// <param name="userContext">用户上下文（可选）</param>
    /// <param name="tenantContext">租户上下文（可选）</param>
    /// <param name="localizer">本地化器（可选）</param>
    public TaktNewsFavoriteService(
        ITaktRepository<TaktNewsFavorite> repository,
    ...
    /// <returns>新闻收藏记录表(NewsFavorite)DTO</returns>
    public async Task<TaktNewsFavoriteDto?> GetNewsFavoriteByIdAsync(long id)
    {
        var entity = await _repository.GetByIdAsync(id);
        return entity == null ? null : entity.Adapt<TaktNewsFavoriteDto>();
    }


    /// <summary>
    /// 获取新闻收藏记录表(NewsFavorite)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>新闻收藏记录表(NewsFavorite)选项列表</returns>
    ...
    /// </summary>
    /// <param name="dto">创建新闻收藏记录表(NewsFavorite)DTO</param>
    /// <returns>新闻收藏记录表(NewsFavorite)DTO</returns>
    public async Task<TaktNewsFavoriteDto> CreateNewsFavoriteAsync(TaktNewsFavoriteCreateDto dto)
    {
        ...
        // 验证NewsId、UserId组合的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.NewsId == dto.NewsId && x.UserId == dto.UserId);
        if (!isUnique)
            throw new TaktBusinessException($"新闻收藏记录表NewsId、UserId组合已存在");
    ...
    /// </summary>
    /// <param name="id">新闻收藏记录表(NewsFavorite)ID</param>
    /// <param name="dto">更新新闻收藏记录表(NewsFavorite)DTO</param>
    /// <returns>新闻收藏记录表(NewsFavorite)DTO</returns>
    public async Task<TaktNewsFavoriteDto> UpdateNewsFavoriteAsync(long id, TaktNewsFavoriteUpdateDto dto)
    {
        var entity = await _repository.GetByIdAsync(id);
        if (entity == null)
            throw new TaktBusinessException("validation.newsfavoriteNotFound");
        // 验证NewsId、UserId组合的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.NewsId == dto.NewsId && x.UserId == dto.UserId, id);
        if (!isUnique)
            throw new TaktBusinessException($"新闻收藏记录表NewsId、UserId组合已存在");

        dto.Adapt(entity, typeof(TaktNewsFavoriteUpdateDto), typeof(TaktNewsFavorite));
        entity.UpdatedAt = DateTime.Now;
        await _repository.UpdateAsync(entity);

        return (await GetNewsFavoriteByIdAsync(id)) ?? entity.Adapt<TaktNewsFavoriteDto>();
    }
    ...
    public async Task DeleteNewsFavoriteBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktNewsFavorite>();
    ...
    public async Task<(string fileName, byte[] content)> GetNewsFavoriteTemplateAsync(string? sheetName, string? fileName)
    {
        var (excelSheet, excelFile) = await ResolveExcelImportTemplateNamesAsync(sheetName, fileName, nameof(TaktNewsFavorite));
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportNewsFavoriteAsync(Stream fileStream, string? sheetName)
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktNewsFavorite));
        var importData = await TaktExcelHelper.ImportAsync<TaktNewsFavoriteImportDto>(fileStream, excelSheet);
        ...
        foreach (var item in importData)
        {
            ...
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktNewsFavorite>();
    ...
    public async Task<(string fileName, byte[] content)> ExportNewsFavoriteAsync(TaktNewsFavoriteQueryDto query, string? sheetName, string? fileName)
    {
        var predicate = QueryExpression(query ?? new TaktNewsFavoriteQueryDto());
        List<TaktNewsFavorite> list;
        if (predicate != null)
            list = await _repository.FindAsync(predicate);
        else
            list = await _repository.GetAllAsync();

        var (excelSheet, excelFile) = await ResolveExcelExportNamesAsync(sheetName, fileName, nameof(TaktNewsFavorite));
        ...
        var exportData = list.Select(x => x.Adapt<TaktNewsFavoriteExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建新闻收藏记录表查询表达式
    /// </summary>
    /// <param name="queryDto">新闻收藏记录表查询DTO</param>
    ...
    private static Expression<Func<TaktNewsFavorite, bool>> QueryExpression(TaktNewsFavoriteQueryDto? queryDto)
    {
        ...
        if (!string.IsNullOrEmpty(queryDto?.KeyWords))
        {
            exp = exp.And(x =>
                ...
                x.FavoriteTags!.Contains(queryDto.KeyWords) ||
        ...
        if (queryDto?.FavoriteTime.HasValue == true)
        {
            exp = exp.And(x => x.FavoriteTime == queryDto.FavoriteTime);
        }

        if (!string.IsNullOrEmpty(queryDto?.FavoriteTags))
        {
            exp = exp.And(x => x.FavoriteTags!.Contains(queryDto.FavoriteTags));
        }
