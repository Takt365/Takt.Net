...
// 功能描述：工单变更日志表应用服务，提供TicketChangeLog管理的业务逻辑
//
...
{
    private readonly ITaktRepository<TaktTicketChangeLog> _repository;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="repository">TicketChangeLog仓储</param>
    /// <param name="userContext">用户上下文（可选）</param>
    /// <param name="tenantContext">租户上下文（可选）</param>
    /// <param name="localizer">本地化器（可选）</param>
    public TaktTicketChangeLogService(
        ITaktRepository<TaktTicketChangeLog> repository,
    ...
    /// <returns>工单变更日志表(TicketChangeLog)DTO</returns>
    public async Task<TaktTicketChangeLogDto?> GetTicketChangeLogByIdAsync(long id)
    {
        var entity = await _repository.GetByIdAsync(id);
        return entity == null ? null : entity.Adapt<TaktTicketChangeLogDto>();
    }


    /// <summary>
    /// 获取工单变更日志表(TicketChangeLog)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>工单变更日志表(TicketChangeLog)选项列表</returns>
    ...
    /// </summary>
    /// <param name="dto">创建工单变更日志表(TicketChangeLog)DTO</param>
    /// <returns>工单变更日志表(TicketChangeLog)DTO</returns>
    ...
    /// </summary>
    /// <param name="id">工单变更日志表(TicketChangeLog)ID</param>
    /// <param name="dto">更新工单变更日志表(TicketChangeLog)DTO</param>
    /// <returns>工单变更日志表(TicketChangeLog)DTO</returns>
    public async Task<TaktTicketChangeLogDto> UpdateTicketChangeLogAsync(long id, TaktTicketChangeLogUpdateDto dto)
    {
        var entity = await _repository.GetByIdAsync(id);
        if (entity == null)
            throw new TaktBusinessException("validation.ticketchangelogNotFound");

        dto.Adapt(entity, typeof(TaktTicketChangeLogUpdateDto), typeof(TaktTicketChangeLog));
        entity.UpdatedAt = DateTime.Now;
        await _repository.UpdateAsync(entity);

        return (await GetTicketChangeLogByIdAsync(id)) ?? entity.Adapt<TaktTicketChangeLogDto>();
    }
    ...
    public async Task DeleteTicketChangeLogBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktTicketChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> GetTicketChangeLogTemplateAsync(string? sheetName, string? fileName)
    {
        var (excelSheet, excelFile) = await ResolveExcelImportTemplateNamesAsync(sheetName, fileName, nameof(TaktTicketChangeLog));
        return await TaktExcelHelper.GenerateTemplateAsync<TaktTicketChangeLogTemplateDto>(
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportTicketChangeLogAsync(Stream fileStream, string? sheetName)
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktTicketChangeLog));
        var importData = await TaktExcelHelper.ImportAsync<TaktTicketChangeLogImportDto>(fileStream, excelSheet);
        ...
        foreach (var item in importData)
        {
            ...
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktTicketChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> ExportTicketChangeLogAsync(TaktTicketChangeLogQueryDto query, string? sheetName, string? fileName)
    {
        var predicate = QueryExpression(query ?? new TaktTicketChangeLogQueryDto());
        List<TaktTicketChangeLog> list;
        if (predicate != null)
            list = await _repository.FindAsync(predicate);
        else
            list = await _repository.GetAllAsync();

        var (excelSheet, excelFile) = await ResolveExcelExportNamesAsync(sheetName, fileName, nameof(TaktTicketChangeLog));
        ...
        var exportData = list.Select(x => x.Adapt<TaktTicketChangeLogExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建工单变更日志表查询表达式
    /// </summary>
    /// <param name="queryDto">工单变更日志表查询DTO</param>
