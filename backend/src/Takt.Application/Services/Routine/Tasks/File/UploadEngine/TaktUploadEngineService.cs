...
// 文件名称：TaktUploadEngineService.cs
// 创建时间：2025-01-20
...
/// Takt文件上传服务
/// </summary>
public class TaktUploadEngineService : ITaktUploadEngineService
{
    ...
    public TaktUploadEngineService(IWebHostEnvironment webHostEnvironment, IConfiguration configuration)
    {
        _webHostEnvironment = webHostEnvironment;
        _configuration = configuration;
        var wwwrootPath = TaktFileHelper.GetWwwRootPath(_webHostEnvironment.ContentRootPath);
        _uploadBasePath = Path.Combine(wwwrootPath, "upload");
    ...
    public async Task<FileUploadResultDto> UploadAsync(Stream fileStream, string fileName, TaktFileConstants.FileUploadType fileType, string? targetFileName = null, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<List<FileUploadResultDto>> UploadBatchAsync(List<(Stream stream, string fileName)> files, TaktFileConstants.FileUploadType fileType, CancellationToken cancellationToken = default)
    {
    ...
    public async Task DeleteAsync(string filePath)
    {
        ...
        // 构建完整路径（使用统一的wwwroot路径解析）
        var wwwrootPath = TaktFileHelper.GetWwwRootPath(_webHostEnvironment.ContentRootPath);
        var fullPath = Path.Combine(wwwrootPath, filePath.Replace('/', Path.DirectorySeparatorChar));

        // 检查文件是否存在
        if (TaktFileHelper.FileExists(fullPath))
        {
            try
            {
    ...
    private static string GetUploadPath(TaktFileConstants.FileUploadType fileType)
    {
        return fileType switch
        {
            TaktFileConstants.FileUploadType.Avatar => "avatar",
            TaktFileConstants.FileUploadType.Image => "images",
            TaktFileConstants.FileUploadType.File => "files",
            _ => "files"
        };
    }
