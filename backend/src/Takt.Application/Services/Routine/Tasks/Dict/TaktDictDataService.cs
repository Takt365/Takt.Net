...
public class TaktDictDataService : TaktServiceBase, ITaktDictDataService
{
    ...
    public async Task<TaktDictDataDto> CreateDictDataAsync(TaktDictDataCreateDto dto)
    {
        ...
        // 验证DictTypeCode、DictLabel组合的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.DictTypeCode == dto.DictTypeCode && x.DictLabel == dto.DictLabel);
        if (!isUnique)
            throw new TaktBusinessException($"字典数据表DictTypeCode、DictLabel组合已存在");
    ...
    public async Task<TaktDictDataDto> UpdateDictDataAsync(long id, TaktDictDataUpdateDto dto)
    {
        ...
        // 验证DictTypeCode、DictLabel组合的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.DictTypeCode == dto.DictTypeCode && x.DictLabel == dto.DictLabel, id);
        if (!isUnique)
            throw new TaktBusinessException($"字典数据表DictTypeCode、DictLabel组合已存在");
