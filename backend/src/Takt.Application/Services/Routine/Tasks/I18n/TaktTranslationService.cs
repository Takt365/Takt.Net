...
public class TaktTranslationService : TaktServiceBase, ITaktTranslationService
{
    ...
    public async Task<TaktTranslationDto> CreateTranslationAsync(TaktTranslationCreateDto dto)
    {
        ...
        // 验证ResourceKey、CultureCode组合的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.ResourceKey == dto.ResourceKey && x.CultureCode == dto.CultureCode);
        if (!isUnique)
            throw new TaktBusinessException($"翻译表ResourceKey、CultureCode组合已存在");
    ...
    public async Task<TaktTranslationDto> UpdateTranslationAsync(long id, TaktTranslationUpdateDto dto)
    {
        ...
        // 验证ResourceKey、CultureCode组合的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.ResourceKey == dto.ResourceKey && x.CultureCode == dto.CultureCode, id);
        if (!isUnique)
            throw new TaktBusinessException($"翻译表ResourceKey、CultureCode组合已存在");
