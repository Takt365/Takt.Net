...
// 创建时间：2026-05-11
// 创建人：Takt365
// 功能描述：语言表应用服务接口（主子表），定义Language管理的业务操作
// 
...
public interface ITaktLanguageService
{
    ...
    /// 根据ID获取语言表（包含子表数据）
    /// </summary>
    /// <param name="id">语言表ID</param>
    /// <returns>语言表DTO</returns>
    Task<TaktLanguageDto?> GetLanguageByIdAsync(long id);

    /// <summary>
    /// 获取语言表选项列表（用于下拉框等）
    /// </summary>
    /// <returns>语言表选项列表</returns>
    Task<List<TaktSelectOption>> GetLanguageOptionsAsync();

    /// <summary>
    /// 创建语言表（包含子表数据）
    /// </summary>
    /// <param name="dto">创建语言表DTO</param>
    /// <returns>语言表DTO</returns>
    ...
    /// <summary>
    /// 更新语言表(Language)Status
    /// </summary>
    /// <param name="dto">语言表(Language)StatusDTO</param>
    ...
    /// <returns>语言表(Language)DTO</returns>
    Task<TaktLanguageDto> UpdateLanguageSortAsync(TaktLanguageSortDto dto);
    ...
    /// 导出语言表(Language)
    /// </summary>
    /// <param name="query">语言表(Language)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
