...
public class TaktVocabularyService : TaktServiceBase, ITaktVocabularyService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.WordText, dto.WordText, null, $"敏感词表编码 {dto.WordText} 已存在");
    ...
    public async Task<TaktVocabularyDto> UpdateVocabularyAsync(long id, TaktVocabularyUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.WordText, dto.WordText, id, $"敏感词表编码 {dto.WordText} 已存在");
