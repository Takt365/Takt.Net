...
public class TaktSettingsService : TaktServiceBase, ITaktSettingsService
{
    ...
    public async Task<TaktPagedResult<TaktSettingsDto>> GetSettingsListAsync(TaktSettingsQueryDto queryDto)
    {
    ...
    public async Task<TaktSettingsDto?> GetSettingsByIdAsync(long id)
    {
    ...
    public async Task<List<TaktSelectOption>> GetSettingsOptionsAsync()
    {
    ...
    /// <returns>设置DTO</returns>
    public async Task<TaktSettingsDto> CreateSettingsAsync(TaktSettingsCreateDto dto)
    {
        // 查重验证
    ...
    /// <returns>设置DTO</returns>
    public async Task<TaktSettingsDto> UpdateSettingsAsync(long id, TaktSettingsUpdateDto dto)
    {
        var settings = await _settingsRepository.GetByIdAsync(id);
        if (settings == null)
    ...
    /// <param name="id">设置ID</param>
    /// <returns>任务</returns>
    public async Task DeleteSettingsByIdAsync(long id)
    {
        var settings = await _settingsRepository.GetByIdAsync(id);
        if (settings == null)
            throw new TaktBusinessException("validation.settingNotFound");
    ...
    /// <returns>任务</returns>
    public async Task DeleteSettingsBatchAsync(IEnumerable<long> ids)
    {
    ...
    public async Task<(string fileName, byte[] content)> GetSettingsTemplateAsync(string? sheetName, string? fileName)
    {
        var (excelSheet, excelFile) = await ResolveExcelImportTemplateNamesAsync(sheetName, fileName, nameof(TaktSettings));
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportSettingsAsync(Stream fileStream, string? sheetName)
    {
        ...
        try
        {
            var excelSheet = ResolveExcelSheetName(sheetName, nameof(TaktSettings));
            // 从Excel导入数据
            ...
            foreach (var (item, index) in importData.Select((item, index) => (item, index + 3))) // 第3行开始是数据
            {
                try
                {
                    ...
                    {
                        AddImportError(errors, "validation.importRowSettingKeyRequired", index);
                        fail++;
                        continue;
                    }

                    // 导入时使用验证器手动验证（SettingKey、SettingValue、SettingName、SettingGroup 任意一个重复都报错）
                    await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_settingsRepository, s => s.SettingKey, item.SettingKey, null, GetLocalizedString("validation.importRowSettingKeyExists", "Frontend", index, item.SettingKey));
                    await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_settingsRepository, s => s.SettingValue, item.SettingValue, null, GetLocalizedString("validation.importRowSettingValueExists", "Frontend", index, item.SettingValue ?? string.Empty));
                    await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_settingsRepository, s => s.SettingName, item.SettingName, null, GetLocalizedString("validation.importRowSettingNameExists", "Frontend", index, item.SettingName ?? string.Empty));
                    await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_settingsRepository, s => s.SettingGroup, item.SettingGroup, null, GetLocalizedString("validation.importRowSettingGroupExists", "Frontend", index, item.SettingGroup ?? string.Empty));
    ...
    public async Task<(string fileName, byte[] content)> ExportSettingsAsync(TaktSettingsQueryDto query, string? sheetName, string? fileName)
    {
        // 构建查询条件
        ...
        var (excelSheet, excelFile) = await ResolveExcelExportNamesAsync(sheetName, fileName, nameof(TaktSettings));
