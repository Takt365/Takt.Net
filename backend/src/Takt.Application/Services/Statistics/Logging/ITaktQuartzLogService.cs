...
public interface ITaktQuartzLogService
{
    ...
    Task<TaktPagedResult<TaktQuartzLogDto>> GetListAsync(TaktQuartzLogQueryDto queryDto);
    ...
    Task<TaktQuartzLogDto?> GetByIdAsync(long id);
    ...
    /// <returns>任务日志DTO</returns>
    Task<TaktQuartzLogDto> CreateAsync(TaktCreateQuartzLogDto dto);
    ...
    Task<(string fileName, byte[] content)> ExportAsync(TaktQuartzLogQueryDto query, string? sheetName, string? fileName);
}
