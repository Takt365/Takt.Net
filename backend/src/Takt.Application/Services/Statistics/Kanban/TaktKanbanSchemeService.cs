...
public class TaktKanbanSchemeService : TaktServiceBase, ITaktKanbanSchemeService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.SchemeCode, dto.SchemeCode, null, $"看板方案表编码 {dto.SchemeCode} 已存在");
    ...
    public async Task<TaktKanbanSchemeDto> UpdateKanbanSchemeAsync(long id, TaktKanbanSchemeUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.SchemeCode, dto.SchemeCode, id, $"看板方案表编码 {dto.SchemeCode} 已存在");
