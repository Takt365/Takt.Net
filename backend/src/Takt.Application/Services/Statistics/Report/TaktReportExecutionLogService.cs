...
public class TaktReportExecutionLogService : TaktServiceBase, ITaktReportExecutionLogService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.PlantCode, dto.PlantCode, null, $"报表执行日志表编码 {dto.PlantCode} 已存在");
    ...
    public async Task<TaktReportExecutionLogDto> UpdateReportExecutionLogAsync(long id, TaktReportExecutionLogUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.PlantCode, dto.PlantCode, id, $"报表执行日志表编码 {dto.PlantCode} 已存在");
