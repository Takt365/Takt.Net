...
// 功能描述：员工家庭成员表应用服务，提供EmployeeFamily管理的业务逻辑
//
...
public class TaktEmployeeFamilyService : TaktServiceBase, ITaktEmployeeFamilyService
{
    ...
    /// 构造函数
    /// </summary>
    /// <param name="repository">EmployeeFamily仓储</param>
    ...
    /// <returns>员工家庭成员表(EmployeeFamily)DTO</returns>
    ...
    /// <summary>
    /// 获取员工家庭成员表(EmployeeFamily)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>员工家庭成员表(EmployeeFamily)选项列表</returns>
    public async Task<List<TaktSelectOption>> GetEmployeeFamilyOptionsAsync()
    {
        var all = await _repository.FindAsync(x => x.IsDeleted == 0);
        return all.Select(x => new TaktSelectOption
        {
            DictLabel = x.MemberName ?? string.Empty,
            DictValue = x.MemberName
    ...
    /// </summary>
    /// <param name="dto">创建员工家庭成员表(EmployeeFamily)DTO</param>
    /// <returns>员工家庭成员表(EmployeeFamily)DTO</returns>
    ...
    /// </summary>
    /// <param name="id">员工家庭成员表(EmployeeFamily)ID</param>
    /// <param name="dto">更新员工家庭成员表(EmployeeFamily)DTO</param>
    /// <returns>员工家庭成员表(EmployeeFamily)DTO</returns>
    ...
    public async Task DeleteEmployeeFamilyBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktEmployeeFamily>();
    ...
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktEmployeeFamily));
        ...
        foreach (var item in importData)
        {
            ...
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktEmployeeFamily>();
    ...
    public async Task<(string fileName, byte[] content)> ExportEmployeeFamilyAsync(TaktEmployeeFamilyQueryDto query, string? sheetName, string? fileName)
    {
        ...
        var exportData = list.Select(x => x.Adapt<TaktEmployeeFamilyExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建员工家庭成员表查询表达式
    /// </summary>
    /// <param name="queryDto">员工家庭成员表查询DTO</param>
    ...
    private static Expression<Func<TaktEmployeeFamily, bool>> QueryExpression(TaktEmployeeFamilyQueryDto? queryDto)
    {
        ...
        {
            exp = exp.And(x =>
                x.MemberName!.Contains(queryDto.KeyWords) ||
                x.PhoneNumber!.Contains(queryDto.KeyWords) ||
                x.WorkUnit!.Contains(queryDto.KeyWords) ||
        ...
        {
            exp = exp.And(x => x.MemberName!.Contains(queryDto.MemberName));
        }
        ...
        {
            exp = exp.And(x => x.PhoneNumber!.Contains(queryDto.PhoneNumber));
        }

        if (!string.IsNullOrEmpty(queryDto?.WorkUnit))
        {
            exp = exp.And(x => x.WorkUnit!.Contains(queryDto.WorkUnit));
        }
