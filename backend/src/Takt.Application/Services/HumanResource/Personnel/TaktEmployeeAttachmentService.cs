...
// 功能描述：员工附件表应用服务，提供EmployeeAttachment管理的业务逻辑
//
...
public class TaktEmployeeAttachmentService : TaktServiceBase, ITaktEmployeeAttachmentService
{
    ...
    /// 构造函数
    /// </summary>
    /// <param name="repository">EmployeeAttachment仓储</param>
    ...
    /// <summary>
    /// 获取员工附件表(EmployeeAttachment)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>员工附件表(EmployeeAttachment)选项列表</returns>
    ...
    /// </summary>
    /// <param name="dto">创建员工附件表(EmployeeAttachment)DTO</param>
    /// <returns>员工附件表(EmployeeAttachment)DTO</returns>
    public async Task<TaktEmployeeAttachmentDto> CreateEmployeeAttachmentAsync(TaktEmployeeAttachmentCreateDto dto)
    {
        ...
        if (!isUnique)
            throw new TaktBusinessException($"员工附件表FileCode {dto.FileCode} 已存在");
    ...
    /// </summary>
    /// <param name="id">员工附件表(EmployeeAttachment)ID</param>
    /// <param name="dto">更新员工附件表(EmployeeAttachment)DTO</param>
    /// <returns>员工附件表(EmployeeAttachment)DTO</returns>
    ...
    public async Task DeleteEmployeeAttachmentBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktEmployeeAttachment>();
    ...
    {
        var entity = await _repository.GetByIdAsync(dto.EmployeeAttachmentId);
        if (entity == null)
            throw new TaktBusinessException("validation.employeeattachmentNotFound");
    ...
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktEmployeeAttachment));
        var importData = await TaktExcelHelper.ImportAsync<TaktEmployeeAttachmentImportDto>(fileStream, excelSheet);
    ...
    public async Task<(string fileName, byte[] content)> ExportEmployeeAttachmentAsync(TaktEmployeeAttachmentQueryDto query, string? sheetName, string? fileName)
    {
        ...
        var exportData = list.Select(x => x.Adapt<TaktEmployeeAttachmentExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建员工附件表查询表达式
    /// </summary>
    /// <param name="queryDto">员工附件表查询DTO</param>
    ...
    private static Expression<Func<TaktEmployeeAttachment, bool>> QueryExpression(TaktEmployeeAttachmentQueryDto? queryDto)
    {
        ...
        if (!string.IsNullOrEmpty(queryDto?.KeyWords))
        {
            exp = exp.And(x =>
                ...
                x.AttachmentDescription!.Contains(queryDto.KeyWords) ||
        ...
        {
            exp = exp.And(x => x.AttachmentDescription!.Contains(queryDto.AttachmentDescription));
        }
