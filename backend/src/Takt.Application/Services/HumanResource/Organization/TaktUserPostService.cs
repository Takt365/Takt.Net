...
public class TaktUserPostService : TaktServiceBase, ITaktUserPostService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.PostId, dto.PostId, null, $"岗位用户关联表编码 {dto.PostId} 已存在");
    ...
    public async Task<TaktUserPostDto> UpdateUserPostAsync(long id, TaktUserPostUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.PostId, dto.PostId, id, $"岗位用户关联表编码 {dto.PostId} 已存在");
