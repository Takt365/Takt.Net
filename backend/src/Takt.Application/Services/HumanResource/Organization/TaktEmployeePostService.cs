...
public class TaktEmployeePostService : TaktServiceBase, ITaktEmployeePostService
{
    ...
    public async Task<TaktEmployeePostDto> CreateEmployeePostAsync(TaktEmployeePostCreateDto dto)
    {
        ...
        // 验证PostId、EmployeeId组合的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.PostId == dto.PostId && x.EmployeeId == dto.EmployeeId);
        if (!isUnique)
            throw new TaktBusinessException($"员工岗位关联表PostId、EmployeeId组合已存在");
    ...
    public async Task<TaktEmployeePostDto> UpdateEmployeePostAsync(long id, TaktEmployeePostUpdateDto dto)
    {
        ...
        // 验证PostId、EmployeeId组合的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.PostId == dto.PostId && x.EmployeeId == dto.EmployeeId, id);
        if (!isUnique)
            throw new TaktBusinessException($"员工岗位关联表PostId、EmployeeId组合已存在");
