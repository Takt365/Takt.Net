...
public class TaktSalaryStructureService : TaktServiceBase, ITaktSalaryStructureService
{
    ...
    public async Task<TaktSalaryStructureDto> CreateSalaryStructureAsync(TaktSalaryStructureCreateDto dto)
    {
        ...
        // 验证StructureCode的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.StructureCode, dto.StructureCode);
        if (!isUnique)
            throw new TaktBusinessException($"薪资结构表StructureCode {dto.StructureCode} 已存在");
    ...
    public async Task<TaktSalaryStructureDto> UpdateSalaryStructureAsync(long id, TaktSalaryStructureUpdateDto dto)
    {
        ...
        // 验证StructureCode的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.StructureCode, dto.StructureCode, id);
        if (!isUnique)
            throw new TaktBusinessException($"薪资结构表StructureCode {dto.StructureCode} 已存在");
