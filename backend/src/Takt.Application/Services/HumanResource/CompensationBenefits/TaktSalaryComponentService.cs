...
public class TaktSalaryComponentService : TaktServiceBase, ITaktSalaryComponentService
{
    ...
    public async Task<TaktSalaryComponentDto> CreateSalaryComponentAsync(TaktSalaryComponentCreateDto dto)
    {
        ...
        // 验证ComponentCode的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.ComponentCode, dto.ComponentCode);
        if (!isUnique)
            throw new TaktBusinessException($"薪资组成表ComponentCode {dto.ComponentCode} 已存在");
    ...
    public async Task<TaktSalaryComponentDto> UpdateSalaryComponentAsync(long id, TaktSalaryComponentUpdateDto dto)
    {
        ...
        // 验证ComponentCode的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.ComponentCode, dto.ComponentCode, id);
        if (!isUnique)
            throw new TaktBusinessException($"薪资组成表ComponentCode {dto.ComponentCode} 已存在");
