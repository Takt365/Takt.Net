...
public class TaktPerformancePlanService : TaktServiceBase, ITaktPerformancePlanService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.PlanCode, dto.PlanCode, null, $"绩效方案表编码 {dto.PlanCode} 已存在");
    ...
    public async Task<TaktPerformancePlanDto> UpdatePerformancePlanAsync(long id, TaktPerformancePlanUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.PlanCode, dto.PlanCode, id, $"绩效方案表编码 {dto.PlanCode} 已存在");
