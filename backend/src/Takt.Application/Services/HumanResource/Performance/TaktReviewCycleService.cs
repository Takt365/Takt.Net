...
public class TaktReviewCycleService : TaktServiceBase, ITaktReviewCycleService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.CycleCode, dto.CycleCode, null, $"评审周期表编码 {dto.CycleCode} 已存在");
    ...
    public async Task<TaktReviewCycleDto> UpdateReviewCycleAsync(long id, TaktReviewCycleUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.CycleCode, dto.CycleCode, id, $"评审周期表编码 {dto.CycleCode} 已存在");
