...
public class TaktTrainingCourseService : TaktServiceBase, ITaktTrainingCourseService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.CourseCode, dto.CourseCode, null, $"培训课程表编码 {dto.CourseCode} 已存在");
    ...
    public async Task<TaktTrainingCourseDto> UpdateTrainingCourseAsync(long id, TaktTrainingCourseUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.CourseCode, dto.CourseCode, id, $"培训课程表编码 {dto.CourseCode} 已存在");
