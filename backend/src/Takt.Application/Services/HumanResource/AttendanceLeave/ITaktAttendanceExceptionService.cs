...
// 创建时间：2026-05-10
// 创建人：Takt365
// 功能描述：考勤异常表应用服务接口，定义AttendanceException管理的业务操作
// 
...
public interface ITaktAttendanceExceptionService
{
    ...
    /// 根据ID获取考勤异常表
    /// </summary>
    /// <param name="id">考勤异常表ID</param>
    /// <returns>考勤异常表DTO</returns>
    Task<TaktAttendanceExceptionDto?> GetAttendanceExceptionByIdAsync(long id);

    /// <summary>
    /// 获取考勤异常表选项列表（用于下拉框等）
    /// </summary>
    /// <returns>考勤异常表选项列表</returns>
    Task<List<TaktSelectOption>> GetAttendanceExceptionOptionsAsync();

    /// <summary>
    /// 创建考勤异常表
    /// </summary>
    /// <param name="dto">创建考勤异常表DTO</param>
    /// <returns>考勤异常表DTO</returns>
    ...
    /// </summary>
    /// <param name="id">考勤异常表ID</param>
    /// <param name="dto">更新考勤异常表DTO</param>
    /// <returns>考勤异常表DTO</returns>
    ...
    /// <summary>
    /// 更新考勤异常表(AttendanceException)HandleStatus
    /// </summary>
    /// <param name="dto">考勤异常表(AttendanceException)HandleStatusDTO</param>
    /// <returns>考勤异常表(AttendanceException)DTO</returns>
    Task<TaktAttendanceExceptionDto> UpdateAttendanceExceptionHandleStatusAsync(TaktAttendanceExceptionHandleStatusDto dto);
    ...
    /// </summary>
    /// <param name="query">考勤异常表(AttendanceException)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
