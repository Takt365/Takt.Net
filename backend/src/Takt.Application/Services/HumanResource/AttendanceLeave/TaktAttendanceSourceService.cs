...
public class TaktAttendanceSourceService : TaktServiceBase, ITaktAttendanceSourceService
{
    ...
    public async Task<TaktAttendanceSourceDto> CreateAttendanceSourceAsync(TaktAttendanceSourceCreateDto dto)
    {
        ...
        // 验证EnrollNumber的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.EnrollNumber, dto.EnrollNumber);
        if (!isUnique)
            throw new TaktBusinessException($"考勤源记录表EnrollNumber {dto.EnrollNumber} 已存在");
    ...
    public async Task<TaktAttendanceSourceDto> UpdateAttendanceSourceAsync(long id, TaktAttendanceSourceUpdateDto dto)
    {
        ...
        // 验证EnrollNumber的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.EnrollNumber, dto.EnrollNumber, id);
        if (!isUnique)
            throw new TaktBusinessException($"考勤源记录表EnrollNumber {dto.EnrollNumber} 已存在");
