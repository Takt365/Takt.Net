...
public class TaktAttendanceCorrectionService : TaktServiceBase, ITaktAttendanceCorrectionService
{
    ...
    /// 构造函数
    /// </summary>
    /// <param name="repository">补卡记录仓储</param>
    ...
    public async Task<TaktAttendanceCorrectionDto> CreateAttendanceCorrectionAsync(TaktAttendanceCorrectionCreateDto dto)
    {
        ...
        entity.TargetDate = dto.TargetDate.Date;
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportAttendanceCorrectionAsync(Stream fileStream, string? sheetName)
    {
        ...
        try
        {
            var excelSheet = ResolveExcelSheetName(sheetName, nameof(TaktAttendanceCorrection));
            ...
            var toInsert = new List<TaktAttendanceCorrection>();
            ...
            foreach (var (item, index) in importData.Select((item, index) => (item, index + 3)))
            {
                try
                {
                    ...
                    {
                        AddImportError(errors, "validation.importRowAttendanceCorrectionEmployeeRequired", index);
                        fail++;
                        continue;
                    }
                    ...
                    {
                        AddImportError(errors, "validation.importRowAttendanceCorrectionReasonRequired", index);
                        fail++;
                        continue;
                    }

                    var entity = item.Adapt<TaktAttendanceCorrection>();
                    entity.TargetDate = item.TargetDate.Date;
    ...
    public async Task<(string fileName, byte[] content)> ExportAttendanceCorrectionAsync(TaktAttendanceCorrectionQueryDto query, string? sheetName, string? fileName)
    {
        ...
        if (list == null || list.Count == 0)
            return await TaktExcelHelper.ExportAsync(new List<TaktAttendanceCorrectionExportDto>(), excelSheet, excelFile);
    ...
    /// <summary>
    /// 构建分页、导出与条件查询用的补卡记录筛选表达式（员工、审批状态、目标日期范围、原因/备注关键词）。
    /// </summary>
    /// <param name="q">查询条件，可为 null</param>
    /// <returns>用于仓储的表达式</returns>
    private static Expression<Func<TaktAttendanceCorrection, bool>> QueryExpression(TaktAttendanceCorrectionQueryDto? q)
    {
