...
public class TaktAssetService : TaktServiceBase, ITaktAssetService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.AssetCode, dto.AssetCode, null, $"固定资产表编码 {dto.AssetCode} 已存在");
    ...
    public async Task<TaktAssetDto> UpdateAssetAsync(long id, TaktAssetUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.AssetCode, dto.AssetCode, id, $"固定资产表编码 {dto.AssetCode} 已存在");
