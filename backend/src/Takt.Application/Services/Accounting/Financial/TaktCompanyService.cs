...
public class TaktCompanyService : TaktServiceBase, ITaktCompanyService
{
    ...
    public async Task<TaktCompanyDto> CreateCompanyAsync(TaktCompanyCreateDto dto)
    {
        ...
        // 验证公司代码、CompanyName组合的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.CompanyCode == dto.CompanyCode && x.CompanyName == dto.CompanyName);
        if (!isUnique)
            throw new TaktBusinessException($"公司信息表公司代码、CompanyName组合已存在");
    ...
    public async Task<TaktCompanyDto> UpdateCompanyAsync(long id, TaktCompanyUpdateDto dto)
    {
        ...
        // 验证公司代码、CompanyName组合的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.CompanyCode == dto.CompanyCode && x.CompanyName == dto.CompanyName, id);
        if (!isUnique)
            throw new TaktBusinessException($"公司信息表公司代码、CompanyName组合已存在");
