...
// 功能描述：资产变更记录表应用服务，提供AssetChangeLog管理的业务逻辑
//
...
{
    private readonly ITaktRepository<TaktAssetChangeLog> _repository;
    private readonly ITaktUniqueValidator _uniqueValidator;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="repository">AssetChangeLog仓储</param>
    /// <param name="uniqueValidator">唯一性验证器</param>
    /// <param name="userContext">用户上下文（可选）</param>
    /// <param name="tenantContext">租户上下文（可选）</param>
    /// <param name="localizer">本地化器（可选）</param>
    public TaktAssetChangeLogService(
        ITaktRepository<TaktAssetChangeLog> repository,
    ...
    /// <returns>资产变更记录表(AssetChangeLog)DTO</returns>
    public async Task<TaktAssetChangeLogDto?> GetAssetChangeLogByIdAsync(long id)
    {
        var entity = await _repository.GetByIdAsync(id);
        return entity == null ? null : entity.Adapt<TaktAssetChangeLogDto>();
    }


    /// <summary>
    /// 获取资产变更记录表(AssetChangeLog)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>资产变更记录表(AssetChangeLog)选项列表</returns>
    public async Task<List<TaktSelectOption>> GetAssetChangeLogOptionsAsync()
    {
        var all = await _repository.FindAsync(x => x.IsDeleted == 0);
        return all.Select(x => new TaktSelectOption
        {
            DictLabel = x.AssetCode ?? string.Empty,
            DictValue = x.AssetCode
    ...
    /// </summary>
    /// <param name="dto">创建资产变更记录表(AssetChangeLog)DTO</param>
    /// <returns>资产变更记录表(AssetChangeLog)DTO</returns>
    public async Task<TaktAssetChangeLogDto> CreateAssetChangeLogAsync(TaktAssetChangeLogCreateDto dto)
    {
        ...
        // 验证AssetCode的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.AssetCode, dto.AssetCode);
        if (!isUnique)
            throw new TaktBusinessException($"资产变更记录表AssetCode {dto.AssetCode} 已存在");
    ...
    /// </summary>
    /// <param name="id">资产变更记录表(AssetChangeLog)ID</param>
    /// <param name="dto">更新资产变更记录表(AssetChangeLog)DTO</param>
    /// <returns>资产变更记录表(AssetChangeLog)DTO</returns>
    public async Task<TaktAssetChangeLogDto> UpdateAssetChangeLogAsync(long id, TaktAssetChangeLogUpdateDto dto)
    {
        var entity = await _repository.GetByIdAsync(id);
        if (entity == null)
            throw new TaktBusinessException("validation.assetchangelogNotFound");
        // 验证AssetCode的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.AssetCode, dto.AssetCode, id);
        if (!isUnique)
            throw new TaktBusinessException($"资产变更记录表AssetCode {dto.AssetCode} 已存在");

        dto.Adapt(entity, typeof(TaktAssetChangeLogUpdateDto), typeof(TaktAssetChangeLog));
        entity.UpdatedAt = DateTime.Now;
        await _repository.UpdateAsync(entity);

        return (await GetAssetChangeLogByIdAsync(id)) ?? entity.Adapt<TaktAssetChangeLogDto>();
    }
    ...
    public async Task DeleteAssetChangeLogBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktAssetChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> GetAssetChangeLogTemplateAsync(string? sheetName, string? fileName)
    {
        var (excelSheet, excelFile) = await ResolveExcelImportTemplateNamesAsync(sheetName, fileName, nameof(TaktAssetChangeLog));
        return await TaktExcelHelper.GenerateTemplateAsync<TaktAssetChangeLogTemplateDto>(
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportAssetChangeLogAsync(Stream fileStream, string? sheetName)
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktAssetChangeLog));
        var importData = await TaktExcelHelper.ImportAsync<TaktAssetChangeLogImportDto>(fileStream, excelSheet);
        ...
        foreach (var item in importData)
        {
            ...
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktAssetChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> ExportAssetChangeLogAsync(TaktAssetChangeLogQueryDto query, string? sheetName, string? fileName)
    {
        var predicate = QueryExpression(query ?? new TaktAssetChangeLogQueryDto());
        List<TaktAssetChangeLog> list;
        if (predicate != null)
            list = await _repository.FindAsync(predicate);
        else
            list = await _repository.GetAllAsync();

        var (excelSheet, excelFile) = await ResolveExcelExportNamesAsync(sheetName, fileName, nameof(TaktAssetChangeLog));
        ...
        var exportData = list.Select(x => x.Adapt<TaktAssetChangeLogExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建资产变更记录表查询表达式
    /// </summary>
    /// <param name="queryDto">资产变更记录表查询DTO</param>
