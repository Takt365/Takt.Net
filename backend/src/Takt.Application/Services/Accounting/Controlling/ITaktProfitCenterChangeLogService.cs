...
// 创建时间：2026-05-10
// 创建人：Takt365
// 功能描述：利润中心变更记录表应用服务接口，定义ProfitCenterChangeLog管理的业务操作
// 
...
public interface ITaktProfitCenterChangeLogService
{
    ...
    /// 根据ID获取利润中心变更记录表
    /// </summary>
    /// <param name="id">利润中心变更记录表ID</param>
    /// <returns>利润中心变更记录表DTO</returns>
    Task<TaktProfitCenterChangeLogDto?> GetProfitCenterChangeLogByIdAsync(long id);

    /// <summary>
    /// 获取利润中心变更记录表选项列表（用于下拉框等）
    /// </summary>
    /// <returns>利润中心变更记录表选项列表</returns>
    Task<List<TaktSelectOption>> GetProfitCenterChangeLogOptionsAsync();

    /// <summary>
    /// 创建利润中心变更记录表
    /// </summary>
    /// <param name="dto">创建利润中心变更记录表DTO</param>
    /// <returns>利润中心变更记录表DTO</returns>
    ...
    /// </summary>
    /// <param name="id">利润中心变更记录表ID</param>
    /// <param name="dto">更新利润中心变更记录表DTO</param>
    /// <returns>利润中心变更记录表DTO</returns>
    ...
    Task<(string fileName, byte[] content)> GetProfitCenterChangeLogTemplateAsync(string? sheetName, string? fileName);
    ...
    Task<(int success, int fail, List<string> errors)> ImportProfitCenterChangeLogAsync(Stream fileStream, string? sheetName);

    /// <summary>
    /// 导出利润中心变更记录表(ProfitCenterChangeLog)
    /// </summary>
    /// <param name="query">利润中心变更记录表(ProfitCenterChangeLog)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
    /// <returns>Excel文件信息（文件名和内容）</returns>
    Task<(string fileName, byte[] content)> ExportProfitCenterChangeLogAsync(TaktProfitCenterChangeLogQueryDto query, string? sheetName, string? fileName);
}
