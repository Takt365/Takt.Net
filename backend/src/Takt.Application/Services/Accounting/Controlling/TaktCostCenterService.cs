...
// 功能描述：成本中心表应用服务，提供CostCenter管理的业务逻辑
//
...
{
    private readonly ITaktRepository<TaktCostCenter> _repository;
    private readonly ITaktUniqueValidator _uniqueValidator;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="repository">CostCenter仓储</param>
    /// <param name="uniqueValidator">唯一性验证器</param>
    /// <param name="userContext">用户上下文（可选）</param>
    /// <param name="tenantContext">租户上下文（可选）</param>
    /// <param name="localizer">本地化器（可选）</param>
    public TaktCostCenterService(
        ITaktRepository<TaktCostCenter> repository,
    ...
    public async Task<TaktCostCenterDto?> GetCostCenterByIdAsync(long id)
    {
        ...
        return entity == null ? null : entity.Adapt<TaktCostCenterDto>();
    }


    /// <summary>
    /// 获取成本中心表(CostCenter)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>成本中心表(CostCenter)选项列表</returns>
    public async Task<List<TaktSelectOption>> GetCostCenterOptionsAsync()
    {
        var all = await _repository.FindAsync(x => x.IsDeleted == 0 && x.CostCenterStatus == 1);
        return all.Select(x => new TaktSelectOption
        {
            DictLabel = x.CostCenterName ?? string.Empty,
    ...
    /// 构建树形选项列表（递归）
    /// </summary>
    private List<TaktTreeSelectOption> BuildTreeOptions(List<TaktCostCenter> all, long parentId)
    {
        ...
        foreach (var item in children)
        {
            var option = new TaktTreeSelectOption
            {
                ...
                DictLabel = item.CostCenterName ?? string.Empty,
    ...
    /// <param name="includeDisabled">是否包含禁用的成本中心表</param>
    ...
    public async Task<List<TaktCostCenterTreeDto>> GetCostCenterTreeAsync(long parentId = 0, bool includeDisabled = false)
    {
        ...
        var filteredRecords = includeDisabled
            ? allRecords
            : allRecords.Where(x => x.CostCenterStatus == 1);
        
        var buildTreeStart = DateTime.Now;
        
        // 在内存中构建树形结构
        var treeList = BuildCostCenterTree(filteredRecords.ToList(), parentId);
        
        var elapsed = (DateTime.Now - startTime).TotalMilliseconds;
        var buildElapsed = (DateTime.Now - buildTreeStart).TotalMilliseconds;
        
        TaktLogger.Information("[性能] CostCenter树构建完成 - 总耗时: {Elapsed}ms, 查询耗时: {QueryElapsed}ms, 构建树耗时: {BuildElapsed}ms, 总数: {TotalCount}, 过滤后: {FilteredCount}, 树节点数: {TreeCount}",
            elapsed,
    ...
    public async Task<List<TaktCostCenterDto>> GetCostCenterChildrenAsync(long parentId, bool includeDisabled = false)
    {
        // 构建查询条件
        Expression<Func<TaktCostCenter, bool>> predicate = includeDisabled
            ? x => x.ParentId == parentId
            : x => x.ParentId == parentId && x.CostCenterStatus == 1;
        
        var children = await _repository.FindAsync(predicate);
                return children.OrderBy(x => x.SortOrder).Select(x => x.Adapt<TaktCostCenterDto>()).ToList();
    }
    ...
    /// 创建成本中心表(CostCenter)
    /// </summary>
    /// <param name="dto">创建成本中心表(CostCenter)DTO</param>
    /// <returns>成本中心表(CostCenter)DTO</returns>
    public async Task<TaktCostCenterDto> CreateCostCenterAsync(TaktCostCenterCreateDto dto)
    {
        var entity = dto.Adapt<TaktCostCenter>();
        // 验证公司代码、成本中心编码、成本中心名称组合的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.CompanyCode == dto.CompanyCode && x.CostCenterCode == dto.CostCenterCode && x.CostCenterName == dto.CostCenterName);
        if (!isUnique)
            throw new TaktBusinessException($"成本中心表公司代码、成本中心编码、成本中心名称组合已存在");
    ...
    /// </summary>
    /// <param name="id">成本中心表(CostCenter)ID</param>
    /// <param name="dto">更新成本中心表(CostCenter)DTO</param>
    /// <returns>成本中心表(CostCenter)DTO</returns>
    public async Task<TaktCostCenterDto> UpdateCostCenterAsync(long id, TaktCostCenterUpdateDto dto)
    {
        ...
        // 验证公司代码、成本中心编码、成本中心名称组合的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.CompanyCode == dto.CompanyCode && x.CostCenterCode == dto.CostCenterCode && x.CostCenterName == dto.CostCenterName, id);
        if (!isUnique)
            throw new TaktBusinessException($"成本中心表公司代码、成本中心编码、成本中心名称组合已存在");

        dto.Adapt(entity, typeof(TaktCostCenterUpdateDto), typeof(TaktCostCenter));
        entity.UpdatedAt = DateTime.Now;
        await _repository.UpdateAsync(entity);

        return (await GetCostCenterByIdAsync(id)) ?? entity.Adapt<TaktCostCenterDto>();
    }
    ...
    public async Task DeleteCostCenterBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktCostCenter>();
        foreach (var id in idList)
        {
            var entity = await _repository.GetByIdAsync(id);
        ...
        // 批量更新：设置 IsDeleted = 1，并同步更新 CostCenterStatus = 1（禁用）
    ...
    /// </summary>
    /// <param name="dto">成本中心表(CostCenter)状态DTO</param>
    /// <returns>成本中心表(CostCenter)DTO</returns>
    public async Task<TaktCostCenterDto> UpdateCostCenterStatusAsync(TaktCostCenterStatusDto dto)
    {
        var entity = await _repository.GetByIdAsync(dto.CostCenterId);
        if (entity == null)
            throw new TaktBusinessException("validation.costcenterNotFound");
    ...
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktCostCenter));
        var importData = await TaktExcelHelper.ImportAsync<TaktCostCenterImportDto>(fileStream, excelSheet);
        ...
        foreach (var item in importData)
        {
            ...
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktCostCenter>();
    ...
    public async Task<(string fileName, byte[] content)> ExportCostCenterAsync(TaktCostCenterQueryDto query, string? sheetName, string? fileName)
    {
        ...
        var exportData = list.Select(x => x.Adapt<TaktCostCenterExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建成本中心表查询表达式
    /// </summary>
    /// <param name="queryDto">成本中心表查询DTO</param>
