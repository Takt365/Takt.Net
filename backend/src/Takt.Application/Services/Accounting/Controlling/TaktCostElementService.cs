...
public class TaktCostElementService : TaktServiceBase, ITaktCostElementService
{
    ...
    /// <summary>
    /// 获取成本要素表(CostElement)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>成本要素表(CostElement)选项列表</returns>
    public async Task<List<TaktSelectOption>> GetCostElementOptionsAsync()
    {
        var all = await _repository.FindAsync(x => x.IsDeleted == 0 && x.CostElementStatus == 1);
    ...
    /// 构建树形选项列表（递归）
    /// </summary>
    private List<TaktTreeSelectOption> BuildTreeOptions(List<TaktCostElement> all, long parentId)
    {
    ...
    /// <param name="includeDisabled">是否包含禁用的成本要素表</param>
    ...
    public async Task<List<TaktCostElementTreeDto>> GetCostElementTreeAsync(long parentId = 0, bool includeDisabled = false)
    {
        ...
        var filteredRecords = includeDisabled
            ? allRecords
            : allRecords.Where(x => x.CostElementStatus == 1);
        
        var buildTreeStart = DateTime.Now;
        
        // 在内存中构建树形结构
        var treeList = BuildCostElementTree(filteredRecords.ToList(), parentId);
        
        var elapsed = (DateTime.Now - startTime).TotalMilliseconds;
        var buildElapsed = (DateTime.Now - buildTreeStart).TotalMilliseconds;
        
        TaktLogger.Information("[性能] CostElement树构建完成 - 总耗时: {Elapsed}ms, 查询耗时: {QueryElapsed}ms, 构建树耗时: {BuildElapsed}ms, 总数: {TotalCount}, 过滤后: {FilteredCount}, 树节点数: {TreeCount}",
            elapsed,
    ...
    public async Task<List<TaktCostElementDto>> GetCostElementChildrenAsync(long parentId, bool includeDisabled = false)
    {
        // 构建查询条件
        Expression<Func<TaktCostElement, bool>> predicate = includeDisabled
            ? x => x.ParentId == parentId
            : x => x.ParentId == parentId && x.CostElementStatus == 1;
        
        var children = await _repository.FindAsync(predicate);
                return children.OrderBy(x => x.SortOrder).Select(x => x.Adapt<TaktCostElementDto>()).ToList();
    }
    ...
    /// 创建成本要素表(CostElement)
    /// </summary>
    /// <param name="dto">创建成本要素表(CostElement)DTO</param>
    /// <returns>成本要素表(CostElement)DTO</returns>
    ...
    /// </summary>
    /// <param name="id">成本要素表(CostElement)ID</param>
    /// <param name="dto">更新成本要素表(CostElement)DTO</param>
    /// <returns>成本要素表(CostElement)DTO</returns>
    public async Task<TaktCostElementDto> UpdateCostElementAsync(long id, TaktCostElementUpdateDto dto)
    {
        ...
        return (await GetCostElementByIdAsync(id)) ?? entity.Adapt<TaktCostElementDto>();
    }
    ...
    public async Task DeleteCostElementBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktCostElement>();
        foreach (var id in idList)
        {
            var entity = await _repository.GetByIdAsync(id);
        ...
        // 批量更新：设置 IsDeleted = 1，并同步更新 CostElementStatus = 1（禁用）
    ...
    /// </summary>
    /// <param name="dto">成本要素表(CostElement)状态DTO</param>
    /// <returns>成本要素表(CostElement)DTO</returns>
    public async Task<TaktCostElementDto> UpdateCostElementStatusAsync(TaktCostElementStatusDto dto)
    {
        var entity = await _repository.GetByIdAsync(dto.CostElementId);
        if (entity == null)
            throw new TaktBusinessException("validation.costelementNotFound");
    ...
    public async Task<(string fileName, byte[] content)> GetCostElementTemplateAsync(string? sheetName, string? fileName)
    {
        var (excelSheet, excelFile) = await ResolveExcelImportTemplateNamesAsync(sheetName, fileName, nameof(TaktCostElement));
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportCostElementAsync(Stream fileStream, string? sheetName)
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktCostElement));
        var importData = await TaktExcelHelper.ImportAsync<TaktCostElementImportDto>(fileStream, excelSheet);
        ...
        foreach (var item in importData)
        {
            ...
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktCostElement>();
    ...
    public async Task<(string fileName, byte[] content)> ExportCostElementAsync(TaktCostElementQueryDto query, string? sheetName, string? fileName)
    {
        ...
        var (excelSheet, excelFile) = await ResolveExcelExportNamesAsync(sheetName, fileName, nameof(TaktCostElement));
        ...
        var exportData = list.Select(x => x.Adapt<TaktCostElementExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建成本要素表查询表达式
    /// </summary>
    /// <param name="queryDto">成本要素表查询DTO</param>
    ...
    private static Expression<Func<TaktCostElement, bool>> QueryExpression(TaktCostElementQueryDto? queryDto)
    {
        ...
        if (!string.IsNullOrEmpty(queryDto?.KeyWords))
        {
            exp = exp.And(x =>
                ...
                x.CostElementName!.Contains(queryDto.KeyWords) ||
        ...
        {
            exp = exp.And(x => x.CostElementName!.Contains(queryDto.CostElementName));
        }
