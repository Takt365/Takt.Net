...
// 功能描述：成本中心变更记录表应用服务，提供CostCenterChangeLog管理的业务逻辑
//
...
{
    private readonly ITaktRepository<TaktCostCenterChangeLog> _repository;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="repository">CostCenterChangeLog仓储</param>
    /// <param name="userContext">用户上下文（可选）</param>
    /// <param name="tenantContext">租户上下文（可选）</param>
    /// <param name="localizer">本地化器（可选）</param>
    public TaktCostCenterChangeLogService(
        ITaktRepository<TaktCostCenterChangeLog> repository,
    ...
    /// <returns>成本中心变更记录表(CostCenterChangeLog)DTO</returns>
    public async Task<TaktCostCenterChangeLogDto?> GetCostCenterChangeLogByIdAsync(long id)
    {
        var entity = await _repository.GetByIdAsync(id);
        return entity == null ? null : entity.Adapt<TaktCostCenterChangeLogDto>();
    }


    /// <summary>
    /// 获取成本中心变更记录表(CostCenterChangeLog)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>成本中心变更记录表(CostCenterChangeLog)选项列表</returns>
    public async Task<List<TaktSelectOption>> GetCostCenterChangeLogOptionsAsync()
    {
        var all = await _repository.FindAsync(x => x.IsDeleted == 0);
        return all.Select(x => new TaktSelectOption
        {
            DictLabel = x.CostCenterCode ?? string.Empty,
            DictValue = x.CostCenterCode
    ...
    /// </summary>
    /// <param name="dto">创建成本中心变更记录表(CostCenterChangeLog)DTO</param>
    /// <returns>成本中心变更记录表(CostCenterChangeLog)DTO</returns>
    public async Task<TaktCostCenterChangeLogDto> CreateCostCenterChangeLogAsync(TaktCostCenterChangeLogCreateDto dto)
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.CostCenterCode, dto.CostCenterCode, null, $"成本中心变更记录表编码 {dto.CostCenterCode} 已存在");
    ...
    /// </summary>
    /// <param name="id">成本中心变更记录表(CostCenterChangeLog)ID</param>
    /// <param name="dto">更新成本中心变更记录表(CostCenterChangeLog)DTO</param>
    /// <returns>成本中心变更记录表(CostCenterChangeLog)DTO</returns>
    public async Task<TaktCostCenterChangeLogDto> UpdateCostCenterChangeLogAsync(long id, TaktCostCenterChangeLogUpdateDto dto)
    {
        var entity = await _repository.GetByIdAsync(id);
        if (entity == null)
            throw new TaktBusinessException("validation.costcenterchangelogNotFound");

        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.CostCenterCode, dto.CostCenterCode, id, $"成本中心变更记录表编码 {dto.CostCenterCode} 已存在");

        dto.Adapt(entity, typeof(TaktCostCenterChangeLogUpdateDto), typeof(TaktCostCenterChangeLog));
        entity.UpdatedAt = DateTime.Now;
        await _repository.UpdateAsync(entity);

        return (await GetCostCenterChangeLogByIdAsync(id)) ?? entity.Adapt<TaktCostCenterChangeLogDto>();
    }
    ...
    public async Task DeleteCostCenterChangeLogBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktCostCenterChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> GetCostCenterChangeLogTemplateAsync(string? sheetName, string? fileName)
    {
        var (excelSheet, excelFile) = await ResolveExcelImportTemplateNamesAsync(sheetName, fileName, nameof(TaktCostCenterChangeLog));
        return await TaktExcelHelper.GenerateTemplateAsync<TaktCostCenterChangeLogTemplateDto>(
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportCostCenterChangeLogAsync(Stream fileStream, string? sheetName)
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktCostCenterChangeLog));
        var importData = await TaktExcelHelper.ImportAsync<TaktCostCenterChangeLogImportDto>(fileStream, excelSheet);
        ...
        foreach (var item in importData)
        {
            ...
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktCostCenterChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> ExportCostCenterChangeLogAsync(TaktCostCenterChangeLogQueryDto query, string? sheetName, string? fileName)
    {
        var predicate = QueryExpression(query ?? new TaktCostCenterChangeLogQueryDto());
        List<TaktCostCenterChangeLog> list;
        if (predicate != null)
            list = await _repository.FindAsync(predicate);
        else
            list = await _repository.GetAllAsync();

        var (excelSheet, excelFile) = await ResolveExcelExportNamesAsync(sheetName, fileName, nameof(TaktCostCenterChangeLog));
        ...
        var exportData = list.Select(x => x.Adapt<TaktCostCenterChangeLogExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建成本中心变更记录表查询表达式
    /// </summary>
    /// <param name="queryDto">成本中心变更记录表查询DTO</param>
