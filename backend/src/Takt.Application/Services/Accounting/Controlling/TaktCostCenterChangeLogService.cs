...
public class TaktCostCenterChangeLogService : TaktServiceBase, ITaktCostCenterChangeLogService
{
    ...
    public async Task<TaktCostCenterChangeLogDto> CreateCostCenterChangeLogAsync(TaktCostCenterChangeLogCreateDto dto)
    {
        ...
        // 验证成本中心编码的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.CostCenterCode, dto.CostCenterCode);
        if (!isUnique)
            throw new TaktBusinessException($"成本中心变更记录表成本中心编码 {dto.CostCenterCode} 已存在");
    ...
    public async Task<TaktCostCenterChangeLogDto> UpdateCostCenterChangeLogAsync(long id, TaktCostCenterChangeLogUpdateDto dto)
    {
        ...
        // 验证成本中心编码的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.CostCenterCode, dto.CostCenterCode, id);
        if (!isUnique)
            throw new TaktBusinessException($"成本中心变更记录表成本中心编码 {dto.CostCenterCode} 已存在");
