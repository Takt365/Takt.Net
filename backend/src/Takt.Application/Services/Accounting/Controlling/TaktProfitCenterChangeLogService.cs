...
public class TaktProfitCenterChangeLogService : TaktServiceBase, ITaktProfitCenterChangeLogService
{
    ...
    public async Task<TaktProfitCenterChangeLogDto> CreateProfitCenterChangeLogAsync(TaktProfitCenterChangeLogCreateDto dto)
    {
        ...
        // 验证ProfitCenterCode的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.ProfitCenterCode, dto.ProfitCenterCode);
        if (!isUnique)
            throw new TaktBusinessException($"利润中心变更记录表ProfitCenterCode {dto.ProfitCenterCode} 已存在");
    ...
    public async Task<TaktProfitCenterChangeLogDto> UpdateProfitCenterChangeLogAsync(long id, TaktProfitCenterChangeLogUpdateDto dto)
    {
        ...
        // 验证ProfitCenterCode的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.ProfitCenterCode, dto.ProfitCenterCode, id);
        if (!isUnique)
            throw new TaktBusinessException($"利润中心变更记录表ProfitCenterCode {dto.ProfitCenterCode} 已存在");
