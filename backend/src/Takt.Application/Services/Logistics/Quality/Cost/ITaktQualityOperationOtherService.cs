...
// 创建时间：2026-05-10
// 创建人：Takt365
// 功能描述：品质业务其他通常业务费用明细表应用服务接口（主子表），定义QualityOperationOther管理的业务操作
// 
...
public interface ITaktQualityOperationOtherService
{
    ...
    /// <summary>
    /// 批量删除品质业务其他通常业务费用明细表(QualityOperationOther)（级联删除子表）
    /// </summary>
    ...
    Task<(string fileName, byte[] content)> GetQualityOperationOtherTemplateAsync(string? sheetName, string? fileName);
    ...
    Task<(int success, int fail, List<string> errors)> ImportQualityOperationOtherAsync(Stream fileStream, string? sheetName);

    /// <summary>
    /// 导出品质业务其他通常业务费用明细表(QualityOperationOther)
    /// </summary>
    /// <param name="query">品质业务其他通常业务费用明细表(QualityOperationOther)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
    /// <returns>Excel文件信息（文件名和内容）</returns>
    Task<(string fileName, byte[] content)> ExportQualityOperationOtherAsync(TaktQualityOperationOtherQueryDto query, string? sheetName, string? fileName);
}
