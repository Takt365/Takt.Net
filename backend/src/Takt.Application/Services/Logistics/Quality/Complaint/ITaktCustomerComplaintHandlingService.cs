// ========================================
// 项目名称：节拍数字工厂 ·Takt Digital Factory (TDF) 
// 命名空间：Takt.Application.Services.Logistics.Quality.Complaint
// 文件名称：ITaktCustomerComplaintHandlingService.cs
// 创建时间：2026-05-11
// 创建人：Takt365
// 功能描述：客诉处理记录表应用服务接口（主子表），定义CustomerComplaintHandling管理的业务操作
// 
// 版权信息：Copyright (c) 2026 Takt  All rights reserved.
// 免责声明：此软件使用 MIT License，作者不承担任何使用风险。
// ========================================

using Takt.Application.Dtos.Logistics.Quality.Complaint;
using Takt.Shared.Models;

namespace Takt.Application.Services.Logistics.Quality.Complaint;

/// <summary>
/// 客诉处理记录表应用服务接口（主子表）
/// </summary>
public interface ITaktCustomerComplaintHandlingService
{
    /// <summary>
    /// 获取客诉处理记录表列表（分页）
    /// </summary>
    /// <param name="queryDto">查询DTO</param>
    /// <returns>分页结果</returns>
    Task<TaktPagedResult<TaktCustomerComplaintHandlingDto>> GetCustomerComplaintHandlingListAsync(TaktCustomerComplaintHandlingQueryDto queryDto);

    /// <summary>
    /// 根据ID获取客诉处理记录表（包含子表数据）
    /// </summary>
    /// <param name="id">客诉处理记录表ID</param>
    /// <returns>客诉处理记录表DTO</returns>
    Task<TaktCustomerComplaintHandlingDto?> GetCustomerComplaintHandlingByIdAsync(long id);

    /// <summary>
    /// 获取客诉处理记录表选项列表（用于下拉框等）
    /// </summary>
    /// <returns>客诉处理记录表选项列表</returns>
    Task<List<TaktSelectOption>> GetCustomerComplaintHandlingOptionsAsync();

    /// <summary>
    /// 创建客诉处理记录表（包含子表数据）
    /// </summary>
    /// <param name="dto">创建客诉处理记录表DTO</param>
    /// <returns>客诉处理记录表DTO</returns>
    Task<TaktCustomerComplaintHandlingDto> CreateCustomerComplaintHandlingAsync(TaktCustomerComplaintHandlingCreateDto dto);

    /// <summary>
    /// 更新客诉处理记录表（包含子表数据）
    /// </summary>
    /// <param name="id">客诉处理记录表ID</param>
    /// <param name="dto">更新客诉处理记录表DTO</param>
    /// <returns>客诉处理记录表DTO</returns>
    Task<TaktCustomerComplaintHandlingDto> UpdateCustomerComplaintHandlingAsync(long id, TaktCustomerComplaintHandlingUpdateDto dto);

    /// <summary>
    /// 删除客诉处理记录表(CustomerComplaintHandling)（级联删除子表）
    /// </summary>
    /// <param name="id">客诉处理记录表(CustomerComplaintHandling)ID</param>
    /// <returns>任务</returns>
    Task DeleteCustomerComplaintHandlingByIdAsync(long id);

    /// <summary>
    /// 批量删除客诉处理记录表(CustomerComplaintHandling)（级联删除子表）
    /// </summary>
    /// <param name="ids">客诉处理记录表(CustomerComplaintHandling)ID列表</param>
    /// <returns>任务</returns>
    Task DeleteCustomerComplaintHandlingBatchAsync(IEnumerable<long> ids);

    /// <summary>
    /// 更新客诉处理记录表(CustomerComplaintHandling)HandlingStatus
    /// </summary>
    /// <param name="dto">客诉处理记录表(CustomerComplaintHandling)HandlingStatusDTO</param>
    /// <returns>客诉处理记录表(CustomerComplaintHandling)DTO</returns>
    Task<TaktCustomerComplaintHandlingDto> UpdateCustomerComplaintHandlingHandlingStatusAsync(TaktCustomerComplaintHandlingHandlingStatusDto dto);

    /// <summary>
    /// 获取导入模板
    /// </summary>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
    /// <returns>Excel模板文件信息（文件名和内容）</returns>
    Task<(string fileName, byte[] content)> GetCustomerComplaintHandlingTemplateAsync(string? sheetName, string? fileName);

    /// <summary>
    /// 导入客诉处理记录表(CustomerComplaintHandling)
    /// </summary>
    /// <param name="fileStream">Excel文件流</param>
    /// <param name="sheetName">工作表名称</param>
    /// <returns>导入结果（成功数量、失败数量、错误信息列表）</returns>
    Task<(int success, int fail, List<string> errors)> ImportCustomerComplaintHandlingAsync(Stream fileStream, string? sheetName);

    /// <summary>
    /// 导出客诉处理记录表(CustomerComplaintHandling)
    /// </summary>
    /// <param name="query">客诉处理记录表(CustomerComplaintHandling)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
    /// <returns>Excel文件信息（文件名和内容）</returns>
    Task<(string fileName, byte[] content)> ExportCustomerComplaintHandlingAsync(TaktCustomerComplaintHandlingQueryDto query, string? sheetName, string? fileName);
}

