...
// 功能描述：出货检验单变更日志表应用服务，提供FqcOrderChangeLog管理的业务逻辑
//
...
{
    private readonly ITaktRepository<TaktFqcOrderChangeLog> _repository;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="repository">FqcOrderChangeLog仓储</param>
    /// <param name="userContext">用户上下文（可选）</param>
    /// <param name="tenantContext">租户上下文（可选）</param>
    /// <param name="localizer">本地化器（可选）</param>
    public TaktFqcOrderChangeLogService(
        ITaktRepository<TaktFqcOrderChangeLog> repository,
    ...
    /// <returns>出货检验单变更日志表(FqcOrderChangeLog)DTO</returns>
    public async Task<TaktFqcOrderChangeLogDto?> GetFqcOrderChangeLogByIdAsync(long id)
    {
        var entity = await _repository.GetByIdAsync(id);
        return entity == null ? null : entity.Adapt<TaktFqcOrderChangeLogDto>();
    }


    /// <summary>
    /// 获取出货检验单变更日志表(FqcOrderChangeLog)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>出货检验单变更日志表(FqcOrderChangeLog)选项列表</returns>
    ...
    /// </summary>
    /// <param name="dto">创建出货检验单变更日志表(FqcOrderChangeLog)DTO</param>
    /// <returns>出货检验单变更日志表(FqcOrderChangeLog)DTO</returns>
    ...
    /// </summary>
    /// <param name="id">出货检验单变更日志表(FqcOrderChangeLog)ID</param>
    /// <param name="dto">更新出货检验单变更日志表(FqcOrderChangeLog)DTO</param>
    /// <returns>出货检验单变更日志表(FqcOrderChangeLog)DTO</returns>
    public async Task<TaktFqcOrderChangeLogDto> UpdateFqcOrderChangeLogAsync(long id, TaktFqcOrderChangeLogUpdateDto dto)
    {
        var entity = await _repository.GetByIdAsync(id);
        if (entity == null)
            throw new TaktBusinessException("validation.fqcorderchangelogNotFound");

        dto.Adapt(entity, typeof(TaktFqcOrderChangeLogUpdateDto), typeof(TaktFqcOrderChangeLog));
        entity.UpdatedAt = DateTime.Now;
        await _repository.UpdateAsync(entity);

        return (await GetFqcOrderChangeLogByIdAsync(id)) ?? entity.Adapt<TaktFqcOrderChangeLogDto>();
    }
    ...
    public async Task DeleteFqcOrderChangeLogBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktFqcOrderChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> GetFqcOrderChangeLogTemplateAsync(string? sheetName, string? fileName)
    {
        var (excelSheet, excelFile) = await ResolveExcelImportTemplateNamesAsync(sheetName, fileName, nameof(TaktFqcOrderChangeLog));
        return await TaktExcelHelper.GenerateTemplateAsync<TaktFqcOrderChangeLogTemplateDto>(
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportFqcOrderChangeLogAsync(Stream fileStream, string? sheetName)
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktFqcOrderChangeLog));
        var importData = await TaktExcelHelper.ImportAsync<TaktFqcOrderChangeLogImportDto>(fileStream, excelSheet);
        ...
        foreach (var item in importData)
        {
            ...
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktFqcOrderChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> ExportFqcOrderChangeLogAsync(TaktFqcOrderChangeLogQueryDto query, string? sheetName, string? fileName)
    {
        var predicate = QueryExpression(query ?? new TaktFqcOrderChangeLogQueryDto());
        List<TaktFqcOrderChangeLog> list;
        if (predicate != null)
            list = await _repository.FindAsync(predicate);
        else
            list = await _repository.GetAllAsync();

        var (excelSheet, excelFile) = await ResolveExcelExportNamesAsync(sheetName, fileName, nameof(TaktFqcOrderChangeLog));
        ...
        var exportData = list.Select(x => x.Adapt<TaktFqcOrderChangeLogExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建出货检验单变更日志表查询表达式
    /// </summary>
    /// <param name="queryDto">出货检验单变更日志表查询DTO</param>
