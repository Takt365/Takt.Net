...
// 功能描述：进货检验单变更日志表应用服务，提供IqcOrderChangeLog管理的业务逻辑
//
...
{
    private readonly ITaktRepository<TaktIqcOrderChangeLog> _repository;
    private readonly ITaktUniqueValidator _uniqueValidator;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="repository">IqcOrderChangeLog仓储</param>
    /// <param name="uniqueValidator">唯一性验证器</param>
    /// <param name="userContext">用户上下文（可选）</param>
    /// <param name="tenantContext">租户上下文（可选）</param>
    /// <param name="localizer">本地化器（可选）</param>
    public TaktIqcOrderChangeLogService(
        ITaktRepository<TaktIqcOrderChangeLog> repository,
    ...
    /// </summary>
    /// <param name="id">进货检验单变更日志表(IqcOrderChangeLog)ID</param>
    /// <returns>进货检验单变更日志表(IqcOrderChangeLog)DTO</returns>
    public async Task<TaktIqcOrderChangeLogDto?> GetIqcOrderChangeLogByIdAsync(long id)
    {
        var entity = await _repository.GetByIdAsync(id);
        return entity == null ? null : entity.Adapt<TaktIqcOrderChangeLogDto>();
    }


    /// <summary>
    /// 获取进货检验单变更日志表(IqcOrderChangeLog)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>进货检验单变更日志表(IqcOrderChangeLog)选项列表</returns>
    ...
    /// </summary>
    /// <param name="dto">创建进货检验单变更日志表(IqcOrderChangeLog)DTO</param>
    /// <returns>进货检验单变更日志表(IqcOrderChangeLog)DTO</returns>
    ...
    /// </summary>
    /// <param name="id">进货检验单变更日志表(IqcOrderChangeLog)ID</param>
    /// <param name="dto">更新进货检验单变更日志表(IqcOrderChangeLog)DTO</param>
    /// <returns>进货检验单变更日志表(IqcOrderChangeLog)DTO</returns>
    public async Task<TaktIqcOrderChangeLogDto> UpdateIqcOrderChangeLogAsync(long id, TaktIqcOrderChangeLogUpdateDto dto)
    {
        var entity = await _repository.GetByIdAsync(id);
        if (entity == null)
            throw new TaktBusinessException("validation.iqcorderchangelogNotFound");
        dto.Adapt(entity, typeof(TaktIqcOrderChangeLogUpdateDto), typeof(TaktIqcOrderChangeLog));
        entity.UpdatedAt = DateTime.Now;
        await _repository.UpdateAsync(entity);

        return (await GetIqcOrderChangeLogByIdAsync(id)) ?? entity.Adapt<TaktIqcOrderChangeLogDto>();
    }
    ...
    /// </summary>
    /// <param name="ids">进货检验单变更日志表(IqcOrderChangeLog)ID列表</param>
    /// <returns>任务</returns>
    public async Task DeleteIqcOrderChangeLogBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktIqcOrderChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> GetIqcOrderChangeLogTemplateAsync(string? sheetName, string? fileName)
    {
        var (excelSheet, excelFile) = await ResolveExcelImportTemplateNamesAsync(sheetName, fileName, nameof(TaktIqcOrderChangeLog));
        return await TaktExcelHelper.GenerateTemplateAsync<TaktIqcOrderChangeLogTemplateDto>(
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportIqcOrderChangeLogAsync(Stream fileStream, string? sheetName)
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktIqcOrderChangeLog));
        var importData = await TaktExcelHelper.ImportAsync<TaktIqcOrderChangeLogImportDto>(fileStream, excelSheet);
        ...
        foreach (var item in importData)
        {
            ...
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktIqcOrderChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> ExportIqcOrderChangeLogAsync(TaktIqcOrderChangeLogQueryDto query, string? sheetName, string? fileName)
    {
        var predicate = QueryExpression(query ?? new TaktIqcOrderChangeLogQueryDto());
        List<TaktIqcOrderChangeLog> list;
        if (predicate != null)
            list = await _repository.FindAsync(predicate);
        else
            list = await _repository.GetAllAsync();

        var (excelSheet, excelFile) = await ResolveExcelExportNamesAsync(sheetName, fileName, nameof(TaktIqcOrderChangeLog));
        ...
        var exportData = list.Select(x => x.Adapt<TaktIqcOrderChangeLogExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建进货检验单变更日志表查询表达式
    /// </summary>
    /// <param name="queryDto">进货检验单变更日志表查询DTO</param>
