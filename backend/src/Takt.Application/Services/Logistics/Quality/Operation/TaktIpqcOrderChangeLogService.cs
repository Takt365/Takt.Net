...
// 功能描述：制程检验单变更日志表应用服务，提供IpqcOrderChangeLog管理的业务逻辑
//
...
{
    private readonly ITaktRepository<TaktIpqcOrderChangeLog> _repository;
    private readonly ITaktUniqueValidator _uniqueValidator;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="repository">IpqcOrderChangeLog仓储</param>
    /// <param name="uniqueValidator">唯一性验证器</param>
    /// <param name="userContext">用户上下文（可选）</param>
    /// <param name="tenantContext">租户上下文（可选）</param>
    /// <param name="localizer">本地化器（可选）</param>
    public TaktIpqcOrderChangeLogService(
        ITaktRepository<TaktIpqcOrderChangeLog> repository,
    ...
    /// <returns>制程检验单变更日志表(IpqcOrderChangeLog)DTO</returns>
    public async Task<TaktIpqcOrderChangeLogDto?> GetIpqcOrderChangeLogByIdAsync(long id)
    {
        var entity = await _repository.GetByIdAsync(id);
        return entity == null ? null : entity.Adapt<TaktIpqcOrderChangeLogDto>();
    }


    /// <summary>
    /// 获取制程检验单变更日志表(IpqcOrderChangeLog)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>制程检验单变更日志表(IpqcOrderChangeLog)选项列表</returns>
    ...
    /// </summary>
    /// <param name="dto">创建制程检验单变更日志表(IpqcOrderChangeLog)DTO</param>
    /// <returns>制程检验单变更日志表(IpqcOrderChangeLog)DTO</returns>
    ...
    /// </summary>
    /// <param name="id">制程检验单变更日志表(IpqcOrderChangeLog)ID</param>
    /// <param name="dto">更新制程检验单变更日志表(IpqcOrderChangeLog)DTO</param>
    /// <returns>制程检验单变更日志表(IpqcOrderChangeLog)DTO</returns>
    public async Task<TaktIpqcOrderChangeLogDto> UpdateIpqcOrderChangeLogAsync(long id, TaktIpqcOrderChangeLogUpdateDto dto)
    {
        var entity = await _repository.GetByIdAsync(id);
        if (entity == null)
            throw new TaktBusinessException("validation.ipqcorderchangelogNotFound");
        dto.Adapt(entity, typeof(TaktIpqcOrderChangeLogUpdateDto), typeof(TaktIpqcOrderChangeLog));
        entity.UpdatedAt = DateTime.Now;
        await _repository.UpdateAsync(entity);

        return (await GetIpqcOrderChangeLogByIdAsync(id)) ?? entity.Adapt<TaktIpqcOrderChangeLogDto>();
    }
    ...
    public async Task DeleteIpqcOrderChangeLogBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktIpqcOrderChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> GetIpqcOrderChangeLogTemplateAsync(string? sheetName, string? fileName)
    {
        var (excelSheet, excelFile) = await ResolveExcelImportTemplateNamesAsync(sheetName, fileName, nameof(TaktIpqcOrderChangeLog));
        return await TaktExcelHelper.GenerateTemplateAsync<TaktIpqcOrderChangeLogTemplateDto>(
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportIpqcOrderChangeLogAsync(Stream fileStream, string? sheetName)
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktIpqcOrderChangeLog));
        var importData = await TaktExcelHelper.ImportAsync<TaktIpqcOrderChangeLogImportDto>(fileStream, excelSheet);
        ...
        foreach (var item in importData)
        {
            ...
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktIpqcOrderChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> ExportIpqcOrderChangeLogAsync(TaktIpqcOrderChangeLogQueryDto query, string? sheetName, string? fileName)
    {
        var predicate = QueryExpression(query ?? new TaktIpqcOrderChangeLogQueryDto());
        List<TaktIpqcOrderChangeLog> list;
        if (predicate != null)
            list = await _repository.FindAsync(predicate);
        else
            list = await _repository.GetAllAsync();

        var (excelSheet, excelFile) = await ResolveExcelExportNamesAsync(sheetName, fileName, nameof(TaktIpqcOrderChangeLog));
        ...
        var exportData = list.Select(x => x.Adapt<TaktIpqcOrderChangeLogExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建制程检验单变更日志表查询表达式
    /// </summary>
    /// <param name="queryDto">制程检验单变更日志表查询DTO</param>
