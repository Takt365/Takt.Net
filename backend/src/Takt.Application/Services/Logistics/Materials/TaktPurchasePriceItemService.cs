...
public class TaktPurchasePriceItemService : TaktServiceBase, ITaktPurchasePriceItemService
{
    ...
    public async Task<TaktPurchasePriceItemDto> CreatePurchasePriceItemAsync(TaktPurchasePriceItemCreateDto dto)
    {
        ...
        // 验证PurchasePriceId、LineNumber、MaterialCode组合的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.PurchasePriceId == dto.PurchasePriceId && x.LineNumber == dto.LineNumber && x.MaterialCode == dto.MaterialCode);
        if (!isUnique)
            throw new TaktBusinessException($"采购价格明细表PurchasePriceId、LineNumber、MaterialCode组合已存在");
    ...
    public async Task<TaktPurchasePriceItemDto> UpdatePurchasePriceItemAsync(long id, TaktPurchasePriceItemUpdateDto dto)
    {
        ...
        // 验证PurchasePriceId、LineNumber、MaterialCode组合的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.PurchasePriceId == dto.PurchasePriceId && x.LineNumber == dto.LineNumber && x.MaterialCode == dto.MaterialCode, id);
        if (!isUnique)
            throw new TaktBusinessException($"采购价格明细表PurchasePriceId、LineNumber、MaterialCode组合已存在");
