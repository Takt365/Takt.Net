...
public class TaktPurchasePriceService : TaktServiceBase, ITaktPurchasePriceService
{
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportPurchasePriceAsync(Stream fileStream, string? sheetName)
    {
        ...
        try
        {
            var excelSheet = ResolveExcelSheetName(sheetName, nameof(TaktPurchasePrice));
            // 从Excel导入数据
            ...
            foreach (var (item, index) in importData.Select((item, index) => (item, index + 3))) // 第3行开始是数据
            {
                try
                {
                    ...
                    {
                        AddImportError(errors, "validation.importRowPurchasePriceSupplierCodeRequired", index);
                        fail++;
                        continue;
                    }
