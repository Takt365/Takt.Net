...
public class TaktPurchaseOrderService : TaktServiceBase, ITaktPurchaseOrderService
{
    ...
    public async Task<TaktPurchaseOrderDto> CreateAsync(TaktPurchaseOrderCreateDto dto)
    {
        ...
        {
            // TODO: 实现订单编码生成逻辑
            throw new TaktBusinessException("validation.purchaseOrderCodeRequired");
        }
    ...
    public async Task<TaktPurchaseOrderDto> UpdateAsync(long id, TaktPurchaseOrderUpdateDto dto)
    {
        ...
        {
            throw new TaktBusinessException("validation.purchaseOrderApprovedCannotModify");
        }
    ...
    public async Task DeleteAsync(long id)
    {
        ...
        if (order.OrderStatus >= 2)
        {
            throw new TaktBusinessException("validation.purchaseOrderApprovedCannotDelete");
        }
    ...
    public async Task DeleteAsync(IEnumerable<long> ids)
    {
        ...
        if (invalidOrders.Any())
        {
            ...
            throw new TaktLocalizedException("validation.purchaseOrderBatchApprovedCannotDelete", "Frontend", orderCodes);
        }
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportAsync(Stream fileStream, string? sheetName)
    {
        ...
        try
        {
            var excelSheet = ResolveExcelSheetName(sheetName, nameof(TaktPurchaseOrder));
            // 从Excel导入数据
            ...
            foreach (var (item, index) in importData.Select((item, index) => (item, index + 3))) // 第3行开始是数据
            {
                try
                {
                    ...
                    {
                        AddImportError(errors, "validation.importRowPurchaseOrderCodeRequired", index);
                        fail++;
                        continue;
                    }
                    ...
                    {
                        AddImportError(errors, "validation.importRowPurchaseSupplierCodeRequired", index);
                        fail++;
                        continue;
                    }
                    ...
                    {
                        AddImportError(errors, "validation.importRowPurchaseSupplierNameRequired", index);
                        fail++;
                        continue;
                    }

                    // 导入时使用验证器手动验证（OrderCode唯一）
                    await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_orderRepository, o => o.OrderCode, item.OrderCode, null, null, GetLocalizedString("validation.importRowPurchaseOrderCodeExists", "Frontend", index, item.OrderCode));
