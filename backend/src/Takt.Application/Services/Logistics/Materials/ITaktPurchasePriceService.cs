...
// 创建时间：2026-05-10
// 创建人：Takt365
// 功能描述：采购价格表应用服务接口（主子表），定义PurchasePrice管理的业务操作
// 
...
public interface ITaktPurchasePriceService
{
    ...
    /// <summary>
    /// 批量删除采购价格表(PurchasePrice)（级联删除子表）
    /// </summary>
    ...
    /// <summary>
    /// 更新采购价格表(PurchasePrice)PriceStatus
    /// </summary>
    /// <param name="dto">采购价格表(PurchasePrice)PriceStatusDTO</param>
    /// <returns>采购价格表(PurchasePrice)DTO</returns>
    Task<TaktPurchasePriceDto> UpdatePurchasePricePriceStatusAsync(TaktPurchasePricePriceStatusDto dto);
    ...
    /// </summary>
    /// <param name="query">采购价格表(PurchasePrice)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
