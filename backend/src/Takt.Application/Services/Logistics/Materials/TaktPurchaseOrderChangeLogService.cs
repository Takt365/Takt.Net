...
public class TaktPurchaseOrderChangeLogService : TaktServiceBase, ITaktPurchaseOrderChangeLogService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.OrderCode, dto.OrderCode, null, $"采购订单变更记录表编码 {dto.OrderCode} 已存在");
    ...
    public async Task<TaktPurchaseOrderChangeLogDto> UpdatePurchaseOrderChangeLogAsync(long id, TaktPurchaseOrderChangeLogUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.OrderCode, dto.OrderCode, id, $"采购订单变更记录表编码 {dto.OrderCode} 已存在");
