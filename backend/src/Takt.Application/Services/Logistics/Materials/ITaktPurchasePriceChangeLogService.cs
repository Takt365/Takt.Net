...
// 创建时间：2026-05-10
// 创建人：Takt365
// 功能描述：采购价格变更记录表应用服务接口，定义PurchasePriceChangeLog管理的业务操作
// 
...
public interface ITaktPurchasePriceChangeLogService
{
    ...
    /// 根据ID获取采购价格变更记录表
    /// </summary>
    /// <param name="id">采购价格变更记录表ID</param>
    /// <returns>采购价格变更记录表DTO</returns>
    Task<TaktPurchasePriceChangeLogDto?> GetPurchasePriceChangeLogByIdAsync(long id);

    /// <summary>
    /// 获取采购价格变更记录表选项列表（用于下拉框等）
    /// </summary>
    /// <returns>采购价格变更记录表选项列表</returns>
    Task<List<TaktSelectOption>> GetPurchasePriceChangeLogOptionsAsync();

    /// <summary>
    /// 创建采购价格变更记录表
    /// </summary>
    /// <param name="dto">创建采购价格变更记录表DTO</param>
    /// <returns>采购价格变更记录表DTO</returns>
    ...
    /// </summary>
    /// <param name="id">采购价格变更记录表ID</param>
    /// <param name="dto">更新采购价格变更记录表DTO</param>
    /// <returns>采购价格变更记录表DTO</returns>
    ...
    Task<(string fileName, byte[] content)> GetPurchasePriceChangeLogTemplateAsync(string? sheetName, string? fileName);
    ...
    Task<(int success, int fail, List<string> errors)> ImportPurchasePriceChangeLogAsync(Stream fileStream, string? sheetName);

    /// <summary>
    /// 导出采购价格变更记录表(PurchasePriceChangeLog)
    /// </summary>
    /// <param name="query">采购价格变更记录表(PurchasePriceChangeLog)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
    /// <returns>Excel文件信息（文件名和内容）</returns>
    Task<(string fileName, byte[] content)> ExportPurchasePriceChangeLogAsync(TaktPurchasePriceChangeLogQueryDto query, string? sheetName, string? fileName);
}
