...
// 创建时间：2026-05-11
// 创建人：Takt365
// 功能描述：产品序列号入库表应用服务接口（主子表），定义ProductSerialInbound管理的业务操作
// 
...
public interface ITaktProductSerialInboundService
{
    ...
    /// <summary>
    /// 根据ID获取产品序列号入库表（包含子表数据）
    /// </summary>
    /// <param name="id">产品序列号入库表ID</param>
    /// <returns>产品序列号入库表DTO</returns>
    Task<TaktProductSerialInboundDto?> GetProductSerialInboundByIdAsync(long id);

    /// <summary>
    /// 获取产品序列号入库表选项列表（用于下拉框等）
    /// </summary>
    /// <returns>产品序列号入库表选项列表</returns>
    Task<List<TaktSelectOption>> GetProductSerialInboundOptionsAsync();

    /// <summary>
    /// 创建产品序列号入库表（包含子表数据）
    /// </summary>
    /// <param name="dto">创建产品序列号入库表DTO</param>
    /// <returns>产品序列号入库表DTO</returns>
    ...
    /// <summary>
    /// 批量删除产品序列号入库表(ProductSerialInbound)（级联删除子表）
    /// </summary>
    ...
    Task<(string fileName, byte[] content)> GetProductSerialInboundTemplateAsync(string? sheetName, string? fileName);
    ...
    Task<(int success, int fail, List<string> errors)> ImportProductSerialInboundAsync(Stream fileStream, string? sheetName);

    /// <summary>
    /// 导出产品序列号入库表(ProductSerialInbound)
    /// </summary>
    /// <param name="query">产品序列号入库表(ProductSerialInbound)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
    /// <returns>Excel文件信息（文件名和内容）</returns>
    Task<(string fileName, byte[] content)> ExportProductSerialInboundAsync(TaktProductSerialInboundQueryDto query, string? sheetName, string? fileName);
}
