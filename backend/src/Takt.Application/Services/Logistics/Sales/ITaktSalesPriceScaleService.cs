...
public interface ITaktSalesPriceScaleService
{
    ...
    /// <summary>
    /// 根据ID获取销售价格阶梯表（包含子表数据）
    /// </summary>
    /// <param name="id">销售价格阶梯表ID</param>
    /// <returns>销售价格阶梯表DTO</returns>
    Task<TaktSalesPriceScaleDto?> GetSalesPriceScaleByIdAsync(long id);

    /// <summary>
    /// 获取销售价格阶梯表选项列表（用于下拉框等）
    /// </summary>
    /// <returns>销售价格阶梯表选项列表</returns>
    Task<List<TaktSelectOption>> GetSalesPriceScaleOptionsAsync();

    /// <summary>
    /// 创建销售价格阶梯表（包含子表数据）
    /// </summary>
    /// <param name="dto">创建销售价格阶梯表DTO</param>
    /// <returns>销售价格阶梯表DTO</returns>
    ...
    /// <summary>
    /// 批量删除销售价格阶梯表(SalesPriceScale)（级联删除子表）
    /// </summary>
    ...
    Task<(string fileName, byte[] content)> GetSalesPriceScaleTemplateAsync(string? sheetName, string? fileName);
    ...
    Task<(int success, int fail, List<string> errors)> ImportSalesPriceScaleAsync(Stream fileStream, string? sheetName);

    /// <summary>
    /// 导出销售价格阶梯表(SalesPriceScale)
    /// </summary>
    /// <param name="query">销售价格阶梯表(SalesPriceScale)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
    /// <returns>Excel文件信息（文件名和内容）</returns>
    Task<(string fileName, byte[] content)> ExportSalesPriceScaleAsync(TaktSalesPriceScaleQueryDto query, string? sheetName, string? fileName);
}
