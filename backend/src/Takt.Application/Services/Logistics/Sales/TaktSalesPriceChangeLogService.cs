...
// 功能描述：销售价格变更记录表应用服务，提供SalesPriceChangeLog管理的业务逻辑
//
...
{
    private readonly ITaktRepository<TaktSalesPriceChangeLog> _repository;

    /// <summary>
    /// 构造函数
    /// </summary>
    /// <param name="repository">SalesPriceChangeLog仓储</param>
    /// <param name="userContext">用户上下文（可选）</param>
    /// <param name="tenantContext">租户上下文（可选）</param>
    /// <param name="localizer">本地化器（可选）</param>
    public TaktSalesPriceChangeLogService(
        ITaktRepository<TaktSalesPriceChangeLog> repository,
    ...
    /// <returns>销售价格变更记录表(SalesPriceChangeLog)DTO</returns>
    public async Task<TaktSalesPriceChangeLogDto?> GetSalesPriceChangeLogByIdAsync(long id)
    {
        var entity = await _repository.GetByIdAsync(id);
        return entity == null ? null : entity.Adapt<TaktSalesPriceChangeLogDto>();
    }


    /// <summary>
    /// 获取销售价格变更记录表(SalesPriceChangeLog)选项列表（用于下拉框等）
    /// </summary>
    /// <returns>销售价格变更记录表(SalesPriceChangeLog)选项列表</returns>
    ...
    /// </summary>
    /// <param name="dto">创建销售价格变更记录表(SalesPriceChangeLog)DTO</param>
    /// <returns>销售价格变更记录表(SalesPriceChangeLog)DTO</returns>
    ...
    /// </summary>
    /// <param name="id">销售价格变更记录表(SalesPriceChangeLog)ID</param>
    /// <param name="dto">更新销售价格变更记录表(SalesPriceChangeLog)DTO</param>
    /// <returns>销售价格变更记录表(SalesPriceChangeLog)DTO</returns>
    public async Task<TaktSalesPriceChangeLogDto> UpdateSalesPriceChangeLogAsync(long id, TaktSalesPriceChangeLogUpdateDto dto)
    {
        var entity = await _repository.GetByIdAsync(id);
        if (entity == null)
            throw new TaktBusinessException("validation.salespricechangelogNotFound");

        dto.Adapt(entity, typeof(TaktSalesPriceChangeLogUpdateDto), typeof(TaktSalesPriceChangeLog));
        entity.UpdatedAt = DateTime.Now;
        await _repository.UpdateAsync(entity);

        return (await GetSalesPriceChangeLogByIdAsync(id)) ?? entity.Adapt<TaktSalesPriceChangeLogDto>();
    }
    ...
    public async Task DeleteSalesPriceChangeLogBatchAsync(IEnumerable<long> ids)
    {
        ...
        // 获取所有要删除的实体
        var entities = new List<TaktSalesPriceChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> GetSalesPriceChangeLogTemplateAsync(string? sheetName, string? fileName)
    {
        var (excelSheet, excelFile) = await ResolveExcelImportTemplateNamesAsync(sheetName, fileName, nameof(TaktSalesPriceChangeLog));
        return await TaktExcelHelper.GenerateTemplateAsync<TaktSalesPriceChangeLogTemplateDto>(
    ...
    public async Task<(int success, int fail, List<string> errors)> ImportSalesPriceChangeLogAsync(Stream fileStream, string? sheetName)
    {
        var (excelSheet, _) = await ResolveExcelImportTemplateNamesAsync(sheetName, null, nameof(TaktSalesPriceChangeLog));
        var importData = await TaktExcelHelper.ImportAsync<TaktSalesPriceChangeLogImportDto>(fileStream, excelSheet);
        ...
        foreach (var item in importData)
        {
            ...
            {
                // TODO: 添加必要的验证逻辑
                var entity = item.Adapt<TaktSalesPriceChangeLog>();
    ...
    public async Task<(string fileName, byte[] content)> ExportSalesPriceChangeLogAsync(TaktSalesPriceChangeLogQueryDto query, string? sheetName, string? fileName)
    {
        var predicate = QueryExpression(query ?? new TaktSalesPriceChangeLogQueryDto());
        List<TaktSalesPriceChangeLog> list;
        if (predicate != null)
            list = await _repository.FindAsync(predicate);
        else
            list = await _repository.GetAllAsync();

        var (excelSheet, excelFile) = await ResolveExcelExportNamesAsync(sheetName, fileName, nameof(TaktSalesPriceChangeLog));
        ...
        var exportData = list.Select(x => x.Adapt<TaktSalesPriceChangeLogExportDto>()).ToList();
        return await TaktExcelHelper.ExportAsync(
            exportData,
            excelSheet,
            excelFile
    ...
    /// 构建销售价格变更记录表查询表达式
    /// </summary>
    /// <param name="queryDto">销售价格变更记录表查询DTO</param>
