...
public class TaktSalesOrderItemService : TaktServiceBase, ITaktSalesOrderItemService
{
    ...
    public async Task<TaktSalesOrderItemDto> CreateSalesOrderItemAsync(TaktSalesOrderItemCreateDto dto)
    {
        ...
        // 验证SalesOrderId、LineNumber组合的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.SalesOrderId == dto.SalesOrderId && x.LineNumber == dto.LineNumber);
        if (!isUnique)
            throw new TaktBusinessException($"销售订单明细表SalesOrderId、LineNumber组合已存在");
    ...
    public async Task<TaktSalesOrderItemDto> UpdateSalesOrderItemAsync(long id, TaktSalesOrderItemUpdateDto dto)
    {
        ...
        // 验证SalesOrderId、LineNumber组合的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.SalesOrderId == dto.SalesOrderId && x.LineNumber == dto.LineNumber, id);
        if (!isUnique)
            throw new TaktBusinessException($"销售订单明细表SalesOrderId、LineNumber组合已存在");
