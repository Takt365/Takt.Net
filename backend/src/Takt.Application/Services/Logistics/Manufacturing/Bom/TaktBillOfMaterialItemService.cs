...
public class TaktBillOfMaterialItemService : TaktServiceBase, ITaktBillOfMaterialItemService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.BomCode, dto.BomCode, null, $"物料清单明细表编码 {dto.BomCode} 已存在");
    ...
    public async Task<TaktBillOfMaterialItemDto> UpdateBillOfMaterialItemAsync(long id, TaktBillOfMaterialItemUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.BomCode, dto.BomCode, id, $"物料清单明细表编码 {dto.BomCode} 已存在");
