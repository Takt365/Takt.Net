...
// 创建时间：2026-05-10
// 创建人：Takt365
// 功能描述：组立不良明细表应用服务接口（主子表），定义AssyDefectDetail管理的业务操作
// 
...
public interface ITaktAssyDefectDetailService
{
    ...
    /// <summary>
    /// 根据ID获取组立不良明细表（包含子表数据）
    /// </summary>
    /// <param name="id">组立不良明细表ID</param>
    /// <returns>组立不良明细表DTO</returns>
    Task<TaktAssyDefectDetailDto?> GetAssyDefectDetailByIdAsync(long id);

    /// <summary>
    /// 获取组立不良明细表选项列表（用于下拉框等）
    /// </summary>
    /// <returns>组立不良明细表选项列表</returns>
    Task<List<TaktSelectOption>> GetAssyDefectDetailOptionsAsync();

    /// <summary>
    /// 创建组立不良明细表（包含子表数据）
    /// </summary>
    /// <param name="dto">创建组立不良明细表DTO</param>
    /// <returns>组立不良明细表DTO</returns>
    ...
    /// <summary>
    /// 批量删除组立不良明细表(AssyDefectDetail)（级联删除子表）
    /// </summary>
    ...
    Task<(string fileName, byte[] content)> GetAssyDefectDetailTemplateAsync(string? sheetName, string? fileName);
    ...
    Task<(int success, int fail, List<string> errors)> ImportAssyDefectDetailAsync(Stream fileStream, string? sheetName);

    /// <summary>
    /// 导出组立不良明细表(AssyDefectDetail)
    /// </summary>
    /// <param name="query">组立不良明细表(AssyDefectDetail)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
    /// <returns>Excel文件信息（文件名和内容）</returns>
    Task<(string fileName, byte[] content)> ExportAssyDefectDetailAsync(TaktAssyDefectDetailQueryDto query, string? sheetName, string? fileName);
}
