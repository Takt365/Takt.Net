...
public class TaktPcbaOutputService : TaktServiceBase, ITaktPcbaOutputService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.PlantCode, dto.PlantCode, null, $"PCBA日报表编码 {dto.PlantCode} 已存在");
    ...
    public async Task<TaktPcbaOutputDto> UpdatePcbaOutputAsync(long id, TaktPcbaOutputUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.PlantCode, dto.PlantCode, id, $"PCBA日报表编码 {dto.PlantCode} 已存在");
