...
public class TaktStandardOperationRateService : TaktServiceBase, ITaktStandardOperationRateService
{
    ...
    public async Task<TaktStandardOperationRateDto> CreateStandardOperationRateAsync(TaktStandardOperationRateCreateDto dto)
    {
        ...
        // 验证工厂编码、FinancialYear、OperationType组合的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.PlantCode == dto.PlantCode && x.FinancialYear == dto.FinancialYear && x.OperationType == dto.OperationType);
        if (!isUnique)
            throw new TaktBusinessException($"标准生产稼动率表工厂编码、FinancialYear、OperationType组合已存在");
    ...
    public async Task<TaktStandardOperationRateDto> UpdateStandardOperationRateAsync(long id, TaktStandardOperationRateUpdateDto dto)
    {
        ...
        // 验证工厂编码、FinancialYear、OperationType组合的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.PlantCode == dto.PlantCode && x.FinancialYear == dto.FinancialYear && x.OperationType == dto.OperationType, id);
        if (!isUnique)
            throw new TaktBusinessException($"标准生产稼动率表工厂编码、FinancialYear、OperationType组合已存在");
