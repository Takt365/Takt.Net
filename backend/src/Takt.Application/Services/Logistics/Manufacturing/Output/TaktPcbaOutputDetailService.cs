...
public class TaktPcbaOutputDetailService : TaktServiceBase, ITaktPcbaOutputDetailService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.SerialNo, dto.SerialNo, null, $"PCBA日报明细表编码 {dto.SerialNo} 已存在");
    ...
    public async Task<TaktPcbaOutputDetailDto> UpdatePcbaOutputDetailAsync(long id, TaktPcbaOutputDetailUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.SerialNo, dto.SerialNo, id, $"PCBA日报明细表编码 {dto.SerialNo} 已存在");
