...
public class TaktEcAttachmentService : TaktServiceBase, ITaktEcAttachmentService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.DocNo, dto.DocNo, null, $"设变附件表编码 {dto.DocNo} 已存在");
    ...
    public async Task<TaktEcAttachmentDto> UpdateEcAttachmentAsync(long id, TaktEcAttachmentUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.DocNo, dto.DocNo, id, $"设变附件表编码 {dto.DocNo} 已存在");
