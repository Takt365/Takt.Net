...
public class TaktEcNoticeService : TaktServiceBase, ITaktEcNoticeService
{
    ...
    {
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.NoticeNo, dto.NoticeNo, null, $"工程变更通知单表编码 {dto.NoticeNo} 已存在");
    ...
    public async Task<TaktEcNoticeDto> UpdateEcNoticeAsync(long id, TaktEcNoticeUpdateDto dto)
    {
        ...
        await TaktUniqueValidatorExtensions.ValidateUniqueAsync(_repository, x => x.NoticeNo, dto.NoticeNo, id, $"工程变更通知单表编码 {dto.NoticeNo} 已存在");
