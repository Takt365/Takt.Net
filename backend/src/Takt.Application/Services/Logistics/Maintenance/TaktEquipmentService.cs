...
public class TaktEquipmentService : TaktServiceBase, ITaktEquipmentService
{
    ...
    public async Task<TaktEquipmentDto> CreateEquipmentAsync(TaktEquipmentCreateDto dto)
    {
        ...
        // 验证工厂编码、EquipmentCode、EquipmentName组合的唯一性
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.PlantCode == dto.PlantCode && x.EquipmentCode == dto.EquipmentCode && x.EquipmentName == dto.EquipmentName);
        if (!isUnique)
            throw new TaktBusinessException($"工厂设备表工厂编码、EquipmentCode、EquipmentName组合已存在");
    ...
    public async Task<TaktEquipmentDto> UpdateEquipmentAsync(long id, TaktEquipmentUpdateDto dto)
    {
        ...
        // 验证工厂编码、EquipmentCode、EquipmentName组合的唯一性（排除当前记录）
        var isUnique = await _uniqueValidator.IsUniqueAsync(_repository, x => x.PlantCode == dto.PlantCode && x.EquipmentCode == dto.EquipmentCode && x.EquipmentName == dto.EquipmentName, id);
        if (!isUnique)
            throw new TaktBusinessException($"工厂设备表工厂编码、EquipmentCode、EquipmentName组合已存在");
