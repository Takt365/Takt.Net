...
// 创建时间：2026-05-11
// 创建人：Takt365
// 功能描述：工厂设备表应用服务接口（主子表），定义Equipment管理的业务操作
// 
...
public interface ITaktEquipmentService
{
    ...
    /// 根据ID获取工厂设备表（包含子表数据）
    /// </summary>
    /// <param name="id">工厂设备表ID</param>
    /// <returns>工厂设备表DTO</returns>
    Task<TaktEquipmentDto?> GetEquipmentByIdAsync(long id);

    /// <summary>
    /// 获取工厂设备表选项列表（用于下拉框等）
    /// </summary>
    /// <returns>工厂设备表选项列表</returns>
    Task<List<TaktSelectOption>> GetEquipmentOptionsAsync();

    /// <summary>
    /// 创建工厂设备表（包含子表数据）
    /// </summary>
    /// <param name="dto">创建工厂设备表DTO</param>
    /// <returns>工厂设备表DTO</returns>
    ...
    /// <summary>
    /// 更新工厂设备表(Equipment)WarrantyStatus
    /// </summary>
    /// <param name="dto">工厂设备表(Equipment)WarrantyStatusDTO</param>
    /// <returns>工厂设备表(Equipment)DTO</returns>
    Task<TaktEquipmentDto> UpdateEquipmentWarrantyStatusAsync(TaktEquipmentWarrantyStatusDto dto);

    /// <summary>
    /// 更新工厂设备表(Equipment)Status
    /// </summary>
    /// <param name="dto">工厂设备表(Equipment)StatusDTO</param>
    /// <returns>工厂设备表(Equipment)DTO</returns>
    ...
    /// 导出工厂设备表(Equipment)
    /// </summary>
    /// <param name="query">工厂设备表(Equipment)查询DTO（包含查询条件）</param>
    /// <param name="sheetName">工作表名称</param>
    /// <param name="fileName">文件名</param>
