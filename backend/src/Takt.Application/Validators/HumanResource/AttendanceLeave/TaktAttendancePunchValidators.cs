...
public class TaktAttendancePunchCreateDtoValidator : AbstractValidator<TaktAttendancePunchCreateDto>
{
    ...
    public TaktAttendancePunchCreateDtoValidator(ITaktValidationMessages validationMessages)
    {
        _validationMessages = validationMessages;
        RuleFor(x => x.PunchType)
            .InclusiveBetween(1, 1)
            .WithMessage(_validationMessages.FormatInvalid("entity.attendancepunch.punchtype"));

        RuleFor(x => x.PunchSource)
            .InclusiveBetween(0, 0)
            .WithMessage(_validationMessages.FormatInvalid("entity.attendancepunch.punchsource"));

        RuleFor(x => x.PunchAddress)
            .MaximumLength(256).WithMessage(_validationMessages.LengthMax("entity.attendancepunch.punchaddress", 256))
...
/// </summary>
/// <remarks>
/// 继承 <see cref="TaktAttendancePunchCreateDtoValidator"/> 的所有验证规则，并额外验证：
/// <list type="bullet">
///   <item>AttendancePunchId 必须大于 0</item>
/// </list>
/// </remarks>
public class TaktAttendancePunchUpdateDtoValidator : AbstractValidator<TaktAttendancePunchUpdateDto>
{
    ...
    /// </summary>
    /// <param name="validationMessages">验证消息格式化器</param>
    public TaktAttendancePunchUpdateDtoValidator(ITaktValidationMessages validationMessages)
    {
        _validationMessages = validationMessages;
        Include(new TaktAttendancePunchCreateDtoValidator(validationMessages));

        RuleFor(x => x.AttendancePunchId)
            .GreaterThan(0)
            .WithMessage(_validationMessages.Required("entity.attendancepunch.attendancepunchid"));
