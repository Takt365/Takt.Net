...
public class TaktTicketEvaluationCreateDtoValidator : AbstractValidator<TaktTicketEvaluationCreateDto>
{
    ...
    public TaktTicketEvaluationCreateDtoValidator(ITaktValidationMessages validationMessages)
    {
        _validationMessages = validationMessages;
        RuleFor(x => x.Comment)
            .MaximumLength(1000).WithMessage(_validationMessages.LengthMax("entity.ticketevaluation.comment", 1000))
            .When(x => !string.IsNullOrWhiteSpace(x.Comment));

        RuleFor(x => x.EvaluatorName)
            .MaximumLength(50).WithMessage(_validationMessages.LengthMax("entity.ticketevaluation.evaluatorname", 50))
...
/// </summary>
/// <remarks>
/// 继承 <see cref="TaktTicketEvaluationCreateDtoValidator"/> 的所有验证规则，并额外验证：
/// <list type="bullet">
///   <item>TicketEvaluationId 必须大于 0</item>
/// </list>
/// </remarks>
public class TaktTicketEvaluationUpdateDtoValidator : AbstractValidator<TaktTicketEvaluationUpdateDto>
{
    ...
    /// </summary>
    /// <param name="validationMessages">验证消息格式化器</param>
    public TaktTicketEvaluationUpdateDtoValidator(ITaktValidationMessages validationMessages)
    {
        _validationMessages = validationMessages;
        Include(new TaktTicketEvaluationCreateDtoValidator(validationMessages));

        RuleFor(x => x.TicketEvaluationId)
            .GreaterThan(0)
            .WithMessage(_validationMessages.Required("entity.ticketevaluation.ticketevaluationid"));
