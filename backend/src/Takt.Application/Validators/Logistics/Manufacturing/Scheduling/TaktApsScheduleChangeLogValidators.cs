...
public class TaktApsScheduleChangeLogCreateDtoValidator : AbstractValidator<TaktApsScheduleChangeLogCreateDto>
{
    ...
    public TaktApsScheduleChangeLogCreateDtoValidator(ITaktValidationMessages validationMessages)
    {
        _validationMessages = validationMessages;
        RuleFor(x => x.ChangeFields)
            .MaximumLength(4000).WithMessage(_validationMessages.LengthMax("entity.apsschedulechangelog.changefields", 4000))
            .When(x => !string.IsNullOrWhiteSpace(x.ChangeFields));

        RuleFor(x => x.ChangeType)
            .InclusiveBetween(0, 4)
            .WithMessage(_validationMessages.FormatInvalid("entity.apsschedulechangelog.changetype"));

        RuleFor(x => x.ChangeReason)
            .MaximumLength(1000).WithMessage(_validationMessages.LengthMax("entity.apsschedulechangelog.changereason", 1000))
            .When(x => !string.IsNullOrWhiteSpace(x.ChangeReason));

        RuleFor(x => x.ChangeBy)
            .MaximumLength(50).WithMessage(_validationMessages.LengthMax("entity.apsschedulechangelog.changeby", 50))
...
/// </summary>
/// <remarks>
/// 继承 <see cref="TaktApsScheduleChangeLogCreateDtoValidator"/> 的所有验证规则，并额外验证：
/// <list type="bullet">
///   <item>ApsScheduleChangeLogId 必须大于 0</item>
/// </list>
/// </remarks>
public class TaktApsScheduleChangeLogUpdateDtoValidator : AbstractValidator<TaktApsScheduleChangeLogUpdateDto>
{
    ...
    /// </summary>
    /// <param name="validationMessages">验证消息格式化器</param>
    public TaktApsScheduleChangeLogUpdateDtoValidator(ITaktValidationMessages validationMessages)
    {
        _validationMessages = validationMessages;
        Include(new TaktApsScheduleChangeLogCreateDtoValidator(validationMessages));

        RuleFor(x => x.ApsScheduleChangeLogId)
            .GreaterThan(0)
            .WithMessage(_validationMessages.Required("entity.apsschedulechangelog.apsschedulechangelogid"));
