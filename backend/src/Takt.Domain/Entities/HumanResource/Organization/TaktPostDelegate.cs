...
// 文件名称：TaktPostDelegate.cs
// 功能描述：岗位维度的代理配置（岗位 → 多条代理规则）；delegate_mode 取值见字典 hr_delegate_mode。
//
...
/// 岗位代理（一对多：岗位 → 多条代理规则）。
/// </summary>
[SugarTable("takt_humanresource_organization_post_delegate", "岗位代理表")]
[SugarIndex("ix_takt_humanresource_organization_post_delegate_post_id", nameof(PostId), OrderByType.Asc)]
[SugarIndex("ix_takt_humanresource_organization_post_delegate_mode", nameof(DelegateMode), OrderByType.Asc)]
[SugarIndex("ix_takt_humanresource_organization_post_delegate_config_id", nameof(ConfigId), OrderByType.Asc)]
[SugarIndex("ix_takt_humanresource_organization_post_delegate_is_deleted", nameof(IsDeleted), OrderByType.Asc)]
public class TaktPostDelegate : TaktEntityBase
{
    /// <summary>
    /// 所属岗位 Id（外键：<see cref="TaktPost"/>）
    /// </summary>
