...
public class TaktHoliday : TaktEntityBase
{
    ...
    /// 假日问候语（简短，用于问候语行）
    /// </summary>
    [SugarColumn(ColumnName = "holiday_greeting", ColumnDescription = "假日问候语", ColumnDataType = "nvarchar", Length = 200, IsNullable = true)]
    public string? HolidayGreeting { get; set; }

    /// <summary>
    /// 假日引用/诗句（用于引用区展示）
    /// </summary>
    [SugarColumn(ColumnName = "holiday_quote", ColumnDescription = "假日引用", ColumnDataType = "nvarchar", Length = 500, IsNullable = true)]
