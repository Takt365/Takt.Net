...
public class TaktLanguage : TaktEntityBase
{
    ...
    /// 语言状态（1=启用，0=禁用）
    /// </summary>
    [SugarColumn(ColumnName = "language_status", ColumnDescription = "语言状态", ColumnDataType = "int", IsNullable = false, DefaultValue = "1")]
    public int LanguageStatus { get; set; } = 1;

    /// <summary>
    /// 翻译列表（主子表关系，一个语言可以有多个翻译）
    /// </summary>
    [Navigate(NavigateType.OneToMany, nameof(TaktTranslation.LanguageId))]
    public List<TaktTranslation>? Translations { get; set; }
}
