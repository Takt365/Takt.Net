...
public class TaktMail : TaktEntityBase
{
    ...
    /// 邮件附件列表（主子表关系）
    /// </summary>
    [Navigate(NavigateType.OneToMany, nameof(TaktMailAttachment.MailId))]
    public List<TaktMailAttachment>? Attachments { get; set; }

    /// <summary>
    /// 邮件收件人列表（主子表关系）
    /// </summary>
    [Navigate(NavigateType.OneToMany, nameof(TaktMailRecipient.MailId))]
    public List<TaktMailRecipient>? Recipients { get; set; }
}
