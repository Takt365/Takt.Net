...
// 文件名称：TaktAccountingTitleChangeLog.cs
// 功能描述：会计科目变更记录实体（变更记录表，非历史表）
//
...
/// 会计科目变更记录实体
/// </summary>
[SugarTable("takt_accounting_financial_accounting_title_change_log", "会计科目变更记录表")]
[SugarIndex("ix_takt_accounting_financial_accounting_title_change_log_accounting_title_id", nameof(AccountingTitleId), OrderByType.Asc)]
[SugarIndex("ix_takt_accounting_financial_accounting_title_change_log_change_time", nameof(ChangeTime), OrderByType.Desc)]
[SugarIndex("ix_takt_accounting_financial_accounting_title_change_log_is_deleted", nameof(IsDeleted), OrderByType.Asc)]
public class TaktAccountingTitleChangeLog : TaktEntityBase
{
    /// <summary>
    /// 会计科目ID（主子表关系，序列化为string以避免Javascript精度问题）
    /// </summary>
    [SugarColumn(ColumnName = "accounting_title_id", ColumnDescription = "会计科目ID", ColumnDataType = "bigint", IsNullable = false)]
