...
public interface ITaktDatabaseSchemaProvider
{
    ...
    /// <summary>
    /// 根据选中的数据库（ConfigId）获取该库下的所有数据表列表（用于代码生成器导入表）。
    /// 返回完整列表，并标记每个表的状态（是否已有控制器、是否已导入）。
    /// </summary>
    /// <param name="configId">数据库配置 ID（对应 dbConfigs 中的 ConfigId）</param>
    /// <returns>表名与表描述列表，包含状态标记（HasController、IsImported）</returns>
    Task<IReadOnlyList<TaktDatabaseTableInfo>> GetTablesWithGeneratorAsync(string configId);
